...
The worked integral above already showed the branch behavior (e.g., \(x=\sin y\) vs.\ \(x=\tanh u\)) under domain constraints. Here we keep that example as a template and make the search logic explicit: domain-incompatible heuristic branches are pruned, safe branches can converge to the same antiderivative, and residual checks provide the goal test at each accepted leaf.
