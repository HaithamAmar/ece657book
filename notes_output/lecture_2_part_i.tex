...
We define \emph{safe transformations} as invertible substitutions that allow back-substitution: if \(u=\phi(x)\) and \(F_u\) is an antiderivative of \(T[g](u)\), then \(F_u\!\circ\!\phi\) differentiates back to \(g(x)\). We write \(F\) for antiderivatives of \(f\)-type terms and \(G\) for antiderivatives of \(g\)-type terms, so \(F'(x)=f(x)\) and \(G'(x)=g(x)\). Safe transformations are algebraic substitutions or factorings that survive reversal. Examples include:
...
\begin{itemize}
  ...
  \begin{equation}
    ...
    \label{eq:linear_change_of_variables}
  \end{equation}
...
For real-valued integration we restrict attention to \(x \in (-1,1)\), ensuring the denominator \((1 - x^2)^{5/2}\) is well-defined and nonzero on the interval of interest. The antiderivative derived below is valid on this open interval; the integrand has non-integrable singularities at \(x=\pm1\).

When encountering expressions involving \(1 - x^2\), a classical heuristic substitution is the trigonometric setup
\begin{equation}
\begin{aligned}
x &= \sin y,\\
dx &= \cos y \, dy,\\
1 - \sin^2 y &= \cos^2 y.
\end{aligned}
\label{eq:trig_substitution_setup}
\end{equation}
which leverages the Pythagorean identity in a form that immediately exposes a secant power after substitution. This is the standard move for denominators built from \(1-x^2\): \(x=\sin y\) turns \(1-x^2\) into \(\cos^2 y\).
...
Set \(x = \sin y\), so \(dx = \cos y \, dy\) as in \eqref{eq:trig_substitution_setup}.
We take \(y = \arcsin x\) with \(y \in \left(-\frac{\pi}{2}, \frac{\pi}{2}\right)\) so that the substitution remains bijective on \(x \in (-1,1)\). Geometrically, this is the principal branch of \(\arcsin\); on it \(\cos y \ge 0\), so \(\sqrt{1-\sin^2 y}=\cos y\) has no sign ambiguity.
...
&= \int \frac{4\cos y}{\cos^5 y} \, dy \qquad (\text{since }\cos y \ge 0 \text{ on }(-\tfrac{\pi}{2},\tfrac{\pi}{2})) \\
&= 4 \int \cos^{-4} y \, dy \\
&= 4 \int \sec^{4} y \, dy.
\end{align*}
The exponent arithmetic is now explicit: \((\cos^2 y)^{5/2}=\cos^5 y\) on the chosen branch, and \(\cos y \cdot \cos^{-5}y = \cos^{-4}y\).
...
\begin{itemize}
  ...
  \begin{equation}
    ...
    \label{eq:secant_reduction_formula}
  \end{equation}
\end{itemize}
...
To automate the choice, the system applies the cost heuristic defined above (tree depth, nonlinearity, symbol count) to the two safe paths listed in Step 2; the substitution route typically scores lower and is tried first, with the reduction formula held as a fallback if substitution stalls.
...
\begin{itemize}
  ...
  \begin{aligned}
  4 \int \sec^4 y\, dy
    &= 4 \int \sec^2 y\,(\sec^2 y\, dy) \\
    &= 4 \int (1+\tan^2 y)\sec^2 y\, dy \\
    &= 4 \int (1+u^2)\, du \\
  ...
  \]
  Applying this with \(n=4\) to the running integral \eqref{eq:sec4_integral} gives
  \[
  \begin{aligned}
  4\int \sec^{4} y \, dy
    &= 4\left(\frac{\sec^{2} y \tan y}{3} + \frac{2}{3}\int \sec^{2} y \, dy\right) \\
    &= 4\left(\frac{(1+\tan^2 y)\tan y}{3} + \frac{2}{3}\tan y\right) + C \\
    &= 4\left(\tan y + \tfrac{1}{3}\tan^{3} y\right) + C \\
    &= 4\tan y + \tfrac{4}{3}\tan^{3} y + C,
  \end{aligned}
  \]
  matching the substitution branch exactly.
  \end{itemize}

\paragraph{Back-substitution and check}

With \(F\) denoting the antiderivative of \(f\) throughout this chapter, back-substitution is immediate from the chosen branch:
\[
\cos y = \sqrt{1-\sin^2 y} = \sqrt{1-x^2},
\qquad
\tan y = \frac{\sin y}{\cos y} = \frac{x}{\sqrt{1-x^2}}.
\]
The positive square root is the correct one because \(\cos y \ge 0\) on \(y \in \left(-\frac{\pi}{2}, \frac{\pi}{2}\right)\). Using this identity, both paths yield:
\begin{equation}
F(x) = 4 \left[ x (1 - x^2)^{-1/2} + \frac{x^3}{3(1 - x^2)^{3/2}} \right] + C.
\label{eq:running_example_antiderivative}
\end{equation}
Differentiating \eqref{eq:running_example_antiderivative} term by term gives
\[
\begin{aligned}
F'(x)
&= 4\Big[(1-x^2)^{-1/2} + x^2(1-x^2)^{-3/2} \\
&\qquad\quad + x^2(1-x^2)^{-3/2} + x^4(1-x^2)^{-5/2}\Big] \\
&= 4(1-x^2)^{-5/2}\Big[(1-x^2)^2 + 2x^2(1-x^2) + x^4\Big] \\
&= 4(1-x^2)^{-5/2},
\end{aligned}
\]
so the candidate antiderivative passes the residual check on any interior sample set from \((-1,1)\). Outside \((-1,1)\) the principal-branch integrand is complex-valued; a real continuation rewrites the integral as \(\int 4(x^2-1)^{-5/2}dx\) with \(x=\cosh t\).
...
\begin{figure}[h]
        ...
        \ifdefined\HCode
            ...
            \resizebox{\linewidth}{!}{%
                \begin{tikzpicture}[
                    ...
                    % Level 0
                    \node[problem] (root) at (0,0) {\(\displaystyle \int \frac{4}{(1-x^2)^{5/2}}\, dx\)};
                    ...
                    % Level 3
                    \node[safe] (dx)     at (-6.5, -9.0) {Use \(dx=\cos y\, dy\)};
                    \node[safe] (reduce) at (-6.5, -11.0) {Reduce to \(4\int \sec^4 y\, dy\)};
                    \node[goal] (goal)   at (-6.5, -13.0) {GOAL: \(F(y)=4\tan y+\frac{4}{3}\tan^3 y+C\)};
                    \node[dead] (dead)   at ( 4.2, -8.8) {BACKTRACK\\no closed form on this branch};
                    ...
                    \draw[line] (stall) -- (dead);

                    \draw[line] (subs.south) -- (dx.north);
                    \draw[line] (dx.south) -- (reduce.north);
                    \draw[line] (reduce.south) -- (goal.north);
        ...
        \caption{Transformation tree for the running example integral. Badges \textbf{[S]}/\textbf{[H]} mark safe vs.\ heuristic moves; the left branch shows the sequential safe path, while the right branch terminates in explicit backtracking. The tree view makes solved and dead-end leaves visible.}
        \label{fig:lec3_transform_tree}
\end{figure}


\paragraph{Example:} For the integral problem, the root node is the original integral. From there, branches encode concrete substitutions and algebraic manipulations, and failed heuristic paths explicitly return to the previous decision point.
...
\end{itemize}
\end{tcolorbox}
\begin{tcolorbox}[summarybox, title={Transformation toolkit (safe vs.\ heuristic)}]
\textbf{Preconditions.} Keep domains and branch choices explicit. For example, a square-root substitution may require \(x\in(-1,1)\). Assume principal branches unless noted.
\begin{itemize}
    \item \textbf{Constant factor.} \textbf{Safe:} \(\int a\, g(x)\, dx = a\int g(x)\, dx\) with no domain change. \textbf{Heuristic:} complete the square before trig substitutions, and track any branch cuts you introduce.
    \item \textbf{Linear substitution.} \textbf{Safe:} \(u=ax+b,\; dx=du/a\) with \(a\neq0\), invertible on the stated interval. \textbf{Heuristic:} trig substitutions such as \(x=\sin u\), \(x=\tan u\), or \(t=\tan(x/2)\), with stated branch intervals.
    \item \textbf{Polynomial division / partial fractions.} \textbf{Safe:} split an improper rational function into polynomial plus proper fraction while keeping denominators nonzero on the domain. \textbf{Heuristic:} rationalizing substitutions such as \(x=1/u\) or \(x=u^2\) to expose symmetry, while avoiding new zeros or poles.
    \item \textbf{Log-derivative pattern.} \textbf{Safe:} \(\int f'(x)/f(x)\, dx = \log|f(x)|+C\) when \(f(x)\neq 0\) on the declared domain. \textbf{Heuristic:} template lookups such as Beta/Gamma forms or exponential-times-polynomial motifs, with parameter checks.
\end{itemize}
\end{tcolorbox}
...
\textbf{Exercise 1 (Easy).} Implement a small rule set of \emph{safe transformations} (constant extraction, linear substitution, polynomial division). \\
\textbf{Deliverable:} for each proposed antiderivative \(F\), report a residual check \(F'(x)-f(x)\) on a declared domain.

\smallskip
\textbf{Exercise 2 (Medium).} Add one heuristic transformation (e.g., integration by parts, a trig identity, or a pattern match) and implement backtracking when a branch stalls. \\
\textbf{Deliverable:} a trace showing (state, chosen action, backtrack points) on 3 integrals.

\smallskip
\textbf{Exercise 3 (Medium).} Design one adversarial example where a heuristic rewrite \emph{looks} plausible but fails a domain/branch condition. \\
\textbf{Deliverable:} the counterexample + the exact check that catches it.
...
\medskip
\paragraph{Where we head next.} For the data-driven thread (datasets, objectives, diagnostics, classification), continue to \Cref{chap:supervised,chap:logistic}. For nonlinear function classes and nonconvex training dynamics, continue through \Crefrange{chap:perceptron}{chap:mlp}. Later, when you reach fuzzy inference and evolutionary search, reuse this chapter's core habit: define admissible moves, track budgets, and verify outcomes against explicit criteria.
