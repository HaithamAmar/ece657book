...
\end{equation}
which leverages the Pythagorean identity in a form that immediately exposes a secant power after substitution.
...
\end{equation}
Differentiating \eqref{eq:running_example_antiderivative} term by term shows \(F'(x)=4(1-x^2)^{-5/2}\) on \(|x|<1\). Outside \((-1,1)\) the principal-branch integrand is complex-valued; a real continuation rewrites the integral as \(\int 4(x^2-1)^{-5/2}dx\) with \(x=\cosh t\).
