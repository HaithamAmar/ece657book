...
\end{itemize}
Once you can execute this loop for two neurons, scaling to many neurons is mostly bookkeeping; we return to the organization when we scale up.
...
\begin{itemize}
    ...
    \item Here: we build the smallest nonlinear network and derive its gradient updates, setting up the general multilayer training machinery.
\end{itemize}
...
The gradient gives a direction; the step size $\eta$ sets the distance. In practice you pick $\eta$ small enough to avoid oscillation and large enough to make progress. We return to this choice (learning-rate schedules, momentum, and other practical stabilizers) after the core derivative story is solid.
...
\paragraph{Error terms (reusable local signals).}
Define
\begin{equation}
...
Conceptually, nothing magical happens when you go from two neurons to many layers: it is still the chain rule and the same local derivatives. What changes is the \emph{organization}. You run a forward pass that saves what you will need later, then a backward pass that reuses those saved values to compute every gradient efficiently for a whole batch. Later we turn that idea into an implementable algorithm and show the bookkeeping that keeps it sane.
\end{tcolorbox}
...
\begin{itemize}
    ...
    \item \textbf{Handoff:} the same cache-then-backward discipline scales directly; only bookkeeping grows.
\end{itemize}
\end{tcolorbox}
...
\noindent\textbf{Try these while the network is still small.} These reinforce the two\hyp{}neuron derivation and prepare you for multi-layer bookkeeping.
\begin{itemize}
