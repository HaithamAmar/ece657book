...
In this chapter we keep the story linear and concrete. We build the smallest possible network (two neurons in series), define a performance function, and ask the core question: \emph{how should the weights change to improve performance?} Answering that question forces us to use derivatives (the chain rule) and leads naturally to gradient descent. Along the way we encounter a practical obstacle: hard thresholds are not differentiable, so they do not support gradient-based learning. We replace them with smooth activations and immediately gain a clean update story.

The representation is now a small computation graph (units composed into a system), but the workflow stays familiar: define what ``good'' means (an objective), optimize it, and then verify with quick sanity checks before you scale up. This chapter focuses on the training mechanics, and \Cref{chap:backprop} generalizes them to deep networks. \Cref{fig:roadmap} shows this as the hinge between single-unit models and multilayer training.
...
Start small. Keep every intermediate quantity visible. Let the chain rule explain how the learning signal moves, and use quick numerical checks to catch sign and bookkeeping mistakes early.
\end{tcolorbox}
...
\begin{itemize}
    ...
    \item \textbf{Fix:} pick an activation that actually lets gradients flow (smooth and differentiable).
\end{itemize}
...
The same objective\(\rightarrow\)audit workflow from the supervised toolkit still applies; what changes is the \emph{representation}. You still define what ``good'' means, optimize for it, and then verify. Here, verification starts with fail-fast checks (sign conventions, cached intermediates, numerical gradient sanity checks) before you scale up.
\begin{itemize}
...
We start with squared error here for a simple reason: it keeps the algebra short, so you can see the chain rule without distractions. The point of the chapter is not ``squared error is always the right loss''; the point is learning to turn a forward computation into correct weight updates. Once that story is clear, you can swap the objective and redo only the small set of derivatives that touch the output layer.
\end{tcolorbox}
...
In practice you do not want to recompute expensive expressions during learning. For the sigmoid, once you have computed the output \(y=\sigma(p)\), you also have its slope for free: \(\sigma'(p)=y(1-y)\). This is a small example of a bigger pattern: the forward pass is not just for predictions; it also produces the cached intermediate values that the backward pass reuses.
\end{tcolorbox}
...
Conceptually, nothing magical happens when you go from two neurons to many layers: it is still the chain rule and the same local derivatives. What changes is the \emph{organization}. You run a forward pass that saves what you will need later, then a backward pass that reuses those saved values to compute every gradient efficiently for a whole batch. \Cref{chap:backprop} turns the recursion into an implementable algorithm and shows the bookkeeping that keeps it sane.
\end{tcolorbox}
...
\noindent\textbf{Try these while the network is still small.} These reinforce the two\hyp{}neuron derivation and prepare you for the multi-layer bookkeeping in \Cref{chap:backprop}.
\begin{itemize}
...
\end{itemize}
\medskip
\noindent\textbf{If you are reading for implementation.} Be able to read a forward graph and a backward recursion: local derivatives, cached activations, and a scalar loss driving parameter updates. That is exactly what \Cref{chap:backprop} scales up.
\end{tcolorbox}
