...
\index{multi-layer perceptron (MLP)}
\index{chain rule}
...
\begin{itemize}
    ...
    \item Choose a scalar performance function \(P\), explain what ``improving performance'' means, and sanity\hyp{}check one tiny update numerically.
    \item Use the chain rule to derive the gradients for \(w_2,b_2,\mathbf{w}_1,b_1\) and interpret \(\delta\) terms as reusable local error signals.
    \item Explain why hard thresholds break derivative\hyp{}based learning and why smooth activations restore a usable learning signal.
\end{itemize}
\end{tcolorbox}
...
\begin{aligned}
 ...
 p_2 &= w_2 y_1 + b_2, & y_2 &= f(p_2).
\end{aligned}
\label{eq:two_neuron_forward}
\end{equation}
...
\textbf{Exercise 1 (Easy).} Implement finite differences for the two\hyp{}neuron chain and compare to your analytic gradients. \\
\textbf{Deliverable:} the worst relative error across parameters + the exact \(\epsilon\) you used and why it was not too large/small.

\smallskip
\textbf{Exercise 2 (Medium).} Replace the smooth activation with a hard threshold and attempt to train with the same gradient-descent recipe. Explain, in one paragraph, \emph{exactly} what breaks (mathematically and numerically) and which workaround you would adopt in practice (smooth surrogate, straight-through estimator, or a different optimization view). \\
\textbf{Deliverable:} one training trace (loss vs.\ steps) + the paragraph explanation.

\smallskip
\textbf{Exercise 3 (Medium).} Train a tiny MLP on XOR and treat sensitivity as a debugging signal: run 10 random initializations under the same hyperparameters, then diagnose the variance. \\
\textbf{Deliverable:} a plot of decision regions for 2 successful and 2 failed runs + one paragraph linking the outcomes to initialization scale and activation saturation.
\medskip
