...
With one perceptron, every training example pushes on the same single separator. A network changes the situation by creating intermediate signals that can specialize: some units respond to one pattern, others respond to another, and a later unit can combine their responses. That internal structure is what turns simple units into a flexible nonlinear model. You can read the intermediate signals as learned transformations of the input whose job is to make the final separation easier.
