...
\Cref{chap:perceptron} introduced the perceptron and Adaline: single units that learn by updating weights from data, with Adaline giving our first explicit glimpse of gradient descent on a smooth performance function. In this chapter we keep the story linear and concrete. We build the smallest possible network (two neurons in series), define a performance function, and ask the core question: \emph{how should the weights change to improve performance?} Answering that question forces us to use derivatives (the chain rule) and leads naturally to gradient descent. Along the way we encounter a practical obstacle: hard thresholds are not differentiable, so they do not support gradient-based learning. We replace them with smooth activations and immediately gain a clean update story. This is the conceptual bridge to full backpropagation in \Cref{chap:backprop}. The roadmap in \Cref{fig:roadmap} shows this as the hinge between single-unit models and multilayer training.
...
\begin{tcolorbox}[summarybox,title={A short roadmap: building a trainable network}]
...
\begin{tcolorbox}[summarybox,title={How this chapter fits the workflow}]
...
\end{align}
Even this tiny network introduces the central idea of neural networks: intermediate computations (here $y_1$) are reused and influence the final output $y_2$. A single neuron is a linear classifier; chaining neurons gives a nonlinear representation. With multiple hidden units, that added structure is enough to solve XOR.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        \draw[->] (x) -- node[lab, above, pos=0.55, yshift=6pt, font=\scriptsize] {$p_1=\mathbf{w}_1^\top\mathbf{x}+b_1$} (n1);
        \draw[->] (n1) -- node[lab, below, pos=0.5, yshift=-2pt] {$y_1=f(p_1)$} (n2);
        \draw[->] (n2) -- node[lab, above, pos=0.5, xshift=-6pt, yshift=8pt, font=\scriptsize] {$p_2=w_2 y_1+b_2$} (y);
        \node[lab, below=4pt of y] {$y_2=f(p_2)$};
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Schematic: The minimal neural network used in this chapter is a two-neuron chain. The first unit produces an intermediate signal, and the second unit maps that signal to the final output. Use this when tracking variables and gradients in a toy network before scaling to deeper models.}
    \label{fig:mlp_minimal_chain}
\end{figure}

\begin{tcolorbox}[summarybox,title={Author's note: why a network changes the story}]
...
\begin{tcolorbox}[summarybox,title={A checklist of what we must settle (and why)}]
...
For a fixed input, the performance becomes a surface over the weights. If the surface looks like a ``bowl,'' then the bottom is the optimum. The goal is to move weights along this surface in the direction that improves performance.
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Schematic: Think of performance as a surface over the weights. Gradient descent moves in one vector step (blue), whereas coordinate-wise updates can zig-zag (orange). Use this when building intuition for why gradient directions reduce loss more efficiently than axis-aligned steps.}
...
At this point the story is simple: define $P$, compute $\nabla P$, and update weights. The catch is that computing $\nabla P$ requires derivatives through the activation. When we apply the chain rule to \Cref{eq:two_neuron_forward}, factors like $f'(p_1)$ and $f'(p_2)$ appear immediately.
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Schematic: Hard thresholds block gradient-based learning because the derivative is zero almost everywhere. A smooth activation like the sigmoid provides informative derivatives across a wide range of inputs. Use this when motivating why smooth nonlinearities enable gradient-based training.}
...
\subsection{Deriving weight updates for the two\hyp{}
\label{sec:mlp_deriving_weight_updates_for_the_two}neuron network}
\label{sec:mlp_deriving_weight_updates_for_the_two_sec_mlp_deriving_weight_updates_for_the_two_neuron_network}
...
We now compute the gradients in \Cref{eq:two_neuron_forward} using the chain rule. First note the easy derivatives:
\begin{itemize}
...
Take a single input $x = [1,-1]^\top$, target $t=1$, sigmoid activation $f=\sigma$, and parameters
...
Thus $\nabla_{\mathbf{w}_1}P=\delta_1 x \approx [-0.018,\,+0.018]^\top$ and $\nabla_{w_2}P=\delta_2 y_1\approx -0.050$.
\medskip
...
Nothing essential changes for deeper networks; we simply apply the same chain rule repeatedly. For a layer $l$ with pre\hyp{}activations $\mathbf{p}^{(l)}$ and weights $W^{(l+1)}$, the error signal satisfies
\begin{equation}
\delta^{(l)} = \bigl(\delta^{(l+1)} (W^{(l+1)})^\top\bigr) \circ f'(\mathbf{p}^{(l)}),
...
\medskip
\paragraph{Where we head next.} \Cref{chap:backprop} generalizes this derivation to arbitrary depth and shows how to implement the backward pass efficiently for batches.
