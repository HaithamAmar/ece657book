...
The dimensionality-reduction goals were covered in \Cref{sec:som_dimensionality_reduction_simplifying_high_dimensional_data}; here we only note that SOMs implement a discrete lattice embedding rather than a continuous Euclidean map. The mapping \(f:\mathbb{R}^n \rightarrow \mathbb{R}^m\) is realized implicitly by the learned prototypes and their lattice coordinates, which we define in the SOM construction below.
