...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        panel/.style={draw=gray!34, rounded corners=8pt, fill=gray!3, minimum width=5.2cm, minimum height=5.35cm}
    ]
        % Left panel: grid
        \begin{scope}
            \node[panel] at (2.0,2.0) {};
            \node[font=\normalsize\bfseries] at (2.0,4.48) {BMU neighborhood};
            % draw grid of neurons
            \foreach \i in {0,...,4} {
                \foreach \j in {0,...,4} {
                    \filldraw[fill=white, draw=gray!60] (\i,\j) circle (2.5pt);
                }
            }
            % BMU and neighbors
            \filldraw[fill=cbBlue, draw=cbBlue] (2,2) circle (2.9pt);
            \foreach \p in {(1,2),(3,2),(2,1),(2,3),(1,1),(1,3),(3,1),(3,3)} {
                \filldraw[fill=cbGreen!65, draw=cbGreen!65] \p circle (2.7pt);
            }
            \draw[cbBlue, line width=1.3pt] (2,2) circle (1.28);
            % lattice distance scale (one-step neighbor)
            \draw[<->, gray!74, line width=0.9pt] (2,2.28) -- (3,2.28);
            \node[font=\small, gray!82] at (2.62,2.58) {grid step = 1};
            \node[font=\small, fill=white, rounded corners=2pt, inner sep=1.5pt] at (2.0,1.45) {BMU};
        \end{scope}

        % Right panel: U-Matrix (toy heatmap; colored but grayscale-friendly)
        \begin{scope}[shift={(6.05,0)}]
            \node[panel, minimum width=5.55cm, minimum height=5.35cm] at (2.35,2.0) {};
            \node[font=\normalsize\bfseries] at (2.35,4.48) {U-Matrix};
            \begin{axis}[
                width=4.9cm,
                height=4.9cm,
                view={0}{90},
