...
Three properties are worth keeping in mind. First, the SOM output grid is fixed by design: you choose the grid size up front, and it does not automatically scale with input dimension. Second, learning is driven by competition: each input selects a best-matching unit (BMU). Third, SOMs add cooperation on top of winner-takes-all by updating the BMU and its neighbors together, which is the mechanism that creates smooth organization on the grid rather than a shuffled bag of prototypes.
% Chapter 9 (continued)
...
Each training step repeats the same loop: present an input, identify the BMU using your chosen discriminant (dot product or distance), update the BMU and its neighbors with the cooperative rule, and then decay both the learning rate and neighborhood radius according to the schedule. Over many passes through the data, the neighborhood starts broad (to settle global ordering) and then shrinks (to refine local detail without tearing the map).
