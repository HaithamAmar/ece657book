...
\index{quantization error (QE)}
\index{topographic error (TE)}
\index{U-matrix}
...
Self-organizing networks aim to discover structure in input data without labels. The most prominent example is the \emph{Self-Organizing Map} (SOM), introduced by Teuvo Kohonen \citep{Kohonen1982}. SOMs are used for clustering and for building a visualization-friendly map of a high-dimensional dataset.
...
Two practical SOM diagnostics show up repeatedly: the U-Matrix (a neighbor-distance heat map) and component planes (feature-wise maps). We will use both later as quick audits for whether the map is organizing structure (rather than just shuffling prototypes). A compact definition is: for each unit \(i\) with prototype \(\mathbf{w}_i\), the U\hyp{}Matrix value is the average distance to its grid neighbors \(\mathcal{N}(i)\),
\[
U(i)=\frac{1}{|\mathcal{N}(i)|}\sum_{j\in\mathcal{N}(i)}\|\mathbf{w}_i-\mathbf{w}_j\|,
\]
where \(\mathcal{N}(i)\) is typically the 4\hyp{}neighbor or 8\hyp{}neighbor set on the 2D grid, excluding unit \(i\) itself.
...
\begin{enumerate}
    ...
    \]
    Here \( |C_k| \) is the number of points in cluster \( C_k \).
...
\subsection{Feature Mapping: From Prototype Vectors to Grid Coordinates}
...
\begin{equation}
    \begin{aligned}
        y_i &= \mathbf{w}_i^\top \mathbf{x} \qquad &&\text{(dot-product similarity)},\\
        d_i &= \|\mathbf{x} - \mathbf{w}_i\|_2^2 \qquad &&\text{(squared Euclidean distance)}.
    \end{aligned}
    \label{eq:som_similarity_measures}
\end{equation}
...
c =
\begin{cases}
\arg \max_i y_i, & \text{dot-product similarity},\\
\arg \min_i d_i, & \text{squared Euclidean distance}.
\end{cases}
\]
...
First, the SOM output grid is fixed by design. You choose the grid size up front, and that choice acts like the \emph{resolution} of your map: a larger grid gives you more prototypes and finer visual detail, but it also needs enough data (and enough training) to populate the map meaningfully. A smaller grid forces a coarser organization and can merge behaviors that you might prefer to separate.

Second, learning is driven by competition. Each input selects a best\hyp{}matching unit (BMU), and the BMU's prototype vector is the ``closest representative'' under your similarity rule. If you collapse the neighborhood to a winner\hyp{}only update (i.e., \(\sigma(t)\to 0\)), the algorithm becomes a form of online vector quantization: prototypes still move toward the data, but the \emph{layout} of prototypes on the grid has no reason to be ordered.

Third, SOMs add cooperation on top of winner\hyp{}takes\hyp{}all by updating the BMU and its neighbors together. That cooperation is the mechanism that creates smooth organization on the grid rather than a shuffled bag of prototypes: broad neighborhoods early encourage global ordering, and shrinking neighborhoods later refine local structure without tearing the map.
% Chapter 9 (continued)
...
The discriminant measures in \Cref{eq:som_similarity_measures} are the same ones used in the earlier example; we reuse them here, favoring the distance form when deriving updates.
...
\subsection{Winner-Takes-All Learning: Consolidation After the Numerical Example}
    ...
    \begin{tikzpicture}
        \begin{groupplot}[
            ...
            \nextgroupplot[title={Unregularized}]
                \ifdefined\HCode
                    \addplot3[surf, shader=flat, opacity=0.92, domain=-2:2, y domain=-2:2]{0.8*x^2 + 0.05*y^2};
                \else
                    \addplot3[surf, shader=interp, opacity=0.92, domain=-2:2, y domain=-2:2]{0.8*x^2 + 0.05*y^2};
                \fi
        ...
        \end{groupplot}
    \end{tikzpicture}
    \caption{Regularization smooths an objective surface (schematic). Coupling neighboring prototypes (right) yields wider, flatter basins than the narrow, ill-conditioned unregularized basin (left).}
...
The lesson is that QE is a ``prototype fit'' score, while TE is an ``ordering'' score. Improving QE alone does not prevent a torn map when nearby grid units represent unrelated regions of the input space.
\end{tcolorbox}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        panel/.style={draw=gray!24, rounded corners=6pt, line width=0.65pt, fill=white, minimum width=5.2cm, minimum height=5.25cm}
    ]
        % Left panel: grid
        \begin{scope}
            \node[panel] at (2.0,2.0) {};
            \node[font=\small\bfseries] at (2.0,4.38) {BMU neighborhood};
            % draw grid of neurons
            \foreach \i in {0,...,4} {
                \foreach \j in {0,...,4} {
                    \filldraw[fill=white, draw=gray!58] (\i,\j) circle (2.6pt);
                }
            }
            % BMU and neighbors
            \filldraw[fill=cbBlue, draw=cbBlue] (2,2) circle (3.2pt);
            \foreach \p in {(1,2),(3,2),(2,1),(2,3),(1,1),(1,3),(3,1),(3,3)} {
                \filldraw[fill=cbGreen!65, draw=cbGreen!65] \p circle (2.95pt);
            }
            \draw[cbBlue, line width=1.35pt] (2,2) circle (1.32);
            % lattice distance scale (one-step neighbor)
            \draw[<->, gray!70, line width=0.9pt] (1.96,2.29) -- (3.04,2.29);
            \node[font=\scriptsize, gray!72] at (2.62,2.52) {step = 1};
            \node[font=\scriptsize, fill=white, rounded corners=1.5pt, inner sep=1pt] at (2.0,1.48) {BMU};
        \end{scope}

        % Right panel: U-Matrix (toy heatmap; colored but grayscale-friendly)
        \begin{scope}[shift={(6.05,0)}]
            \node[panel, minimum width=5.55cm, minimum height=5.25cm] at (2.35,2.0) {};
            \node[font=\small\bfseries] at (2.35,4.38) {U-Matrix};
            \begin{axis}[
                width=5.24cm,
                height=5.24cm,
                view={0}{90},
                ...
                colorbar style={
                    ...
                    anchor=north,
                    width=3.20cm,
                    xtick={0.30,0.60,0.90},
                    xticklabel style={font=\scriptsize},
                    xlabel={neighbor distance},
    ...
    \end{adjustbox}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Left: the best matching unit (blue) and its neighborhood on a 5-by-5 SOM grid. Right: a toy U-Matrix of average neighbor distances; lighter cells hint at cluster boundaries. Treat it as a qualitative boundary cue unless preprocessing and scaling are fixed.}
    \label{fig:lec5-som-lattice-umatrix}
\end{figure}

Component planes are the companion view to the U-Matrix: they show one feature at a time across the grid, which helps you interpret what a boundary is \emph{about} (\Cref{fig:lec5-som-component-planes}).
...
Cooperative competitive learning is the winner\hyp{}takes\hyp{}all loop with one extra ingredient: a neighborhood kernel on the \emph{grid}. For each input \(\mathbf{x}(t)\), you (i) compute match scores, (ii) pick the BMU \(c\), (iii) compute neighborhood weights from grid distances \(\|\mathbf{r}_j-\mathbf{r}_c\|\), and (iv) update the BMU and its neighbors with the cooperative rule. The neighborhood term is what turns ``a set of prototypes'' into ``an organized map.''

The two schedules play different roles. The learning rate \(\alpha(t)\) controls how fast prototypes move; the neighborhood width \(\sigma(t)\) controls \emph{how much} the map is encouraged to be globally ordered. If \(\sigma(t)\) shrinks too quickly, the map can freeze before it has a chance to organize (a common source of topology errors). If \(\sigma(t)\) stays large for too long, the map can remain smooth but blurry, with weak separation between regions. This is why the monitoring tools introduced earlier (e.g., QE/TE and stability across seeds) are not decoration: they are how you tell whether the schedule is producing the behavior you intended.
\subsection{Synthesis: SOM Training Rules, Annealing Schedules, and Design Choices}
...
\noindent It is useful to tell the SOM story in \emph{three stages}, even though the code is often written as a longer checklist. The stages are: \textbf{Initialization} (seed the prototype grid), \textbf{Competition} (pick a best-matching unit for each input), and \textbf{Cooperation} (update a neighborhood and decay \(\alpha(t)\) and \(\sigma(t)\)). The six-step procedure in \Cref{sec:som_training_steps} is simply one way to operationalize those stages when implementing the algorithm and deciding when to stop.
...
\textbf{A practical recipe (engineering lens):} Without a strong prior, treat these as starting points rather than rules. Use \(5\sqrt{N}\)--\(10\sqrt{N}\) units when unsure; hex grids reduce anisotropy; wrap-around (toroidal) grids reduce edge effects. Initialize \(\mathbf{w}_j\) from data or along the first two PCs; pick \(\alpha_0\in[0.1,0.5]\) and decay toward \(\alpha_{\min}\approx 10^{-3}\); set \(\sigma_0\) to roughly the map radius and decay toward 1--1.5 cells.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Related and growing variants}]
\textbf{Neural Gas / Growing Neural Gas} \citep{MartinetzBerkovichSchulten1993,Fritzke1994GrowingNeuralGas} drop the fixed grid and instead learn neighborhood structure (and, in the growing variants, add units dynamically). \textbf{Generative topographic mapping (GTM)} (Bishop et al.) provides a probabilistic, topographic embedding with likelihoods/uncertainty. A useful way to place the landscape is this: k-means gives prototypes with no organization; SOMs add a fixed organizing prior; these variants let the organization itself adapt.
\end{tcolorbox}
...
\textbf{Exercise 1 (Easy).} Train a small SOM on a simple 2D dataset where geometry is visible (two clusters, a ring, or a Swiss-roll projection). Visualize the grid and how input points map to BMUs; then plot the U\hyp{}matrix. \\
\textbf{Deliverable:} the mapping plot + the U\hyp{}matrix + one paragraph explaining what the U\hyp{}matrix is revealing.

\smallskip
\textbf{Exercise 2 (Medium).} Train a \(10\times 10\) SOM on a higher-dimensional dataset (e.g., MNIST). Track QE and TE during training and show how input normalization and neighborhood annealing change both metrics. \\
\textbf{Deliverable:} QE/TE curves + one set of component-plane plots + one concrete failure mode you observed (tearing, collapse, or loss of topology).

\smallskip
\textbf{Exercise 3 (Medium).} Compare SOM prototypes to K\hyp{}means centers on the same dataset. Treat the comparison as a question about \emph{what is being optimized}: clustering distortion vs.\ topology preservation. \\
\textbf{Deliverable:} a side-by-side visualization (SOM vs.\ K\hyp{}means) + a short discussion of which method is easier to audit for structure (and why).
