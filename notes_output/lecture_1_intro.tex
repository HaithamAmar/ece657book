...
Intelligent systems are engineered artifacts that perceive, reason, and act under constraints. This chapter introduces the working vocabulary used throughout the book---historical context, core definitions, and recurring design themes. The roadmap in \Cref{fig:roadmap} anchors the strand sequence and reading paths.
...
\begin{itemize}
    \item \textbf{Perception:} Camera captures images, which are converted into numerical arrays.
    \item \textbf{Thinking:} Algorithms classify objects (e.g., stop signs, pedestrians) and predict future states.
    \item \textbf{Action:} Vehicle control systems adjust steering, acceleration, or braking accordingly.
\end{itemize}
...
\begin{description}[style=nextline]
    ...
    \item[Hardware (perception/action)] CMOS sensor, on-board DSP/accelerator, motorized pan/tilt for re-targeting.
    \item[Software (reasoning)] YOLOv8-style detector fine-tuned on site-specific data, fused with Kalman filters for track smoothing and MPC logic that commands the pan/tilt actuator or triggers alerts.
...
Intelligence in systems is often characterized by the ability to:

\begin{itemize}
    \item \textbf{Perceive} and interpret inputs from the environment.
    \item \textbf{Reason} and make decisions based on available information.
    \item \textbf{Learn} from experience to improve performance.
    \item \textbf{Act} autonomously to achieve goals.
\end{itemize}
...
Swarm intelligence can be formalized via decentralized update laws of the form $x_i(t+1) = f\big(x_i(t),\{x_j(t)\}_{j \in \mathcal{N}_i}\big)$, where each agent $i$ interacts only with its neighborhood $\mathcal{N}_i$. Similar update patterns appear again in \Cref{chap:evo}, but we do not focus on stability proofs in this book.
...
\emph{Strong} machine intelligence, in my view, means a system can make itself the subject of its own thought. It keeps self-models (confidence monitors, policy auditors, explanation traces) to critique and revise its reasoning, not just its predictions.\\
The rest of this book uses the simpler four-layer taxonomy (reactive~$\rightarrow$ deliberative~$\rightarrow$ adaptive~$\rightarrow$ meta-cognitive), but the ``subject of its own thought'' view motivates why Level~4 systems need special care in design and governance.
\end{tcolorbox}
...
One practical mitigation is to build systems that:
\begin{itemize}
    \item Keep auditable records of their decisions and updates.
    \item Perform self-inspection and error analysis.
    \item Backtrack and self-correct within explicit, designer-defined bounds.
\end{itemize}
...
\begin{figure}[!b]
    ...
    \caption{Schematic: Roadmap (core supervised path; SOM/fuzzy; optimization/evolutionary).}
    \label{fig:roadmap}
\end{figure}
...
\begin{enumerate}
    \item \textbf{ML-focused path:} \Crefrange{chap:supervised}{chap:logistic} $\rightarrow$ \Crefrange{chap:mlp}{chap:backprop} $\rightarrow$ \Crefrange{chap:cnn}{chap:transformers} $\rightarrow$ \Cref{chap:nlp}.
...
\begin{itemize}
    ...
    \item Use \Cref{fig:roadmap} to choose a reading path and write down which two chapters you will skim first (and why).
\end{itemize}
\end{tcolorbox}

\paragraph{Where we head next.} To make the abstract vocabulary of ``intelligence'' concrete, we start with a deliberately small case study: symbolic integration as transformation search (\Cref{chap:symbolic}). It is a crisp example of a system that decomposes a problem, applies safe rules, branches heuristically, backtracks, and certifies progress or failure with residual checks---capabilities that reappear later, often in data\hyp{}driven form.
