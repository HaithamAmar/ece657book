...
\index{intelligent system}
\index{representation}
\index{audit}
\index{meta-cognition}
...
\begin{itemize}
    \item Define what we mean by an \emph{intelligent system} and distinguish it from an \emph{intelligent machine}.
    \item Place the main strands of modern AI (logic, computation, learning) in a short historical arc.
    \item Use the book's organizing lenses (components, levels, and the representation\(\rightarrow\)objective\(\rightarrow\)update\(\rightarrow\)audit loop) to situate a method and anticipate its failure modes.
\end{itemize}
\end{tcolorbox}

Use this chapter as a standing reference: later tools reuse the same system vocabulary and the same habit of checking assumptions against constraints.

\begin{tcolorbox}[summarybox, title={Design motif}]
\textbf{The unified contract.} Across this book, every intelligent system instantiates the same engineering loop:

\emph{(i) Representation:} choose what the system stores about the world/problem.\\
\emph{(ii) Update/action:} choose how that representation changes (rules, gradients, energy descent, population operators).\\
\emph{(iii) Objective:} define what ``better'' means (loss, likelihood, energy, constraint satisfaction, fitness).\\
\emph{(iv) Audit:} define checks that catch silent failure (leakage, miscalibration, instability, constraint violations, metric gaming).

Paradigms differ mainly in the \emph{space being searched} (parameters, rules, programs, prototypes, populations) and the \emph{feedback signal} available (labels, self\hyp{}supervision, rules, fitness). The rest of the book is a tour of design points under this contract; the back-matter synthesis in \Cref{fig:big-picture-map} and \Cref{tab:big-picture-models} summarizes the map once the chapter tour is complete.
\end{tcolorbox}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        >=Stealth,
        box/.style={draw=gray!45, rounded corners=8pt, line width=0.9pt, text width=5.3cm, minimum height=2.6cm, align=center, inner sep=10pt},
        looparrow/.style={->, line width=1.1pt, gray!72, shorten >=2pt, shorten <=2pt},
        badge/.style={circle, draw=gray!45, fill=white, minimum size=20pt, inner sep=0pt, font=\bfseries\small}
    ]
        \node[box, fill=cbBlue!10]   (repr) at (0,2.05) {%
            {\textbf{\Large Representation}\par}
            \vspace{2pt}
            {\large What is stored?\par}
            \vspace{5pt}
            {\footnotesize\color{black!55}state, features, rules, prototypes}
        };
        \node[box, fill=cbOrange!12] (obj)  at (7.0,2.05) {%
            {\textbf{\Large Objective}\par}
            \vspace{2pt}
            {\large What counts as better?\par}
            \vspace{5pt}
            {\footnotesize\color{black!55}loss, likelihood, energy, fitness}
        };
        \node[box, fill=cbGreen!12]  (upd)  at (7.0,-2.05) {%
            {\textbf{\Large Update}\par}
            \vspace{2pt}
            {\large How does it change?\par}
            \vspace{5pt}
            {\footnotesize\color{black!55}gradients, rules, search,\par population operators}
        };
        \node[box, fill=cbPink!12]   (aud)  at (0,-2.05) {%
            {\textbf{\Large Audit}\par}
            \vspace{2pt}
            {\large What can fail silently?\par}
            \vspace{5pt}
            {\footnotesize\color{black!55}leakage, instability, calibration,\par constraints}
        };
        \node[badge] at ([xshift=10pt,yshift=-10pt]repr.north west) {1};
        \node[badge] at ([xshift=10pt,yshift=-10pt]obj.north west) {2};
        \node[badge] at ([xshift=10pt,yshift=-10pt]upd.north west) {3};
        \node[badge] at ([xshift=10pt,yshift=-10pt]aud.north west) {4};

        \draw[looparrow] (repr.east) -- (obj.west);
        \draw[looparrow] (obj.south) -- (upd.north);
        \draw[looparrow] (upd.west) -- (aud.east);
        \draw[looparrow] (aud.north) -- (repr.south);
    \end{tikzpicture}
    \caption{The recurring engineering loop used throughout the book. Later parts keep the loop but shift where the main design pressure sits.}
    \label{fig:unified_contract_loop}
\end{figure}
\FloatBarrier

The rest of the book can be read as controlled variation on \Cref{fig:unified_contract_loop}. Some chapters keep the objective fixed and change the representation; others keep the representation and feedback signal fixed but change the update rule; later chapters make the audit burden itself a first-class design problem.

\paragraph{What we mean by a model.}
In this book, a \emph{model} is any system that maps observed information to a prediction about an outcome.
Sometimes the prediction is a single value (regression), and often it is a probability distribution over
possible outcomes (classification). The details change from chapter to chapter, but the contract stays the
same: choose a representation, define what it means to be wrong, and then tune the parameters so the
predictions improve under an honest evaluation.
...
% (Intentionally omitted: course logistics/administrivia. This book is written to stand alone.)
...
\subsection{Intelligent Systems}\label{sec:intro_intelligent_systems}
...
\begin{description}[style=nextline]
    ...
    \item[Objectives/constraints] Maintain $<200\,\si{\milli\second}$ end-to-end latency and $<1\%$ false negatives; respect privacy/retention policies.
    ...
    \item[Software (reasoning)] A YOLO-style object detector (e.g., YOLOv8 \citep{Jocher2023UltralyticsYOLOv8}) fine-tuned on site-specific data, fused with Kalman filters for track smoothing and MPC (model predictive control) logic that commands the pan/tilt actuator or triggers alerts.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        >=Stealth,
        strand/.style={draw=gray!48, rounded corners=8pt, line width=1.05pt, minimum width=2.35cm, minimum height=1.12cm, align=center, inner sep=8pt},
        lane/.style={font=\small\bfseries\color{gray!86}, align=right},
        patharrow/.style={->, line width=1.35pt, gray!74},
        connector/.style={->, line width=1.1pt, gray!60},
        crosslink/.style={->, dashed, line width=1.0pt, gray!46}
    ]
        \node[lane] at (-1.65,2.05) {Optimization\\thread};
        \node[lane] at (-1.55,0.0) {Core supervised\\path};
        \node[lane] at (-1.65,-2.05) {Soft-computing\\branch};

        \node[strand, fill=cbPink!15]   (opt) at (4.2,2.05) {Optimization\\[-1pt]\footnotesize GD / regularization};
        \node[strand, fill=cbPink!22]   (ga)  at (6.95,2.05) {Evolutionary\\[-1pt]\footnotesize GA / GP};

        \node[strand, fill=cbBlue!15]    (reg) at (0.0,0.0) {Linear\\[-1pt]\footnotesize regression};
        \node[strand, fill=cbBlue!15]    (log) at (2.20,0.0) {Logistic\\[-1pt]\footnotesize regression};
        \node[strand, fill=cbBlue!18]    (mlp) at (4.40,0.0) {MLP\\[-1pt]\footnotesize backprop};
        \node[strand, fill=cbBlue!18]    (cnn) at (6.60,0.0) {CNN\\[-1pt]\footnotesize conv / pool};
        \node[strand, fill=cbBlue!18]    (rnn) at (8.80,0.0) {RNN\\[-1pt]\footnotesize BPTT};
        \node[strand, fill=cbGreen!18]   (trf) at (11.00,0.0) {Transformer\\[-1pt]\footnotesize attention};

        \node[strand, fill=cbGreen!18]   (som) at (4.40,-2.05) {SOM\\[-1pt]\footnotesize competitive};
        \node[strand, fill=cbOrange!18]  (fuzzy) at (6.95,-2.05) {Fuzzy sets\\[-1pt]\footnotesize \& inference};

        \draw[patharrow] (reg) -- (log);
        \draw[patharrow] (log) -- (mlp);
        \draw[patharrow] (mlp) -- (cnn);
        \draw[patharrow] (cnn) -- (rnn);
        \draw[patharrow] (rnn) -- (trf);

        \draw[patharrow] (opt) -- (ga);
        \draw[connector] (mlp.south) -- (som.north);
        \draw[patharrow] (som) -- (fuzzy);

        \draw[crosslink] (ga.south) to[bend left=18] (trf.north);
        \draw[crosslink] (fuzzy.north) to[bend right=18] (rnn.south);
    \end{tikzpicture}
    \end{adjustbox}
    \captionsetup{justification=raggedright}
    \caption{Roadmap of the book strands. The central path carries the predictive-modeling arc from linear models to Transformers; the lower branch collects competitive learning and fuzzy reasoning; the upper thread tracks optimization ideas that culminate in evolutionary search.}
    \label{fig:roadmap}
\end{figure}

\Cref{fig:roadmap} summarizes the narrative arc of the book: a core supervised path (linear and logistic regression to MLPs to CNNs to RNNs and attention/Transformers), a branch through competitive learning and fuzzy inference for rule-based reasoning, and a parallel thread on optimization culminating in evolutionary computing. Early on, \Cref{chap:symbolic} provides a complementary symbolic-search perspective so we can contrast ``intelligence via transformations'' with ERM-based modeling. Chapters cross-reference one another so you can skim the path most relevant to your project and return for foundational refreshers as needed.
...
\end{enumerate}
Reinforcement learning is a major pillar of modern AI, but it is not a primary focus of this book; for a dedicated treatment, see \citep{SuttonBarto2018}.
...
\begin{itemize}
    \item Intelligent systems integrate perception, decision, and action; we judge them by behavior under constraints, not by anthropomorphic language.
    ...
    \item ``Levels'' are an organizing lens; meta\hyp{}cognition is treated operationally (self\hyp{}monitoring and bounded self\hyp{}correction), and audits are part of the design, not an afterthought.
\end{itemize}
\medskip
...
Even a technically correct model can be harmful when it is deployed as a decision rule. When systems touch people, treat audits (slices, calibration, robustness, and constraint checks) as part of the design contract, not as optional polish.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Exercises}]
\textbf{Exercise 1 (Easy).} Pick one engineered system you know (a recommender, a robot, a control loop, a monitoring pipeline) and write its design in the book's vocabulary: \emph{percepts}, \emph{representation}, \emph{actions}, \emph{objective}, and \emph{constraints}. \\
\textbf{Deliverable:} a one-page block diagram + 5 bullets (one per term) written concretely for your system.

\smallskip
\textbf{Exercise 2 (Medium).} Choose one plausible failure mode for that system that would \emph{not} show up as a single headline metric drop (a silent failure). Define an audit that would expose it (slice, calibration check, constraint test, or robustness probe). \\
\textbf{Deliverable:} the failure-mode description + an explicit test procedure + the pass/fail criterion.

\smallskip
\textbf{Exercise 3 (Medium).} Use \Cref{fig:roadmap} to justify a learning path for your system: which representation family would you start with (symbolic, supervised, neural, fuzzy, evolutionary) and why? \\
\textbf{Deliverable:} a short paragraph that names (i) the representation, (ii) the update/search operator, (iii) the feedback signal, and (iv) the first audit you would run.
\par\medskip
