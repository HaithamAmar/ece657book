...
Think of this chapter as the front matter you will keep returning to: when later chapters introduce new tools, they reuse the same system vocabulary and the same habit of checking assumptions against constraints.
...
\begin{figure}[!b]
    ...
    \caption{Roadmap (core supervised path; SOM/fuzzy; optimization/evolutionary). Use it when choosing a reading path and locating prerequisites before jumping ahead.}
    \label{fig:roadmap}
\end{figure}
...
\paragraph{Where we head next.} \Cref{chap:symbolic} grounds this vocabulary in a compact case study: symbolic integration as transformation search. The example shows decomposition, safe rewrites, heuristic branching, backtracking, and residual checks in one place; those same capabilities reappear later in data\hyp{}driven settings.
