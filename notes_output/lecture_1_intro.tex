...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        >=Stealth,
        mainnode/.style={draw=gray!80, rounded corners=2pt, line width=1.0pt, minimum width=1.6cm, minimum height=0.75cm, align=center, inner sep=3pt, font=\footnotesize\bfseries},
        subnode/.style={draw=gray!30, rounded corners=2pt, line width=0.6pt, minimum width=1.6cm, minimum height=0.75cm, align=center, inner sep=3pt, font=\footnotesize},
        rowlabel/.style={anchor=east, font=\footnotesize\bfseries, text=gray!90, align=right},
        mainarrow/.style={-{Stealth[length=2.5mm,width=2mm]}, line width=1.2pt, gray!80},
        subarrow/.style={-{Stealth[length=2.0mm,width=1.5mm]}, line width=0.6pt, gray!45}
    ]
        \node[rowlabel] at (0.2, 1.4)  {Optimization};
        \node[rowlabel] at (0.2, 0.0)  {Predictive path};
        \node[rowlabel] at (0.2, -1.4) {Soft computing};

        \node[mainnode, fill=cbBlue!12] (reg) at (1.8, 0)   {Linear\\regression};
        \node[mainnode, fill=cbBlue!12] (log) at (3.9, 0)   {Logistic\\regression};
        \node[mainnode, fill=cbBlue!12] (mlp) at (6.0, 0)   {MLP};
        \node[mainnode, fill=cbBlue!12] (cnn) at (8.1, 0)   {CNN};
        \node[mainnode, fill=cbBlue!12] (rnn) at (10.2, 0)  {RNN};
        \node[mainnode, fill=cbBlue!12, minimum width=1.8cm] (trf) at (12.5, 0) {Transformer};

        \node[subnode, fill=cbPink!15] (opt) at (6.0, 1.4)  {GD /\\ regularization};
        \node[subnode, fill=cbPink!15] (evo) at (9.15, 1.4) {Evolutionary\\search};

        \node[subnode, fill=cbGreen!15]  (som)   at (6.0, -1.4) {SOM};
        \node[subnode, fill=cbOrange!15] (fuzzy) at (8.1, -1.4) {Fuzzy\\inference};

        \draw[mainarrow] (reg.east) -- (log.west);
        \draw[mainarrow] (log.east) -- (mlp.west);
        \draw[mainarrow] (mlp.east) -- (cnn.west);
        \draw[mainarrow] (cnn.east) -- (rnn.west);
        \draw[mainarrow] (rnn.east) -- (trf.west);

        \draw[subarrow] (opt.east) -- (evo.west);
        \draw[subarrow] (mlp.south) -- (som.north);
        \draw[subarrow] (som.east) -- (fuzzy.west);
    \end{tikzpicture}
    \end{adjustbox}
    \captionsetup{justification=raggedright}
    \caption{Roadmap of the book strands: predictive modeling in the middle, optimization above, and soft computing below.}
    \label{fig:roadmap}
\end{figure}
