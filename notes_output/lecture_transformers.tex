...
\Cref{chap:rnn} made the sequence problem explicit: stateful computation plus gradients that must flow across time. \Cref{chap:nlp} then supplied the missing front end for language---turning symbols into vectors with useful geometry. Transformers keep the sequence focus but remove recurrence, replacing it with attention so information can move globally in one layer. The roadmap in \Cref{fig:roadmap} shows this as the modern sequence branch.
