...
\index{transformer}
\index{attention}
\index{attention!self-attention}
\index{attention!cross-attention}
\index{multi-head attention}
\index{positional encoding}
\index{positional encoding!RoPE (rotary position embedding)}
\index{positional encoding!ALiBi}
\index{BERT}
\index{GPT}
\index{LoRA}
\index{KV cache}
...
\index{fine-tuning}
\index{pre-training}
\index{masked language modeling}
\index{scaled dot-product attention}
...
\begin{itemize}
  ...
  \item Distinguish self-attention from cross-attention and describe where each appears, including masking (padding/causal) and positional encodings.
  \item Describe a Transformer block (residual paths, layer norm, FFN) and the common objectives (MLM/CLM) behind the major families (BERT/GPT/encoder--decoder).
\end{itemize}
\end{tcolorbox}
...
\begin{split}
    ...
    \alpha_{t j} &= \operatorname{softmax}_j(e_{t j}),\\
    \mathbf{c}_t &= \sum_{j=1}^{S} \alpha_{t j}\,\mathbf{h}_j.
\end{split}
\label{eq:transformers_context_seq2seq}
\end{equation}
Here \(\operatorname{softmax}_j\) denotes a softmax over the source positions \(j\); explicitly,
\[
\alpha_{t j}=\frac{\exp(e_{t j})}{\sum_{j'=1}^{S}\exp(e_{t j'})}.
\]
This normalization makes the weights nonnegative and ensures they sum to one across \(j\).
...
\citep{Bahdanau2015}.
That is the core move: different output positions can consult different parts of the source. In modern terminology this is \emph{cross-attention}: queries come from the decoder state, while keys and values come from the encoder states.
This is the bridge from encoder--decoder RNNs to Transformers: the attention math stays; what changes is
that Transformers build the states \(\mathbf{h}_j\) and \(\mathbf{s}_t\) without recurrence. This is distinct from decoder self-attention, where queries, keys, and values all come from the decoder sequence itself.
...
I like to read attention as the engineering version of relevance: a weighted retrieval operator, not a
mystical reasoning module. A query asks what is needed, keys advertise what each candidate is about, and
values carry the content that gets mixed. Similarity scores between queries and keys become nonnegative
weights that sum to one; the output is the weighted sum of the value vectors.
\end{tcolorbox}
...
For this toy calculation we bypass the learned projection matrices and set \(Q=K=V=\mathbf{I}_2\). In a trained Transformer, separate learned linear maps produce \(Q\), \(K\), and \(V\) from the token embeddings. Let \(d_k=d_v=2\). Without masking, the scaled scores are
\[
...
A useful intuition in language is that several relations matter at once. Some languages encode gender;
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        \node[proc, minimum width=2.4cm] (ffn_real) at ([yshift=-0.6cm]ln2_real.south) {FFN};
...
\Cref{fig:lec13_micro_figures} provides a quick visual summary of positional encoding, key/value (KV) cache reuse, and Low-Rank Adaptation (LoRA): a parameter-efficient tuning trick where the base weights stay fixed and you learn a small low-rank update.
...
\begin{figure}[t]
    ...
    \begin{scope}[xshift=0cm]
        ...
        \begin{axis}[
            ...
            \addplot[enc, cbOrange, samples=200, domain=0:2*pi] {cos(deg(x))};
        \end{axis}
    ...
    \caption{Transformer micro-views. Left: positional encodings (sinusoidal/rotary) add order information. Center: KV cache stores past keys/values so decoding a new token reuses prior context. Right: LoRA freezes a base weight matrix \(\mathbf{W}\) and trains a low-rank update \(\Delta\mathbf{W}=\mathbf{B}\mathbf{A}\) for parameter-efficient tuning.}
...
In implementation, masks are usually applied additively to attention logits before softmax:
\begin{equation}
\mathbf{S}_{\text{mask}}
...
M_{ij} =
\begin{cases}
0, & \text{if key } j \text{ is visible to query } i,\\
-\infty, & \text{if key } j \text{ is masked.}
\end{cases}
\label{eq:transformers_additive_mask_logits}
\end{equation}
Then \(\mathbf{A}=\operatorname{softmax}(\mathbf{S}_{\text{mask}})\) row-wise and \(\mathbf{Z}=\mathbf{A}\mathbf{V}\), so masked positions contribute zero probability.
...
This connects directly to KV caching in decoder inference: with one-token-at-a-time decoding, the cache only stores past keys/values, so there are no future columns to attend to; during training (full sequence in parallel), the explicit causal mask is still required.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \end{groupplot}
    \end{tikzpicture}
    \caption{Attention masks as heatmaps (queries on rows, keys on columns). Left: padding mask zeroes out attention to padded \emph{key columns} of a shorter sequence in a packed batch; padded-query rows are typically ignored downstream by output/loss masking. Right: causal mask enforces autoregressive flow by blocking attention to future tokens.}
...
Transformer blocks are built from the same few ingredients repeated many times: attention, a small feedforward network, and
...
The feedforward sublayer (FFN) is applied independently to each position (row). In matrix form it is typically two linear
...
\begin{tcolorbox}[summarybox, breakable, title={Implementation sketch: block pseudocode, training defaults, and one step}]
...
\end{tcolorbox}
\paragraph{KV cache in plain terms (decoder inference).}
At generation step \(t\), the decoder computes one new query vector \(\mathbf{q}_t\). The old keys/values from steps \(1,\dots,t-1\) are already stored in memory, so you do \emph{not} recompute them. You only compute \(\mathbf{k}_t,\mathbf{v}_t\), append them to the cache, and attend over the full cached set:
\begin{equation}
\mathbf{z}_t
= \operatorname{softmax}\!\left(\frac{\mathbf{q}_t \mathbf{K}_{1:t}^{\top}}{\sqrt{d_k}}\right)\mathbf{V}_{1:t},
\label{eq:transformers_kv_cache_decode_step}
\end{equation}
where \(\mathbf{K}_{1:t}\) and \(\mathbf{V}_{1:t}\) are the matrices formed by stacking all cached keys/values up to step \(t\). The engineering tradeoff is simple: bigger context means bigger cache. So context size is not only a modeling choice; it is also a memory/latency budget choice at inference time.

\begin{tcolorbox}[summarybox, title={Tokenization note from a GPT-2-style implementation}]
The nanochat reference implementation reinforces four practical points:
\begin{itemize}
    \item GPT-style tokenizers are often \emph{byte-level BPE}: this avoids unknown-token failures on odd characters and mixed text.
    \item A begin-of-sequence delimiter is treated as a real token (often named \texttt{<|bos|>} or historically \texttt{<|endoftext|>}).
    \item The same tokenizer must be used at train and inference time; changing it changes token IDs and breaks model behavior.
    \item Tokenization affects effective context usage: if text splits into fewer tokens, the same context window can carry more content.
\end{itemize}
For readers who are not programming-focused: think of tokenization as the model's ``measurement scale.'' If you change the scale, all learned weights are calibrated to the wrong units. For a compact, end-to-end study repo (tokenizer through serving), nanochat is a good example to browse. \citep{KarpathyNanochat}
\end{tcolorbox}
\paragraph{Code--math dictionary.} In code blocks, \texttt{x} is the token-index tensor (input IDs), \texttt{h} is the token-representation array \(\mathbf{H}\), \texttt{mask} is the attention mask, and \texttt{embed(x)} denotes an embedding lookup into the learned table \(\mathbf{W}\) (written algebraically as a row-selection, e.g., \(\mathbf{W}[w_t]\), in \Cref{chap:nlp}).
...
\begin{sloppypar}
GPT-style models are decoder-only Transformers: the cross-attention block is removed, and each layer keeps masked self-attention plus an FFN. The causal mask enforces the autoregressive factorization:
\end{sloppypar}
\begin{equation}
    \begin{aligned}
        p_\theta(w_{1:n}) &= \prod_{t=1}^{n-1} p_\theta\!\left(w_{t+1}\mid w_{1:t}\right),\\
        \mathcal{L}_{\text{CLM}} &= -\sum_{t=1}^{n-1} \log p_\theta\!\left(w_{t+1}\mid w_{1:t}\right).
    \end{aligned}
    \label{eq:transformers_clm_factorization}
\end{equation}
The training objective (causal language modeling, CLM) is the corresponding negative log-likelihood in \eqref{eq:transformers_clm_factorization}.
This is where it is easy to mix up training and generation. During training you do \emph{not} feed back the model's sampled token at each step; you feed the true prefix (teacher forcing) and compute the loss for all positions in parallel under the causal mask (as in the code block earlier). During inference you do sample/argmax and append one token at a time, and that is where decoding settings (temperature, top-\(k\), top-\(p\)) matter; see \Cref{sec:transformers_decoding_and_evaluation}.
...
At this point the core mechanics are in place. The rest of the chapter shifts from \emph{what a Transformer layer does} to three practical questions readers usually care about next. First, how do we keep attention usable when context gets long and the quadratic cost becomes painful? Second, once pre-training has produced a capable base model, how do we adapt it to a task or domain without breaking the useful structure it already learned? Third, when the model emits a probability distribution rather than a sentence, how do decoding and post-training decisions change what a user actually sees? Keeping these stages separate helps: architecture, pre-training, adaptation, and deployment are connected, but they are not the same engineering decision.
...
Long-context tricks keep the base architecture affordable; fine-tuning answers a different question. By the time we fine-tune, the model already knows how to represent tokens, mix context, and predict the next symbol. What changes now is the \emph{behavior we want from that machinery}. Sometimes we want a domain shift (medical reports, legal text, telemetry logs). Sometimes we want a task shift (classification, summarization, retrieval support, instruction following). The practical concern is always the same: move the model enough to become useful on the new task, but not so much that we erase useful general behavior or make the run impossible to reproduce.
...
\emph{Parameter-efficient fine-tuning (PEFT)} methods (LoRA, adapters, prefix/prompt tuning, and related variants) inject small trainable modules while freezing most of the base model, enabling rapid adaptation with lower memory and more predictable changes. In LoRA, you freeze a weight matrix \(\mathbf{W}\) and learn a low-rank update \(\Delta \mathbf{W}=\mathbf{B}\mathbf{A}\) (rank \(r\)), so only a small number of parameters move during tuning. These methods are attractive when you want many task-specific variants of a shared base model, or when you need to keep the base weights fixed for deployment and auditing.
...
A trained language model does not produce a sentence directly; it produces logits over the next-token vocabulary. Decoding is the separate procedure that converts those scores into an actual sequence. This distinction matters because an observed failure may arise either from the learned distribution itself or from the inference rule used to select tokens. Training determines which continuations receive probability mass; decoding determines how that distribution is turned into behavior at generation time.
...
and the cross-entropy (CE) loss trains the model to put mass on the observed next token.

\begin{tcolorbox}[summarybox, title={Where the proper-scoring story reappears}]
Attention weights and output probabilities are different objects. The row-wise softmax in \(\mathbf{A}=\operatorname{softmax}(\mathbf{Q}\mathbf{K}^\top/\sqrt{d_k})\) creates \emph{internal routing weights} over keys; it is part of the representation mechanism, not the final predictive distribution scored by log loss or the Brier score from \Cref{chap:supervised}. The predictive probabilities appear only after the language-model head maps \(\mathbf{h}_t\) to vocabulary logits \(\boldsymbol{\ell}_t\) and a vocabulary softmax converts those logits into \(p_\theta(x_{t+1}\mid x_{1:t})\). In that sense, sequence modeling keeps the same contract as earlier classification chapters: the architecture builds a representation, but the proper scoring rule is applied to the model's output distribution.
\end{tcolorbox}
...
Pre-training, fine-tuning, decoding, and alignment operate at different stages of the system. Pre-training learns broad statistical structure; fine-tuning specializes that structure to a task or domain; decoding converts token probabilities into sequences at inference time; alignment modifies post-training behavior relative to human preferences or safety constraints. Keeping these stages distinct helps when diagnosing failures, because the source of a problem may lie in the data, the task definition, the retrieval setup, the decoding policy, or the evaluation protocol rather than in alignment alone.
...
\subsection{Advanced notes (optional)}
\label{sec:transformers_advanced_attention_and_efficiency_notes_snapshot}
The following notes are intended as orientation to ideas that now appear frequently in modern model descriptions. The goal is not to add a new prerequisite list, but to identify the problem each idea addresses and the trade-off it introduces.

\paragraph{Relative/rotary positions.}
RoPE \citep{Su2021RoPE} changes how position enters the attention score. Instead of adding a fixed positional vector to the token representation, it rotates each query/key coordinate pair by a position-dependent angle. Concretely, for position \(m\) the \(d\)-dimensional query is partitioned into \(d/2\) pairs \((q_{2i}, q_{2i+1})\), and each pair is rotated by angle \(m\theta_i\) where \(\theta_i = 10000^{-2i/d}\). The important consequence is simpler than the formula: the dot product between a query at position \(m\) and a key at position \(n\) depends on the \emph{relative} offset \(m-n\). That is why RoPE often extrapolates better to longer contexts. ALiBi \citep{Press2022ALiBi} takes an even lighter approach: it subtracts a linear distance penalty directly from the attention logits. In practice, think of absolute tables, RoPE, and ALiBi as three ways to tell attention where tokens sit; the main engineering question is whether you value fixed anchors or better long-context extrapolation.

\paragraph{KV-cache management.}
Decoder-only inference stores per-layer key/value tensors so the model does not recompute the whole prefix at every step. This makes generation feasible, but it turns context length into a memory-budget question. Chunked caching, paged attention, and sliding windows are engineering responses to that constraint. Speculative decoding and assisted decoding push the same idea further: let a cheaper draft model propose tokens, then let the full model verify them before committing.

\paragraph{Efficient kernels and recurrent alternatives.}
FlashAttention \citep{Dao2022FlashAttention} keeps the same exact attention computation but changes the implementation so blocks of the attention map are streamed through fast memory (SRAM) rather than materialized wastefully. Long-context variants then push further by mixing windowed attention, recurrent memory, or low-rank approximations. State-space models such as Mamba \citep{Gu2023Mamba} attack the same long-context problem from a different angle: instead of saying ``make quadratic attention cheaper,'' they ask whether a different sequence operator can preserve long-range structure with linear-time updates.

\paragraph{Mixture-of-experts and routing.}
Sparse MoE layers \citep{Shazeer2017MoE} increase model capacity by activating only a subset of expert blocks per token. The appealing story is ``more capacity without paying full dense cost.'' The engineering warning is that routing is now part of the optimization problem. Router z-losses, capacity factors, and load-balancing losses are not decoration; they are what prevent the system from collapsing onto a few overused experts.

\paragraph{Test-time scaling.}
Not every performance gain comes from retraining. Decoding choices such as nucleus sampling, temperature schedules, beam-style search variants, and guided or assisted generation change the accuracy/latency frontier at inference time. This is why decoding settings belong in experiment logs. If two runs use different sampling rules, they are not the same system from an evaluation standpoint.
...
\begin{table}[h]
\centering
\caption{RNNs versus Transformers: engineering trade-offs. Use it as a design quick reference when deciding whether sequential inductive bias or parallel global context matters more for the task at hand.}
...
\begin{tcolorbox}[summarybox, breakable, title={Key takeaways}]
...
\clearpage
\begin{tcolorbox}[summarybox, breakable, title={Exercises}]
\textbf{Exercise 1 (Easy).} Hand-compute one masked self-attention step for a 3-token prefix. Write the causal mask, compute the attention weights, and compute the output vectors. \\
\textbf{Deliverable:} the mask matrix + the attention weights (as numbers) + a short note explaining exactly which future tokens were blocked and why.

\smallskip
\textbf{Exercise 2 (Medium).} Build a tiny decoder-only model for next-token prediction on a small synthetic corpus (or a tiny character corpus) and run a controlled masking experiment: train once with a correct causal mask and once with an incorrect mask that leaks future tokens. \\
\textbf{Deliverable:} the training curves + one concrete ``cheating'' example that the leaky model can solve but the correctly masked model cannot.

\smallskip
\textbf{Exercise 3 (Medium).} Verify the representation claim: without positional information, a self-attention block is permutation-equivariant. Test this by permuting token order and checking that outputs permute the same way; then add positional encodings and show the equivariance breaks in the expected way. \\
\textbf{Deliverable:} one small numerical sanity check (before/after positions) + one paragraph linking the observation to why positional design matters.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={If you are skipping ahead}]
After this chapter, the book pivots away from neural sequence models, so this is the last stop for masking discipline and evaluation hygiene. If you need the embedding objectives and the bias/deployment checklist that feed into this chapter, see \Cref{chap:nlp}.
\end{tcolorbox}

\medskip
\paragraph{Where we head next.}
Up to here, most updates were differentiable: loss \(\rightarrow\) gradients \(\rightarrow\) parameters. \Cref{chap:softcomp} pivots to a different design point under the same engineering loop: \emph{explicit rules and graded concepts} (fuzzy systems), where the representation is linguistic membership and the update mechanism is rule composition and operator choices. \Cref{chap:evo} then closes the loop with \emph{budgeted black\hyp{}box search} (evolutionary optimization) for cases where gradients are unavailable, objectives are rugged, or constraints dominate.
