...
BERT \citep{Devlin2019} is a stack of Transformer \emph{encoders}. There is no causal mask: each token can attend to both left and right context, so the representation at position \(i\) can use the whole sentence.
...
Word2Vec taught us a useful lesson: meaning leaks into geometry through co-occurrence. If you repeatedly see certain words together, a vector model can pick that up and cluster them.
...
BERT-style models typically prepend a special \texttt{[CLS]} token and (for sentence pairs) separate segments with \texttt{[SEP]}. Because \texttt{[CLS]} participates in the same self-attention as every other token, its hidden state is repeatedly updated as the model mixes information across the sequence. In practice, the final-layer \texttt{[CLS]} vector is treated as a learned, task-adaptable summary and is fed to a small classifier head for sentence-level tasks.
...
\paragraph{From hidden states to a next-token distribution.}
At position \(t\), the final block produces a hidden state \(\mathbf{h}_t\in\mathbb{R}^{d_{\text{model}}}\). A language model head is a linear map into the vocabulary \(\mathcal{V}\):
\begin{equation}
