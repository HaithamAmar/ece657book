...
\Cref{chap:rnn} made the sequence problem explicit: stateful computation plus gradients that must flow across time. Transformers keep the sequence focus but remove recurrence, replacing it with attention so information can move globally in one layer. The roadmap in \Cref{fig:roadmap} shows this as the modern sequence branch.
...
It is helpful to view attention as each token asking ``who else helps me understand my role,'' with masks and layer norms enforcing the rules of that dialogue. The scaled dot-product equations that follow merely quantify that per-token focus allocation.
\end{tcolorbox}

\subsection{Why transformers after RNNs?}
\label{sec:transformers_why_transformers_after_rnns}
\Cref{chap:rnn} closed with recurrent models that process tokens sequentially, leaving them prone to vanishing/exploding gradients, limited receptive fields, and low hardware utilization. Transformers replace the recurrence with attention so every position can condition on any other in a single layer, enabling parallel hardware use and more direct long-range interactions \citep{Vaswani2017}.
...
\medskip
\noindent\textbf{If you are skipping ahead.} When reading \Cref{chap:nlp}, keep the encoder/decoder distinction and the causality mask in mind. Many ``mysterious'' metric failures reduce to masking, tokenization, or evaluation protocol.
\end{tcolorbox}

\medskip
\paragraph{Where we head next.} \Cref{chap:nlp} applies these architectures to language modeling and embeddings (tokenizers, weight tying, perplexity), and covers downstream metrics (BLEU/ROUGE/AUPRC) and bias/calibration checks. Keep the encoder/decoder distinctions in mind as we map them to concrete NLP pipelines; \Cref{chap:rnn} contrasts BPTT and sequential latency, and \Cref{chap:cnn} notes that ViTs \citep{Dosovitskiy2021} inherit convolutional translation equivariance only after patchification plus position encodings.
