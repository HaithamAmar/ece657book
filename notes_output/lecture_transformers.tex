...
\begin{tcolorbox}[summarybox, title={Tokenization note from a GPT-2-style implementation (why students should care)}]
...
\begin{itemize}
    \item \textbf{Relative/rotary positions.} RoPE \citep{Su2021RoPE} encodes position by rotating query and key vectors rather than adding a fixed embedding to the token representation. Concretely, for a position \(m\) the \(d\)-dimensional query is partitioned into \(d/2\) pairs of dimensions; each pair \((q_{2i}, q_{2i+1})\) is rotated by angle \(m\theta_i\) where \(\theta_i = 10000^{-2i/d}\). Because the dot product of a rotated query at position \(m\) with a rotated key at position \(n\) depends only on the \emph{relative} displacement \(m - n\) (by the rotation subtraction identity), the model can attend to relative distance without explicitly computing it. This property allows trained RoPE models to extrapolate better to sequence lengths beyond those seen during training, which is why nearly all modern large-language-model families (LLaMA, Mistral, Qwen) adopt RoPE as their default positional scheme. ALiBi \citep{Press2022ALiBi} takes a simpler approach: it subtracts a linear bias proportional to token distance from attention logits, requiring no change to embeddings; the trade-off is that absolute tables keep fixed anchors useful for classification tokens while rotary/relative schemes favour extrapolation and smoothly sliding context windows.
