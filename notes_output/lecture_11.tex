% Chapter 19
\section{Evolutionary Computing: Search, Selection, and Adaptation}\label{chap:evo}
\index{evolutionary computing}
\index{genetic algorithm (GA)}
\index{selection (GA)}
\index{crossover (GA)}
\index{mutation (GA)}
\index{fitness function}
\index{particle swarm optimisation (PSO)}
\index{NEAT (NeuroEvolution of Augmenting Topologies)}
\index{neuro-evolution}
\index{differential evolution (DE)}
\index{multi-objective optimisation}
\index{NSGA-II}
\index{genetic programming (GP)}
...
Fuzzy inference systems are a behavioral modeling tool: we write down the reasoning logic directly so a human can audit why a particular decision was made. Evolutionary computing aims at a different target. It encodes \emph{candidate designs} and uses a simplified selection--variation loop---an algorithmic echo of natural evolution---to search for designs that perform well under a fixed evaluation budget (the Karl Sims\hyp{}style intuition: emergence via selection and variation, not hand-coded reasoning; see \citealp{Sims1994}). The point is not to be faithful to evolutionary biology; the point is that this process can produce designs that respond well to situations under constraints and noise even when gradients are unavailable and the knobs are mixed discrete/continuous.
...
\paragraph{Connection to the Ch.\,1 case study.}
The autonomous-vehicle example sketched in \Cref{chap:intro} highlights a familiar outer-loop problem: some design choices are continuous, some are discrete, and the only reliable score comes from closed-loop evaluation under realistic constraints. Evolutionary search is a practical tool in exactly that regime, where threshold policies, architecture choices, and trade-offs must be tuned under a fixed evaluation budget.
...
\begin{tcolorbox}[summarybox, title={Author's note: why GAs remain relevant in modern practice}]
Genetic algorithms remain relevant because many high-impact optimization loops are still black-box, noisy, and mixed discrete/continuous. Hyperparameter and pipeline tuning are canonical examples: one evaluation may be expensive, stochastic, and constraint-laden, while gradients are unavailable or misleading at the system level. In that setting, population search is not a historical artifact; it is an engineering tool for budgeted improvement.
\par\smallskip
This also explains the bridge from Part III. Fuzzy systems expose structured, interpretable policy components (membership shapes and rules), and evolutionary search provides a practical mechanism for tuning those components when manual adjustment does not scale.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Application note: budgeted search in practice}]
\textbf{Robot gait / locomotion.} Suppose we want a legged robot to move forward robustly across slightly different terrains. The design has mixed knobs: continuous parameters (gains, trajectory coefficients, penalty weights) and discrete choices (controller mode, safety logic, constraint-handling policy). A single trial is expensive and noisy, so it is natural to treat performance as a black-box fitness, average over multiple seeds, and search under a strict evaluation budget.\par
\textbf{Hyperparameter and pipeline tuning (including LLM systems).} Many modern learning pipelines have the same structure: discrete design choices (model family, system configuration variant, data filters) plus continuous knobs (learning rates, thresholds, temperatures), measured by an empirical score with variance across runs. Evolutionary search is attractive when gradients are unavailable, the evaluation surface is rugged, or the objective invites silent failure modes. Read this chapter as the non\hyp{}differentiable sibling of ERM: same objective\hyp{}and\hyp{}audit discipline, different update mechanism.\par
...
\begin{tcolorbox}[summarybox, title={Application example: GA tuning of a previously built fuzzy controller}]
Revisit the Mamdani-style fuzzy controller from the fuzzy trilogy (\Crefrange{chap:softcomp}{chap:fuzzyinference}). Inputs such as tracking error \(e_t=r_t-y_t\) and change-of-error \(\Delta e_t\) are fuzzified, rules fire, consequents are aggregated, and a centroid defuzzifier produces a crisp action \(u_t\).
\par\smallskip
A GA enters only as a tuning layer \emph{around} that fixed pipeline. A chromosome encodes quantities already introduced in Part~III: membership-function parameters (e.g., breakpoints of triangular/trapezoidal sets), optional rule weights, and (if you allow it) a discrete choice of operator defaults from a small admissible set (t\hyp{}norm, s\hyp{}norm, implication, defuzzifier).
\par\smallskip
If you want the membership functions to reflect graded human judgments (not only closed-loop performance), you can tune those same parameters against expert-provided pairs \(\{(x_i,\hat{\mu}_i)\}\) while enforcing ordering and overlap constraints. The GA does not change what a membership function \emph{means}; it only gives you a disciplined search procedure for fitting its shape under constraints.
\par\smallskip
Each chromosome is evaluated by running the closed loop and scoring a fitness/objective such as
\[
J = w_1\,\mathrm{MAE}(e) + w_2\,\mathrm{Overshoot} + w_3\,\mathrm{ControlEffort}
    + \lambda\,\mathrm{Penalty}_{\text{constraints}} ,
\]
where the penalty term covers hard limits (actuator saturation, safety constraints, monotonicity constraints on memberships). This is the same fuzzy controller you can audit by inspection, with a different answer to the tuning question: instead of hand tweaking, you search under a fixed evaluation budget.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Application example: GA over neural networks (outer-loop design search)}]
Neural networks are trained by gradients, but many \emph{design} decisions around them are discrete, constrained, and evaluation\hyp{}limited. Consider designing a CNN (\Cref{chap:cnn}), an RNN (\Cref{chap:rnn}), or a Transformer (\Cref{chap:transformers}) for a fixed task and dataset. The inner loop is familiar: train by backpropagation and score generalization on a validation split. The outer loop is the hard part: choose architecture and training knobs under a compute budget.
\par\smallskip
A GA makes that outer loop explicit. Encode a candidate design as a chromosome \(\mathbf{g}\) containing discrete choices (depth, width, kernel sizes, pooling schedule, sequence length/window, optimizer family) and continuous knobs (learning rate, weight decay, dropout rate). Define an objective that matches the book's audit discipline:
\[
J(\mathbf{g}) = \widehat{\mathcal{L}}_{\text{val}}(\mathbf{g})
  + \lambda\,\mathrm{Penalty}_{\text{latency/memory}}(\mathbf{g}),
\]
where \(\widehat{\mathcal{L}}_{\text{val}}(\mathbf{g})\) is measured after a fixed training budget (epochs, steps, or wall time) using the same monitoring habits from earlier chapters (learning curves, calibration, slice checks).
\par\smallskip
The point is not to replace backpropagation. The point is that evolutionary search can sit \emph{above} backpropagation when the knobs are mixed and the evaluation surface is noisy: it proposes designs, you train them, you score them, and selection plus variation keeps improving what you try next under a fixed budget.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Application example: jet-nozzle shape optimization (experiment in the loop)}]
In a wind-tunnel setting, the ``model'' is a physical experiment and the design variables are geometric. Let \(\mathbf{x}\) parameterize a nozzle profile (for example, a small set of spline control points subject to thickness and smoothness constraints). A natural objective is ``maximize thrust'' (or efficiency) while penalizing constraint violations:
\[
\min_{\mathbf{x}\in\mathcal{D}} \; J(\mathbf{x}) = -T(\mathbf{x}) + \rho\,\mathrm{Penalty}_{\text{manufacturing/safety}}(\mathbf{x}).
\]
There is no gradient through the tunnel. Each evaluation consumes time and budget, and measurements are noisy. Evolutionary search fits this regime: maintain a population of candidate shapes, test them, keep the better ones, perturb/recombine, and iterate until the improvement stalls or the trial budget is exhausted.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={When GA is not the first choice}]
If the objective is smooth, gradients are trustworthy, and constraints are easy to encode, gradient-based or convex optimization methods are usually faster and easier to tune. When the evaluation budget is tiny (tens to hundreds of trials), Bayesian optimization can be a strong alternative \citep{Frazier2018}. Use GA when those assumptions fail: discontinuities, mixed-variable encodings, simulator-in-the-loop objectives, or hard constraints that make local gradients uninformative.
\end{tcolorbox}
...
Imagine a function \( J \) with multiple peaks and valleys (local maxima and minima). The global minimum is the lowest valley, but many local minima exist that can trap naive optimization methods. A local method (think: ``follow the slope downhill'') commits to one trajectory: once it falls into a basin, it has no built-in mechanism to ``jump'' to a different valley unless you restart it or inject noise.

Evolutionary computing approaches the same picture by keeping \emph{many} candidates alive at once. A population gives you coverage: some candidates sit near promising regions, others explore alternatives. Selection then biases the next round toward candidates that look better under \(J\), while variation operators propose new points that can move across basins.

\paragraph{Goal:} Instead of guaranteeing the global optimum, evolutionary computing aims to find a \emph{good enough} solution---one that is sufficiently close to optimal and found within a reasonable computational budget. This goal is practical: when one objective evaluation is expensive (simulation, hardware, or human checks), the right question is often ``what is the best design I can get before I run out of budget?''
...
In realistic workflows, one evaluation may involve simulation runtime, hardware experiments, or human-in-the-loop checks. Under those costs, exhaustive enumeration is not just slow but operationally infeasible, which is why evolutionary methods are framed as budgeted search.

\subsection{Summary}
\label{sec:evo_summary}

Checkpoint: you now have the \emph{why} for evolutionary search---rugged landscapes, constraints, discontinuities, and budgets that make local improvement unreliable. The rest of this chapter develops one canonical workflow in three passes:
...
Evolutionary computing treats optimization as budgeted population search: evaluate candidates, bias reproduction toward better ones, generate variation (crossover/mutation), replace the population, and repeat until budget or convergence criteria trigger. Genetic algorithms are the canonical instance of this loop.

The core tradeoff is always the same: exploration for coverage versus exploitation for immediate gains. In engineering terms, you want enough selection pressure to make progress, but not so much that the population collapses around one mediocre basin too early. This is why GA work is rarely just ``run the loop'': you also watch traces (best/mean fitness) and at least one diversity proxy, and you adjust operators when the behavior stops matching your intent.

\subsection{Biological Inspiration: Evolutionary Concepts}
\label{sec:evo_biological_inspiration_evolutionary_concepts}

This section sets terminology and motivation. If you already think of a GA as ``selection + variation + replacement under a fitness budget,'' use it as a notation pass; the algorithmic details begin in \Cref{sec:evo_genetic_algorithms_modeling_chromosomes}.
...
In GA terms, this maps to representation: one chromosome encodes one candidate solution, and each gene encodes one design decision (binary switch, integer slot, real-valued parameter, or subtree element).
...
\end{sloppypar}
This intuition is carried directly into algorithm design: crossover recombines useful partial structures from different parents, while mutation injects fresh variation so search does not collapse prematurely around one basin.
...
In GA terms, the biology analogy reduces to an engineering cycle: keep a population (coverage), apply fitness-based selection (pressure), recombine parents (structure transfer), mutate offspring (novelty), and replace survivors (iteration). Each operator controls a specific failure mode: too much pressure collapses diversity, too little pressure stalls progress, and weak mutation traps the search in local basins.

Two clarifications keep the analogy honest. First, ``genes'' are not physical objects in a GA; they are coordinates in an \emph{encoding}. That means operator design must respect the representation: swapping substrings is natural for binary strings, but it can break permutation encodings unless you use permutation-safe crossover/mutation. Second, the point of stochasticity is not randomness for its own sake; it is controlled variability that lets the algorithm explore complex, multimodal landscapes when deterministic methods struggle or when gradients are not trustworthy.
...
\begin{table}[h]
\centering
\caption{Biological analogies and their genetic-algorithm counterparts. Use this only as an intuition bridge; the formal algorithmic objects remain encodings, operators, and selection rules.}
...
\label{eq:ga_chromosome_representation}
\end{equation}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        >=Stealth,
        card/.style={draw=gray!30, rounded corners=3pt, fill=white, line width=0.6pt},
        cardtitle/.style={font=\footnotesize\bfseries, text=gray!80},
        flow/.style={-{Stealth[length=2.2mm,width=1.6mm]}, line width=0.9pt, gray!70},
        box/.style={draw=gray!40, rounded corners=2pt, fill=white, minimum width=1.8cm, minimum height=0.55cm, font=\footnotesize, align=center},
        tiny/.style={font=\scriptsize, text=gray!70}
    ]
        % --- Selection Card ---
        \begin{scope}[shift={(0,0)}]
            \node[card, minimum width=4.2cm, minimum height=3.0cm] at (0,0) {};
            \node[cardtitle] at (0, 1.2) {Selection};

            \node[font=\footnotesize, anchor=east] at (-1.1, 0.45) {fit};
            \fill[cbBlue!50] (-0.9, 0.3) rectangle (1.0, 0.6);
            \draw[gray!40, line width=0.5pt] (-0.9, 0.3) rectangle (1.3, 0.6);
            \node[tiny, anchor=west] at (1.4, 0.45) {0.60};

            \node[font=\footnotesize, anchor=east] at (-1.1, -0.05) {mid};
            \fill[cbBlue!30] (-0.9, -0.2) rectangle (0.4, 0.1);
            \draw[gray!40, line width=0.5pt] (-0.9, -0.2) rectangle (1.3, 0.1);
            \node[tiny, anchor=west] at (1.4, -0.05) {0.42};

            \node[font=\footnotesize, anchor=east] at (-1.1, -0.55) {low};
            \fill[cbBlue!10] (-0.9, -0.7) rectangle (-0.1, -0.4);
            \draw[gray!40, line width=0.5pt] (-0.9, -0.7) rectangle (1.3, -0.4);
            \node[tiny, anchor=west] at (1.4, -0.55) {0.18};
        \end{scope}

        % --- Crossover Card ---
        \begin{scope}[shift={(5.40,0)}]
            \node[card, minimum width=5.0cm, minimum height=3.0cm] at (0,0) {};
            \node[cardtitle] at (0, 1.2) {Crossover};

            \node[font=\footnotesize, anchor=east] at (-0.6, 0.5) {parent A};
            \fill[cbBlue!40] (-0.4, 0.35) rectangle (2.0, 0.65);
            \draw[gray!40, line width=0.5pt] (-0.4, 0.35) rectangle (2.0, 0.65);

            \node[font=\footnotesize, anchor=east] at (-0.6, 0) {parent B};
            \fill[cbOrange!30] (-0.4, -0.15) rectangle (2.0, 0.15);
            \draw[gray!40, line width=0.5pt] (-0.4, -0.15) rectangle (2.0, 0.15);

            \draw[dashed, gray!80, line width=0.8pt] (0.8, 0.85) -- (0.8, -0.25);
            \node[tiny, fill=white, inner sep=1pt] at (0.8, 0.85) {cut};

            \draw[flow] (0.5, -0.25) -- (0.5, -0.5);
            \draw[flow] (1.1, -0.25) -- (1.1, -0.5);

            \node[font=\footnotesize, anchor=east] at (-0.6, -0.65) {child 1};
            \fill[cbBlue!40] (-0.4, -0.8) rectangle (0.8, -0.5);
            \fill[cbOrange!30] (0.8, -0.8) rectangle (2.0, -0.5);
            \draw[gray!40, line width=0.5pt] (-0.4, -0.8) rectangle (2.0, -0.5);
            \draw[gray!40, line width=0.5pt] (0.8, -0.8) -- (0.8, -0.5);

            \node[font=\footnotesize, anchor=east] at (-0.6, -1.15) {child 2};
            \fill[cbOrange!30] (-0.4, -1.3) rectangle (0.8, -1.0);
            \fill[cbBlue!40] (0.8, -1.3) rectangle (2.0, -1.0);
            \draw[gray!40, line width=0.5pt] (-0.4, -1.3) rectangle (2.0, -1.0);
            \draw[gray!40, line width=0.5pt] (0.8, -1.3) -- (0.8, -1.0);
        \end{scope}

        % --- Constraints Card ---
        \begin{scope}[shift={(2.7, -3.7)}]
            \node[card, minimum width=7.6cm, minimum height=3.0cm] at (0,0) {};
            \node[cardtitle] at (0, 1.2) {Constraints};

            \node[box, minimum width=1.8cm] (cand) at (-2.4, 0.3) {candidate};
            \node[draw=gray!50, diamond, aspect=1.8, fill=white, minimum width=1.5cm, minimum height=0.7cm, font=\footnotesize, align=center, inner sep=1pt] (feas) at (0, 0.3) {feasible?};
            \node[box, fill=cbGreen!15, minimum width=1.8cm] (eval) at (2.4, 0.3) {evaluate};

            \node[box, fill=cbOrange!15, minimum width=1.8cm] (repair) at (0, -0.8) {repair /\\ penalty};

            \draw[flow] (cand.east) -- (feas.west);
            \draw[flow] (feas.east) -- (eval.west) node[midway, above, tiny, yshift=1pt] {yes};
            \draw[flow] (feas.south) -- (repair.north) node[midway, right, tiny, xshift=1pt] {no};
            \draw[flow] (repair.east) -| (eval.south);
        \end{scope}
    \end{tikzpicture}
    \end{adjustbox}
    \caption{Evolutionary micro-operators. Selection increases the sampling rate of fitter candidates, crossover recombines parent segments, and constraint handling routes infeasible candidates through repair or penalty before evaluation.}
...
(p_2)_i, & \text{if } u_i = 1.
\end{cases}
\label{eq:ga_gene_selection_mask}
\end{equation}
...
\begin{sloppypar}
For minimization problems, selection still expects ``higher is better,'' so map the cost to a monotone fitness score, for example \(F(x)=-f(x)\) or \(F(x)=1/(1+f(x))\). Then add penalties or feasibility rules for constraint violations.
\end{sloppypar}

\subsection{Population Initialization and Size}
\label{sec:evo_population_initialization_and_size}

Once encoding is decided, the initial population is generated by sampling genotypes within the search space. Purely random initialization is the default because it makes minimal assumptions and gives broad coverage early. If you have domain knowledge, you can also seed a fraction of the population with plausible candidates (and keep the remainder random) so the search starts with both coverage and a few ``good guesses.''

For constrained problems, initialization is also where you decide how you will treat feasibility: sample only feasible candidates (possibly with a repair step), or allow infeasible ones but attach penalties/feasibility rules during fitness evaluation. Either way, the initialization policy quietly shapes what the algorithm can discover in its first few generations.
...
\noindent A useful engineering view is to think in terms of evaluation budget. If you can afford \(B\) objective evaluations total, then (up to bookkeeping overhead) you are choosing how to split that budget between population size \(M\) and number of generations \(G\): roughly \(B \approx M \times G\). Large \(M\) buys diversity per generation; large \(G\) buys more refinement steps. The right balance depends on evaluation cost and on how quickly selection pressure collapses diversity for your encoding.
...
After initialization, genetic operators are applied to evolve the population. In practice, these operators
are the \emph{control knobs} that decide how aggressively the search exploits what looks good versus how
much it explores new regions of the design space.

\paragraph{Where operators sit in the GA loop.}
At a high level, each generation repeats the same pipeline:
\begin{enumerate}
    \item Evaluate fitness (and apply penalties/feasibility rules if constrained).
    \item Select parents (controls \emph{selection pressure}).
    \item Generate variation via crossover and mutation (controls \emph{mixing} and \emph{novelty}).
    \item Form the next population (often with elitism or replacement rules).
\end{enumerate}

The next subsections unpack these pieces: selection in \Cref{sec:evo_selection_in_genetic_algorithms}, crossover in \Cref{sec:evo_crossover_operator}, and mutation in \Cref{sec:evo_mutation_operator}.
...
Selection answers the question: \emph{who gets to reproduce?} It is the main mechanism that biases sampling
toward better candidates, and it therefore determines the algorithm's \emph{selection pressure}. Too much
pressure collapses diversity and risks premature convergence; too little reduces the GA to near-random
search. In this section we formalize how fitness values translate into parent choice (roulette-style
probabilities), then compare alternatives (ranking and tournament selection) that are often more stable when
raw fitness values are poorly scaled. The goal is to connect the big-picture GA loop to implementable parent
selection rules you can audit and tune.
...
\begin{enumerate}
    ...
    \end{aligned}
    \label{eq:ga_one_point_crossover}
    \end{equation}
\end{enumerate}
...
\begin{tikzpicture}[
  ...
  line width=1.0pt,
  block/.style={
    draw=gray!48, rounded corners=3pt,
    minimum width=41mm, minimum height=13mm,
    align=center, fill=cbBlue!8
  },
  operator/.style={block, fill=cbOrange!11},
  terminal/.style={block, fill=cbGreen!14, minimum width=28mm},
  decision/.style={
    draw=gray!48, diamond, aspect=2.0,
    inner sep=1.5pt, align=center,
    minimum width=26mm, minimum height=16mm,
    fill=cbPink!10
  },
  flow/.style={->, gray!72},
  loop/.style={->, dashed, gray!68},
  lab/.style={font=\footnotesize, fill=white, inner sep=1pt},
  stage/.style={font=\scriptsize\bfseries, text=gray!78}
]
...
\node[terminal, right=18mm of term] (stop) {Stop};

\node[operator, below=18mm of fit] (sel) {Selection};
\node[operator, right=of sel] (cross) {Crossover};
\node[operator, right=of cross] (mut) {Mutation};
\node[block, below=18mm of cross, fill=cbGreen!8] (rep) {Replacement};

\node[stage] at ($(fit.north)+(0,9mm)$) {Evaluation};
\node[stage] at ($(cross.south)+(0,-10mm)$) {Variation loop};
\node[stage] at ($(stop.north)+(0,8mm)$) {Exit};

% --- Main flow ---
\draw[flow] (init) -- (fit);
\draw[flow] (fit) -- (term);
\draw[flow] (term) -- node[above, lab]{yes} (stop);
...
\draw[<->, dashed, gray!60] ([yshift=4mm]sel.east) -- node[above, lab, yshift=2pt]{variation operators} ([yshift=4mm]mut.west);

\end{tikzpicture}
\end{adjustbox}
\caption{GA control flow. Initialization leads to fitness evaluation and a termination check. If the stop criterion is not met, the algorithm cycles through selection, crossover, mutation, and replacement before evaluating the next generation. Use this figure to verify that your implementation preserves the intended loop structure.}
...
\end{itemize}
\paragraph{Reproducibility and fair comparison} Fix random seeds when debugging, run many seeds (e.g., 20+) for reporting, match evaluation budgets across algorithms, and report mean/median best-so-far with variability bands. Log all hyperparameters and share code/configs to make comparisons fair, following \Cref{app:repro_standards} as the default reporting protocol.
...
\label{sec:evo_particle_swarm_optimisation}

Particle Swarm Optimisation is a population-based metaheuristic inspired by collective behaviour in bird flocks and fish schools. Each candidate solution (``particle'') moves through the search space by blending its own best-known position with the swarm's best-known position.

\paragraph{Update equations.}
Each particle \(i\) has a position \(\mathbf{x}_i\) and a velocity \(\mathbf{v}_i\). At each iteration:
\begin{align*}
  \mathbf{v}_i^{(t+1)} &= \omega \, \mathbf{v}_i^{(t)}
    + c_1 r_1 \bigl(\mathbf{p}_i - \mathbf{x}_i^{(t)}\bigr)
    + c_2 r_2 \bigl(\mathbf{g} - \mathbf{x}_i^{(t)}\bigr), \\
  \mathbf{x}_i^{(t+1)} &= \mathbf{x}_i^{(t)} + \mathbf{v}_i^{(t+1)},
\end{align*}
where \(\mathbf{p}_i\) is particle \(i\)'s personal best position, \(\mathbf{g}\) is the global best position across the entire swarm, \(\omega\) is the \emph{inertia weight} (controls exploration vs.\ exploitation), \(c_1\) and \(c_2\) are the \emph{cognitive} and \emph{social} coefficients, and \(r_1, r_2 \sim \mathcal{U}(0,1)\) are fresh random draws each iteration.

\paragraph{Hyperparameter guidance.}
Common defaults are \(\omega \approx 0.7\), \(c_1 \approx c_2 \approx 1.5\), and a swarm size around 20--50 particles. A linearly decreasing inertia schedule (\(\omega\) from roughly 0.9 to 0.4) shifts the search from broad exploration early to local refinement late. For constrained problems, apply Deb's feasibility rules or velocity clamping to keep particles within bounds.

\paragraph{Comparison with GAs and DE.}
PSO has no crossover or mutation operators; all information sharing happens through the velocity update. This makes PSO very easy to implement and well-suited to continuous, unconstrained (or lightly constrained) problems. GAs and DE generally outperform PSO when the search space has many discrete components or when diversity preservation is critical. PSO can converge prematurely on multimodal landscapes; restart heuristics or topology variants (ring, von Neumann) mitigate this.

\paragraph{Pointer: neuro-evolution and NEAT.}
Both GAs and PSO can optimize neural network weights and topologies simultaneously---an approach called \emph{neuro-evolution}. The landmark algorithm in this space is NEAT (NeuroEvolution of Augmenting Topologies), which grows network structure incrementally using historical markings to align crossover across networks with different topologies. NEAT demonstrated that topology-and-weight evolution could solve problems that fixed-topology neuro-evolution could not, and it has experienced a resurgence in reinforcement learning settings where the loss landscape is non-differentiable or highly non-stationary. Modern descendants (HyperNEAT, POET, Open-Ended Learning) extend the idea to indirect encodings and competitive co-evolution.
...
\[
\begin{aligned}
\text{Parent 1:}\quad & f_1 = (x_1 \times x_3) + (x_1 + x_4), \\
\text{Parent 2:}\quad & f_2 = (x_2 - 5) \times (x_4 + 1).
\end{aligned}
\]
...
\caption[Sample Pareto front for two objectives]{Sample Pareto front for two objectives. NSGA-II keeps all non-dominated points (blue) while pushing dominated solutions (orange) toward the front via selection, yielding a spread of trade-offs in one run. The point is to interpret multi-objective results as a set of trade-offs rather than a single optimum. \textit{Source: author-generated.}}
...
\medskip
\noindent\textbf{Ethical considerations.} Evolutionary search will exploit whatever you write down as fitness. If the objective is a proxy for the real goal, the optimizer can ``win'' in ways you did not intend (metric gaming, unsafe corner solutions, hidden constraint violations). Treat objective design and constraint auditing as part of system safety, not as afterthoughts.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Exercises}]
\textbf{Exercise 1 (Easy).} Turn the chapter's one-generation trace into a reproducible unit test: implement the exact selection/crossover/mutation steps used in \Cref{tab:ga-toy} and verify that your code reproduces the same intermediate population (parents chosen, offspring produced, and replacement). \\
\textbf{Deliverable:} a step-by-step table matching \Cref{tab:ga-toy} + one sentence explaining what would make such a trace \emph{non-reproducible}.

\smallskip
\textbf{Exercise 2 (Medium).} Study selection pressure vs.\ diversity on a multi-modal 1D objective (any bounded interval): run tournament selection with several tournament sizes (or roulette vs.\ tournament) under a fixed evaluation budget. Track both best fitness and a diversity proxy (e.g., genotype variance or number of unique individuals). \\
\textbf{Deliverable:} best-fitness curves (median over seeds) + one diversity plot + a short conclusion about when the GA ``wins'' by collapsing diversity too early.

\smallskip
\textbf{Exercise 3 (Medium).} Compare constraint handling strategies under the same fitness-call budget: penalty vs.\ feasibility-first selection (or repair). Use a constrained objective where the unconstrained optimum is infeasible so the difference is visible. \\
\textbf{Deliverable:} a plot of feasibility rate over generations + the best feasible fitness over time + one paragraph explaining what your constraint strategy is implicitly prioritizing.

\medskip
\noindent\textbf{Fast-track note.} This chapter is largely self-contained. If you revisit earlier model chapters, keep the common thread explicit: each method still needs a clear objective, a diagnostic that exposes failure, and a reporting protocol that survives replication.
\end{tcolorbox}
