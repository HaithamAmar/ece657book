...
The roadmap in \Cref{chap:intro} keeps four strands in view: statistical/data-driven modeling, biological/neural representation learning, behavioral rule-based fuzzy systems, and evolutionary search. This chapter develops the evolutionary thread. See \Cref{fig:roadmap} for the strand placement.
...
\begin{itemize}
    ...
    \item \textbf{Biological inspiration (neural networks):} The neural strand (\Crefrange{chap:mlp}{chap:nlp}) abstracts neurons and representation learning to build powerful function approximators.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \end{groupplot}
    \end{tikzpicture}
    \caption{Schematic: Evolutionary micro-operators. Left: fitter individuals get sampled more often (roulette/tournament). Middle: crossover splices parents by a mask (one-point shown). Right: constraint handling routes offspring through repair/penalty/feasibility before evaluation.}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Schematic: Illustrative GA run showing the best and mean normalized fitness over 50 generations. Flat regions motivate ``no improvement'' stopping rules, while steady separation between best and mean indicates ongoing selection pressure.}
...
\end{tikzpicture}
\end{adjustbox}
\caption{Schematic: GA flowchart showing the iterative process: initialization leads to fitness evaluation and a termination check. If not terminated, the algorithm proceeds through selection, crossover, mutation, and replacement, which then feeds the next generation's fitness evaluation.}
...
\caption{Schematic: Toy GA generation on a bounded interval. One crossover and mutation illustrate how the fitness function guides selection before the next generation.}
\label{tab:ga-toy}
\end{table}
...
\item \textbf{Deb's rules (1998/2000):} (i) if one solution is feasible and the other is not, pick the feasible one; (ii) if both are feasible, pick the better objective; (iii) if both are infeasible, pick the one with smaller total constraint violation. Adaptive penalties can be layered on top when violations persist.
\end{itemize}
\paragraph{Reproducibility and fair comparison} Fix random seeds when debugging, run many seeds (e.g., 20+) for reporting, match evaluation budgets across algorithms, and report mean/median best-so-far with variability bands. Log all hyperparameters and share code/configs to make comparisons fair.
...
\begin{figure}[t]
    ...
    \caption{Schematic: Sample Pareto front for two objectives. NSGA-II keeps all non-dominated points (blue) while pushing dominated solutions (orange) toward the front via selection, yielding a spread of trade-offs in one run.}
...
\paragraph{Where we head next.} This chapter closes the soft-computing thread. For background refreshers, \Cref{app:linear_systems} summarizes linear-systems prerequisites and \Cref{app:kernels} collects kernel-method context used earlier in the book.
