...
f = \{80, 75, 60, 65, 40, 20\}.
\]
Higher-fitness chromosomes are more likely to survive and reproduce. A common proportional rule assigns
\[
P_s(\mathbf{c}) \approx \frac{f(\mathbf{c})}{\sum_{\mathbf{c}'} f(\mathbf{c}')},
\]
so that fitter candidates appear more frequently in the parent pool over generations. The detailed selection mechanics (roulette, ranking, elitism) and a numeric roulette example follow in \Cref{sec:evo_selection_in_genetic_algorithms}.
...
The GA loop alternates fitness evaluation with selection, crossover, mutation, and replacement until a stopping criterion is met. Use \Cref{fig:lec11-ga-flow} to verify control flow, \Cref{tab:ga-toy} for a numeric trace of one generation, and \Cref{sec:evo_pseudocode_representation} for a direct implementation template; all three describe the same loop at different levels of detail.
