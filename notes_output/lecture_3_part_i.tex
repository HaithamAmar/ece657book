...
\textbf{What stays the same (the contract).} We still follow the ERM loop: choose a representation, define a
loss, optimize, then verify with honest evaluation and diagnostics.
...
choices are loosely inspired by how we think biological systems process information (as sketched in the next
section), but the target remains an engineering model, not a faithful brain simulation.

\medskip
\textbf{How the next chapters build.}
\begin{itemize}
