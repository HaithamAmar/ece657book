...
\index{universal approximation theorem (UAT)}
\index{decision boundary}
\index{Hebb rule}
\index{perceptron convergence theorem}
...
\begin{itemize}
  \item Write the forward computation of a perceptron (\(\mathbf{w}^\top\mathbf{x}+b\)) and interpret its decision boundary geometrically as a hyperplane.
  \item Implement (or trace) the perceptron and Adaline update rules and explain what triggers an update.
  \item Explain, with XOR as the motivating counterexample, why a single linear separator is not enough and why later chapters introduce smooth activations and gradient-based training.
\end{itemize}
\end{tcolorbox}
...
\textbf{What stays the same.} We still follow the empirical risk minimization (ERM) loop: choose a representation, define a loss, optimize, then verify with honest evaluation and diagnostics. As we proceed, you will find that this theme generalizes across many of the algorithms we discuss.
...
\paragraph{Shapes and convention.} We use the row-major (deep-learning) convention from the start. A single example is a row vector \(\mathbf{a}^{(l)}\in\mathbb{R}^{1\times n_l}\), a mini\hyp{}batch stacks examples by rows \(\mathbf{A}^{(l)}\in\mathbb{R}^{B\times n_l}\), and weights map features by right multiplication \(\mathbf{W}^{(l)}\in\mathbb{R}^{n_{l-1}\times n_l}\). Biases \(\mathbf{b}^{(l)}\in\mathbb{R}^{n_l}\) broadcast across the batch: \(\mathbf{Z}^{(l)}=\mathbf{A}^{(l-1)}\mathbf{W}^{(l)}+\mathbf{1}_B(\mathbf{b}^{(l)})^\top\), where \(\mathbf{1}_B\in\mathbb{R}^{B\times 1}\) is an all-ones column vector. For a concrete check, if \(B=4\), \(n_{l-1}=3\), and \(n_l=5\), then \(\mathbf{A}^{(l-1)}\in\mathbb{R}^{4\times 3}\), \(\mathbf{W}^{(l)}\in\mathbb{R}^{3\times 5}\), and \(\mathbf{Z}^{(l)}\in\mathbb{R}^{4\times 5}\). We reserve \(\phi(\cdot)\) for kernel feature maps (\Cref{app:kernels}).

With that convention fixed, if we denote the input vector as \(\mathbf{x}\), the output of layer \(l\) as \(\mathbf{a}^{(l)}\), and the weight matrix connecting layer \(l-1\) to layer \(l\) as \(\mathbf{W}^{(l)}\), the feedforward operation is:
\begin{equation}
\begin{aligned}
...
\begin{itemize}
    ...
    \[
        ...
        \end{cases}
    \]
        Allows for inhibitory (negative) outputs, mimicking excitatory and inhibitory neuron behavior. We adopt the convention \(f(0)=0\); some authors either leave \(\mathrm{sgn}(0)\) undefined or set it to \(+1\), so it is helpful to state the choice explicitly.
...
\subsection{The McCulloch--Pitts Neuron: Logical Computation and Limits}
...
\begin{itemize}
    \item \textbf{AND:} For inputs \(x_1, x_2\), set \(w_1 = w_2 = 1\) and \(\theta = 2\). The unit outputs 1 only when both inputs are 1, matching the AND truth table.
    \item \textbf{OR:} Keep \(w_1 = w_2 = 1\) but set \(\theta = 1\). The unit outputs 1 when at least one input is 1, matching OR.
\end{itemize}
...
\begin{equation}
    ...
    \label{eq:perceptron}
\end{equation}
where \(\mathbf{x}\) is the input vector, \(\mathbf{w}\) the weight vector, and \(b\) the bias (threshold). Here \(\mathbf{w}^\top\mathbf{x}\) is simply the scalar inner product; under the book's row\hyp{}major convention you can read it as \(\mathbf{x}\mathbf{w}\) as well.
...
Let \(\mathbf{w}^{(t)}\) denote the perceptron weights after \(t\) mistakes, with \(\mathbf{w}^{(0)}=\mathbf{0}\). Each mistake updates:
\[
...
\begin{tcolorbox}[summarybox, title={Universal Approximation Theorem (UAT)}]
\textbf{Statement} \citep{Cybenko1989,Hornik1991}: A feedforward network with a single hidden layer containing a \emph{sufficient} (finite) number of units with a non-constant, bounded, and monotone-increasing continuous activation function can approximate any continuous function \(f: \mathbb{R}^n \to \mathbb{R}\) on a compact set \(K \subset \mathbb{R}^n\) to any desired accuracy \(\epsilon > 0\).

Formally, for any \(\epsilon > 0\) there exist weights and biases such that
\[
  \sup_{\mathbf{x} \in K} \left| \hat{f}(\mathbf{x}) - f(\mathbf{x}) \right| < \epsilon.
\]
\textbf{What it does \emph{not} say.} The theorem is purely existential: it guarantees that an approximating network \emph{exists} but says nothing about how wide the hidden layer must be, how to find the weights, or how well gradient-based training will locate the solution. In practice, depth (multiple layers) often requires far fewer total parameters than the width needed for a single-layer approximation. The perceptron limitations you just saw are a reminder: expressivity alone does not imply trainability.
\end{tcolorbox}
...
\begin{tcolorbox}[summarybox, breakable, title={Perceptron learning loop (pseudocode)}]
\footnotesize
\begin{verbatim}
function TrainPerceptron(X, y_pm1, eta, epochs):
    # X: N x d features, y in {-1,+1}
    w <- 0_d
    b <- 0
    for epoch in 1..epochs:
        for i in 1..N:
            s <- dot(w, X[i]) + b
            yhat <- +1 if s >= 0 else -1
            if yhat != y_pm1[i]:
                w <- w + eta * y_pm1[i] * X[i]
                b <- b + eta * y_pm1[i]
    return w, b
\end{verbatim}
\normalsize
The logic is intentionally simple: compute a signed score, threshold it, and update only on mistakes. Logistic regression keeps the same linear score but replaces the hard threshold with a probabilistic link and a smooth objective (\Cref{chap:logistic}); multilayer networks replace the single separator with learned intermediate features (\Crefrange{chap:mlp}{chap:backprop}).
\end{tcolorbox}
...
\textbf{Exercise 1 (Easy).} Train a perceptron on a 2D linearly separable dataset. \\
\textbf{Deliverable:} a plot of the data + the learned separating line, and a short note stating the label encoding you used.

\smallskip
\textbf{Exercise 2 (Easy).} Repeat Exercise 1 with Adaline (squared error + gradient descent). \\
\textbf{Deliverable:} one learning curve and a comparison of convergence behavior versus the perceptron.

\smallskip
\textbf{Exercise 3 (Medium).} Build the XOR dataset and show, empirically, the failure mode of a single hyperplane. \\
\textbf{Deliverable:} a plot + a one-paragraph explanation of why this forces the move to multi-layer models in \Cref{chap:mlp}.
\par\medskip
