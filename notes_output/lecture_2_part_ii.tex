...
\index{cross-entropy}
\index{negative log-likelihood (NLL)}
\index{softmax}
\Cref{chap:symbolic} illustrated intelligence as transformation search with explicit goal tests. \Cref{chap:supervised} introduced the data\hyp{}driven view: represent a task with data, choose a hypothesis class, minimize empirical risk under regularization, and audit performance on held\hyp{}out data. This chapter extends that toolkit from regression to classification, and \Cref{fig:roadmap} places it on the core supervised path.
...
\begin{tcolorbox}[summarybox, title={Worked example: a toy decision boundary}]
On a two\hyp{}moons binary classification toy, the ideas in this chapter have a visible effect:
\begin{itemize}
    \item Logistic regression gives a calibrated probabilistic baseline (linear score + sigmoid) that is easy to diagnose.
    \item The nonlinearity we will need later is \emph{not} in the optimizer but in the representation: \Cref{chap:mlp} introduces multilayer features and \Cref{chap:backprop} supplies the training engine that lets those features improve the decision boundary.
\end{itemize}
\end{tcolorbox}
...
With that setup in place, we now formalize the classification target. Unlike regression, where \(y\) is continuous, classification predicts a \emph{discrete label}. We start with the binary case \(y\in\{0,1\}\), where the goal is to estimate a probability
\[
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
    ...
    \caption{Bayes-optimal boundary for two Gaussian classes. Equal covariances and similar priors (LDA setting) yield a linear separator; unequal covariances yield a quadratic boundary. The boundary is near the equal-posterior line (vertical, pink); left/right regions map to predicted classes R0 and R1.}
    \label{fig:lec4_bayes}
\end{figure}


\Cref{fig:lec4_mle_map} provides the low-data MAP-versus-MLE comparison used in this discussion.
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
    ...
    \end{tikzpicture}
        % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
        \caption{MAP estimates interpolate between the prior mean and the data-driven MLE. As the sample size grows, the MAP curve approaches the true mean; with little data, the prior dominates.}
        \label{fig:lec4_mle_map}
    \end{figure}
...
\textbf{Objective:} Minimize binary cross\hyp{}entropy (negative log-likelihood) between true labels \(y\in\{0,1\}\) and predicted probabilities \(\pi(\mathbf{x})=\sigma(\boldsymbol{\beta}^\top \tilde{\mathbf{x}})\).\\
...
Binary labels and \(\pi(\mathbf{x})\) were defined in \Cref{sec:logistic_classification_problem_statement}; here we focus on modeling the log-odds with a linear predictor.
...
This implies that the posterior probability \(\pi(\mathbf{x})\) is the \emph{logistic sigmoid} applied to the linear predictor.

\begin{tcolorbox}[summarybox, title={Author's note: why ``logistic'' and why ``regression''?}]
\emph{Logistic} is the link function: a sigmoid takes a real score and turns it into a probability in \([0,1]\). And \emph{regression} is what remains linear: the \emph{log\hyp{}odds} (the logit) is modeled as a linear function of the features.
...
\begin{equation}
    p(\mathbf{y}\mid \mathbf{X},\boldsymbol{\beta})=\prod_{i=1}^N p_i^{y_i}(1-p_i)^{1-y_i},
...
\end{equation}
Maximizing the log\hyp{}likelihood in \eqref{eq:lec2_loglik} is equivalent to minimizing the negative log\hyp{}likelihood (binary cross\hyp{}entropy). With the design matrix \(\mathbf{X}=[\tilde{\mathbf{x}}_1^\top;\ldots;\tilde{\mathbf{x}}_N^\top]\) and vector \(\mathbf{p}=(p_1,\ldots, p_N)^\top\), the gradient of the negative log\hyp{}likelihood is
\begin{equation}
    \nabla_{\boldsymbol{\beta}}\Big(-\log p(\mathbf{y}\mid \mathbf{X},\boldsymbol{\beta})\Big)
    =\mathbf{X}^\top(\mathbf{p}-\mathbf{y}).
    \label{eq:lec2_logistic_grad}
\end{equation}
The Hessian has the form \(\mathbf{X}^\top \mathbf{W} \mathbf{X}\) where \(\mathbf{W}=\mathrm{diag}(p_i(1-p_i))\succeq 0\), which makes the objective convex and explains why second-order methods work well for moderate feature dimensions.
...
\end{tikzpicture}
            % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
            \caption{The sigmoid maps logits to probabilities (left). The binary cross\hyp{}entropy (negative log\hyp{}likelihood) penalizes confident wrong predictions sharply (middle). Regularization typically shrinks parameter norms as the penalty strength increases (right).}
    \label{fig:lec2_sigmoid_bce}
\end{figure}


\paragraph{Optimization geometry (why iterative solvers)}
Unlike linear regression, logistic regression does not have a closed-form solution for \(\boldsymbol{\beta}\), even though the objective is convex. In practice we therefore rely on iterative solvers (gradient methods, quasi\hyp{}Newton methods, or Newton/iteratively reweighted least squares (IRLS) in moderate dimensions). \Cref{fig:lec4_gd} is a convex quadratic toy that reminds us what an optimization trajectory looks like when we minimize a smooth objective: step size and conditioning shape how quickly iterates contract toward the minimizer.
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
        ...
        \ensuretikzbackgroundlayers
    \end{tikzpicture}
        \caption{Gradient-descent iterates contracting toward the minimizer of a convex quadratic cost (schematic). Ellipses are level sets; arrows show the ``steepest descent along contours'' direction. Poor conditioning stretches the ellipses and slows convergence.}
        \label{fig:lec4_gd}
    \end{figure}
...
\begin{figure}[h]
    ...
    \fi
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption[Illustrative logistic-regression boundary]{Illustrative logistic-regression boundary. The dashed line marks the linear decision boundary at probability 0.5; labeled contours show how the posterior varies smoothly with margin, enabling calibrated decisions and adjustable thresholds. \textit{Source: author-generated.}}
...
\begin{itemize}
    \item \textbf{MLE} maximizes the data likelihood under a chosen observation model (for logistic regression: Bernoulli with \(p_i=\sigma(\boldsymbol{\beta}^\top\tilde{\mathbf{x}}_i)\)) \citep{Fisher1922}.
    \item \textbf{MAP} maximizes the posterior, which multiplies the likelihood by a prior \(p(\boldsymbol{\beta})\). In optimization form, MAP adds a penalty \(-\log p(\boldsymbol{\beta})\), which is exactly regularization \citep{Bishop1995}.
\end{itemize}
Two common priors explain the two penalties that appear most often in practice: a zero-mean Gaussian prior yields an L2 (ridge) penalty, while a Laplace prior yields an L1 (lasso) penalty. The L2 penalty also has an older life as a stabilizer for ill-posed inverse problems (Tikhonov regularization) \citep{Tikhonov1963}. The schematic below illustrates the MLE\(\rightarrow\)MAP idea on a simple mean-estimation problem: with little data, the prior matters; with enough data, MAP approaches MLE.
...
\noindent\textbf{Abbreviation note.} ROC = receiver operating characteristic; PR = precision--recall; AUC = area under the curve; ECE = expected calibration error. We use AUROC/AUPRC for the corresponding AUCs.

\begin{tcolorbox}[summarybox, title={Risk \& audit}]
\begin{itemize}
    \item \textbf{Threshold choice:} a high AUC can still yield a poor operating point; pick thresholds using a validation objective that matches the cost.
    \item \textbf{Class imbalance:} report PR curves (or per-class metrics) when positives are rare; audit confusion matrices, not just accuracy.
    \item \textbf{Probability quality:} logits can be miscalibrated; use reliability diagrams/ECE when probabilities are consumed downstream.
    \item \textbf{Feature shortcuts:} strong apparent accuracy can come from spurious correlates; sanity-check with slices and perturbations.
    \item \textbf{Regularization:} tune \(\lambda\) with held-out data; do not report the best test result after repeated tuning.
\end{itemize}
\end{tcolorbox}

Once we have a probabilistic classifier, we need diagnostics that quantify performance on held-out data. For multi-class prediction, the confusion matrix \(C_{ij}\) records the number of examples with true class \(i\) predicted as \(j\). From \(C\) we compute accuracy, per-class precision/recall, and aggregate metrics. \emph{Macro-averaged} precision/recall first evaluate the metric per class and then average them uniformly, whereas \emph{micro-averaged} precision/recall pool all true/false positives across classes before computing the ratio (equivalent to weighting each example equally). Visual inspection (\Cref{fig:lec4_confusion}) helps diagnose systematic errors across classes.
...
\begin{figure}[!htbp]
    ...
    \fi
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption[ROC and PR curves with an explicit operating point]{ROC and PR curves with an explicit operating point. Left: ROC curve with iso-cost lines; right: PR curve with a class-prevalence baseline and iso-F1 contours. Together they visualize threshold trade-offs and calibration quality. \textit{Source: author-generated.}}
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
        ...
        \ensuretikzbackgroundlayers
    \end{tikzpicture}
        \caption{Confusion matrix for a three-class classifier; diagonals dominate, indicating strong accuracy with modest confusion between classes B and C.}
        \label{fig:lec4_confusion}
    \end{figure}
...
\begin{itemize}
    \item Confusing scores with decisions: choose thresholds using costs and PR/ROC trade-offs, not a default 0.5.
    \item Reporting only AUC: check calibration when probabilities are used downstream (reliability/ECE).
    \item Letting imbalance hide failure: stratify splits and audit per-class performance, not only overall accuracy.
\end{itemize}
\end{tcolorbox}
...
\begin{table}[tbp]
\centering
\caption{Handling class imbalance for logistic models (\Cref{chap:logistic} reference table). A compact menu of resampling, weighting, and thresholding strategies for rare positives.}
...
\textbf{Exercise 1 (Easy).} Implement logistic regression from scratch (no library solver): forward logits, sigmoid, cross\hyp{}entropy, and gradients. Verify the gradient on a tiny dataset with a finite-difference check. \\
\textbf{Deliverable:} the worst relative gradient error + the exact loss convention you used (sum vs.\ mean over batch).

\smallskip
\textbf{Exercise 2 (Medium).} Build an imbalanced binary dataset and treat thresholding as part of the model. Plot ROC and PR curves, pick a decision threshold that satisfies a stated constraint (e.g., max false-negative rate), and report calibration (reliability diagram or ECE). \\
\textbf{Deliverable:} ROC/PR plots + the chosen threshold and the achieved constraint + one calibration diagnostic.

\smallskip
\textbf{Exercise 3 (Medium).} Run a slice audit: define 2--3 meaningful slices (by a feature range, a subgroup, or a covariate bin), and report how performance and calibration differ across slices. If one slice is worse, propose one concrete remedy (re-weighting, data collection, or a different feature). \\
\textbf{Deliverable:} a small slice table + one paragraph describing the most actionable failure mode you found.
\par\medskip
\noindent\textbf{If you are skipping ahead.} Keep three ideas: (i) Bayes-optimal boundaries are a conceptual anchor, (ii) logistic regression is the first practical probabilistic baseline, and (iii) evaluation depends on costs, thresholds, and calibration. Those recur throughout the neural and deployment chapters and remain important whenever later chapters output scores that must be turned into decisions.
\end{tcolorbox}

\medskip
\paragraph{Where we head next.} Logistic regression is still linear at the boundary. \Cref{chap:perceptron} introduces trainable neuron models, and stacking nonlinear units then removes that linear ceiling and sets up multilayer networks plus backpropagation. Carry this chapter's decision discipline forward: probability quality, threshold choice, and class-imbalance handling remain core checks even when the model family changes.
