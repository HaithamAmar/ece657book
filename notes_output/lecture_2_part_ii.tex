...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
    ...
    \end{tikzpicture}
    \caption[Synthetic binary dataset.]{Synthetic binary dataset built from two anisotropic Gaussian clusters; shaded ellipses hint at the underlying density while the scattered samples are reused throughout the running examples. Use this when building intuition for how decision boundaries relate to class geometry before fitting discriminative models.}
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
        ...
        \ensuretikzbackgroundlayers
    \end{tikzpicture}
        \caption{Gradient-descent iterates contracting toward the minimizer of a convex quadratic cost. Ellipses are level sets; arrows show the ``steepest descent along contours'' direction. Use this when visualizing why conditioning slows gradient descent.}
        \label{fig:lec1_gd}
    \end{figure}
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
    ...
    \end{tikzpicture}
        % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
        \caption{MAP estimates interpolate between the prior mean and the data-driven MLE. As the sample size grows, the MAP curve approaches the true mean. Use this when explaining how priors matter most in low-data regimes.}
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
        ...
        \ensuretikzbackgroundlayers
    \end{tikzpicture}
        \caption{Confusion matrix for a three-class classifier; diagonals dominate, indicating strong accuracy with modest confusion between classes B and C. Use this when diagnosing which error types dominate in multiclass settings.}
...
\begin{table}[h]
\centering
\caption{Handling class imbalance for logistic models (\Cref{chap:logistic} reference table). Use this when choosing resampling, weighting, and thresholding strategies for rare positives.}
