...
Building on the multilayer perceptron (MLP) architecture (\Cref{chap:mlp}) and its training machinery (\Cref{chap:backprop}), we now introduce a different class of feedforward models: \emph{Radial Basis Function Networks} (RBFNs). Figure \Cref{fig:roadmap} places this as the kernel/prototype branch alongside the MLP path.
