...
Building on the multilayer perceptron (MLP) architecture (\Cref{chap:mlp}) and its training machinery (\Cref{chap:backprop}), we now introduce a different class of feedforward models: \emph{Radial Basis Function Networks} (RBFNs). \Cref{fig:roadmap} places this as the kernel/prototype branch alongside the MLP path.
...
Use \Cref{fig:rbf_architecture_weights} as the architecture anchor for this section.
...
\Cref{fig:rbf_architecture_weights} is the architecture reference separating fixed radial features from trained readout weights.
...
Use \Cref{fig:rbf_gaussian_bumps} as the architecture anchor for this section.

\Cref{fig:rbf_centres} is the center-placement reference used when selecting initialization strategy.
...
Use \Cref{fig:rbf_centres} as the architecture anchor for this section.

\Cref{fig:rbf_primal_dual} is the primal-versus-dual reference used when discussing basis count and Nystr\"om trade-offs.
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{How the width parameter (sigma) influences decision boundaries on a 2D toy dataset. Too-large sigma underfits, intermediate sigma captures the boundary, too-small sigma overfits with fragmented regions. Use \Cref{chap:supervised}'s validation curves to pick model size and regularization. Use it when tuning sigma to balance smoothness against boundary fidelity.}
    \label{fig:rbf_sigma_sweep}
\end{figure}

\Cref{fig:rbf_boundary} is the nonlinear decision-boundary reference for the XOR-style toy example.
...
Use \Cref{fig:rbf_primal_dual} as the architecture anchor for this section.
...
Use \Cref{fig:rbf_boundary} as the architecture anchor for this section.
