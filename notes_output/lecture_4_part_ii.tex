% Chapter 8
\section{Radial Basis Function Networks}\label{chap:rbf}
\index{radial basis function network (RBFN)}
\index{kernel trick}
\index{feature map}
\index{least squares}

Building on the multilayer perceptron (MLP) architecture (\Cref{chap:mlp}) and its training machinery (\Cref{chap:backprop}), this chapter introduces radial basis function networks (RBFNs): three\hyp{}layer models with fixed nonlinear bases and a linear readout. This is a controlled contrast with end-to-end backpropagation: we keep a nonlinear representation, but we separate basis design from readout fitting. \Cref{fig:roadmap} places this as the kernel/prototype branch alongside the MLP path.
...
\Cref{chap:supervised} frames this as a bias--variance tuning problem (capacity, regularization, and diagnostics via learning curves). Kernel methods such as kernel ridge regression and support vector machines (SVMs) interpret the same trade-off through an RBF kernel matrix; here we keep the bases explicit, then connect to the dual/kernel view later in the chapter.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[>=stealth, node distance=1.6cm]
        ...
        % notes
        \node[align=left, font=\scriptsize, above=0.2cm of h1] {Centers/widths\\\((\mathbf{c}_i,\sigma_i)\) set by\\k-means or heuristics};
...
\caption{RBFN architecture. Inputs feed radial units parameterized by centers and widths; a linear readout with weights and bias is trained by a regression or classification loss. In the classical pipeline only the output weights are learned, while centers and widths come from clustering or spacing heuristics; variants also fine-tune centers and widths end-to-end.}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Localized Gaussian basis functions (dashed) and their learned output-weighted combination \(\sum_j w_j\varphi_j(x)\) (solid). Overlapping bumps allow RBF networks to interpolate complex signals smoothly.}
...
\begin{itemize}
    ...
    \item Two radial units centered at \(\mathbf{c}_1=(0,0)^\top\) and \(\mathbf{c}_2=(1,1)^\top\).
...
\begin{itemize}
    \item For simplicity, set \(\sigma^2 = 1\) (so \(2\sigma^2 = 2\)) in the Gaussian activation.
    \item Use two centers: \(\mathbf{c}_1 = (0,0)^\top\) and \(\mathbf{c}_2 = (1,1)^\top\).
    \item Use a Gaussian radial basis function (RBF):
    \[
...
\begin{itemize}
    \item \(\mathbf{c}_i\): the center of the \(i\)-th basis function.
...
\]
This function is smooth, bounded, and has a clear interpretation as a localized receptive field (the region of input space that activates the unit strongly) centered at \(\mathbf{c}\) with width \(\sigma\). It is the most commonly used kernel in RBF networks.
...
\]
which serve as localized similarity measures to each center \(c_i\). When \(\sigma\) is large, many points activate the same basis functions with comparable strength, leading to smooth decision boundaries after the linear output layer. When \(\sigma\) is small, only points very close to a center elicit large activations, yielding sharply varying boundaries that can overfit noise. Visualizing \(\varphi_i(x)\) for several centers illustrates how tuning \(\sigma\) controls the flexibility of the classifier.
...
\]
where \(\mathbf{K}_{ij} = k(\mathbf{x}_i,\mathbf{x}_j)\) uses the same Gaussian kernel. Setting \(M=N\) and letting the RBF centers coincide with the training points recovers this dual form exactly. Finite \(M\) acts like a Nystr\"om approximation: \(\boldsymbol{\Phi}\mathbf{w}\) projects onto a subset of kernel features using \(M<N\) landmark bases, rather than expanding on all \(N\) training points.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[scale=0.95]
        ...
        \node[font=\scriptsize] at (0.5,0.4) {$(\boldsymbol{\Phi}^\top\boldsymbol{\Phi}+\lambda I)\mathbf{w}=\boldsymbol{\Phi}^\top \mathbf{y}$};
        \draw[gray!40] (2,0) rectangle (4,1);
        \node[font=\scriptsize] at (3,0.5) {$\mathbf{w}\in\mathbb{R}^{M}$};

        \begin{scope}[shift={(0,-2)}]
            \node at (-3,1.4) {\scriptsize Dual (kernel ridge / SVM)};
            \draw[gray!40] (-4,0) rectangle (-2,1);
        \node[font=\scriptsize] at (-3,0.5) {$\mathbf{K}\in\mathbb{R}^{N\times N}$};
        \node[font=\scriptsize] at (-3,-0.1) {kernel matrix};
        \draw[->, thick] (-1.5,0.5) -- (-0.5,0.5);
        \node[font=\scriptsize] at (0.5,0.8) {solve};
        \node[font=\scriptsize] at (0.5,0.4) {$(\mathbf{K}+\lambda I)\boldsymbol{\alpha}=\mathbf{y}$};
        \draw[gray!40] (2,0) rectangle (4,1);
        \node[font=\scriptsize] at (3,0.5) {$\boldsymbol{\alpha}\in\mathbb{R}^{N}$};
    \end{scope}
...
\begin{enumerate}
    \item Use clustering (e.g., K-means) to find centers \( \mathbf{c}_i \) (or sample centers uniformly at random).
    ...
    \item Evaluate on a validation set and pick \((\sigma,\lambda, M)\) that minimizes validation loss; for classification, cross\hyp{}entropy or hinge losses are also feasible with the same design matrix \(\boldsymbol{\Phi}\).
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Parameter selection:} Choosing the number of neurons \( M \), centers \( \mathbf{c}_i \), and spreads \( \sigma_i \) is nontrivial and often requires heuristics or cross-validation.
...
\textbf{Exercise 1 (Medium).} Train an RBFN on a 2D nonlinear classification task (e.g., two moons). Sweep \(M\), \(\sigma\), and a small \(\lambda\) grid; plot decision boundaries and validation curves. \\
\textbf{Deliverable:} one figure with 3 boundaries (underfit / best / overfit) + the chosen \((M,\sigma,\lambda)\) and a short bias--variance diagnosis.

\smallskip
\textbf{Exercise 2 (Easy).} Treat regularization as numerical hygiene: for one fixed dataset and basis, plot the condition number of the linear system versus \(\lambda\) and show how predictions become stable as \(\lambda\) increases from 0. \\
\textbf{Deliverable:} condition-number vs.\ \(\lambda\) plot + one paragraph explaining why ``mathematically correct'' can still be numerically wrong at \(\lambda=0\).

\smallskip
\textbf{Exercise 3 (Medium).} Demonstrate the primal/dual equivalence: use all data points as centers with \(\lambda>0\) and verify that the primal RBFN predictions match kernel ridge regression with an RBF kernel. \\
\textbf{Deliverable:} a short derivation relating \(\mathbf{w}\) and \(\boldsymbol{\alpha}\) + a numerical check that the two prediction vectors agree to tight tolerance.
\par\medskip
