...
\Cref{fig:rbf_architecture_weights} plus the overview above summarize the three-layer flow; we now formalize it algebraically.
...
\Cref{fig:rbf_boundary} shows one such boundary for a fixed choice of centers, width, and ridge regularization.
    ...
    \begin{figure}[t]
        ...
        \end{tikzpicture}
        % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
        \caption{RBFN decision boundary on XOR with 4 centers, \(\sigma=0.8\), and ridge \(\lambda=10^{-3}\). Shading indicates the predicted class under a 0.5 threshold; the black contour marks the 0.5 boundary. Training points are overlaid (class 0: open circles; class 1: filled squares). See \Cref{fig:rbf_sigma_sweep} for sigma effects.}
