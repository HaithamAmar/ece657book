...
\index{natural language processing (NLP)}
\index{word embedding}
\index{Word2Vec}
\index{negative sampling}
\index{tokenization}
\graphicspath{{assets/lec8/}{assets/lec14/}}

\Cref{chap:rnn} framed language as prediction under context: the model must assign probability to the next token from a compressed state. That framing makes the bottleneck obvious. Tokens are discrete identifiers, while the model is a continuous function; if the input geometry is unhelpful, the network cannot generalize systematically from one string to another.

This chapter supplies that geometry. We build word embeddings and the objectives that train them, then show how to sanity\hyp{}check the resulting space (neighbors, analogies, and simple audits). Those same representation primitives become inputs to both recurrent and attention\hyp{}based models; \Cref{chap:transformers} reuses them when recurrence is replaced by attention.
...
In this chapter, we focus on neural methods for natural language processing (NLP) through the lens of representation learning. The recurring question from \Cref{chap:rnn} is practical: what input space makes sequence prediction stable and data efficient? The answer is to learn vectors whose geometry captures contextual similarity.
...
\paragraph{How this chapter is organized.}
...
\paragraph{Notation.}
We use \(V\) for vocabulary size, \(d\) for embedding dimension, \(\mathbf{x}_t\) for one-hot token indicators, and \(\mathbf{e}_t\) for learned embedding vectors. When symbols collide with earlier chapters (e.g., \(d\) as data dimension vs.\ embedding size), the local meaning is authoritative; \Cref{app:notation_collisions} gives the global index.

\paragraph{Problem statement.}\label{sec:nlp_problem_statement}
...
\subsection{Warm-up: from symbols to vectors}
\label{sec:nlp_warmup_from_symbols_to_vectors}

One-hot vectors make discrete tokens compatible with linear algebra, but they do not express \emph{similarity}: synonyms are orthogonal, and a model that treats each basis element independently has no reason to generalize from ``great'' to ``awesome'' without seeing both during training.

\paragraph{One-hot encoding (baseline).}
Fix a dictionary of size \(V\). A token at position \(t\) can be represented as a one-hot row indicator \(\mathbf{x}_t \in \{0,1\}^{1\times V}\) with a single 1. This guarantees uniqueness, but it makes every pair of distinct words equally far apart.

\paragraph{Why one-hot is not enough (two quick failures).}
\textbf{Synonyms.} A sentiment model trained to treat ``great'' as positive has no immediate reason to treat ``awesome'' as positive if the two words are orthogonal basis vectors.\par
\textbf{Document similarity.} ``I enjoyed talking to the monarchs'' and ``I loved conversing with the royals'' express the same meaning, but a bag-of-words similarity score stays low if it cannot recognize that \emph{monarchs} and \emph{royals} occupy similar roles.

\medskip
\noindent This is the same friction point you saw in \Cref{sec:rnn_sentiment_worked_example}: the sequence model can only be as systematic as its input geometry.

\paragraph{Embedding lookup as learned features.}
Instead of using the identity basis, we learn an embedding table \(\mathbf{W}\in\mathbb{R}^{V\times d}\) whose rows are trainable parameters. A one-hot indicator becomes a row lookup:
\[
\mathbf{e}_t=\mathbf{x}_t\mathbf{W}\in\mathbb{R}^{1\times d}.
\]
This is the same linear algebra as one-hot, but now the coordinates are \emph{learned} so that similar words end up nearby when that helps prediction.

A useful mental model is to imagine that each word is a \emph{graded bundle of attributes}. If we could write those attributes down explicitly, each word would already be a vector:

\begin{table}[t]
\centering
\small
\caption{Feature-based word vectorization example. Each word is mapped to a vector of graded semantic features; fractional entries (e.g., 0.5) indicate mixed usage across contexts. Treat this as an intuition pump: real embedding coordinates are learned from data rather than hand-assigned.}
\label{tab:word_feature_vectorization}
...
\Cref{tab:word_feature_vectorization} gives a concrete intuition for mapping symbolic tokens into graded feature vectors.

\begin{tcolorbox}[summarybox, title={Numeric check: why the analogy arithmetic can work}]
Using the toy feature vectors in \Cref{tab:word_feature_vectorization}, the analogy is literal:
\[
...
Of course, hand-crafting features does not scale to large vocabularies, and it bakes in subjective choices. The central move of modern embeddings is to let the \emph{training objective} discover whatever coordinates best support prediction from context.

\paragraph{Subword tokenization and OOV handling.} Practical systems rarely operate on raw word types alone. To reduce vocabulary size and handle out-of-vocabulary (OOV) forms, they tokenize text into \emph{subword units}. Byte Pair Encoding (BPE) and WordPiece learn a compact inventory of frequent character sequences; words are segmented into a small number of subwords that can be re-composed by the model. FastText instead augments word vectors with character \(n\)-gram embeddings, so the representation of an unseen word is the sum of its subword vectors.
...
The practical takeaway is that repeated context patterns reveal latent word properties. In the \emph{pretty} example, co-occurrence with evaluative adjectives points to an intensifier sense, while co-occurrence with visual nouns points to an aesthetic sense.
...
\subsection{Word2Vec at a glance}
...
\subsection{From lookup to objective: compact derivation path}
\label{sec:nlp_mathematical_formulation_of_cbow}

With the conceptual picture fixed, we now run one continuous derivation path from lookup mechanics to CBOW/skip-gram training objectives. Let vocabulary size be \(V\) and embedding dimension be \(d\). Define the embedding matrix \(\mathbf{W} \in \mathbb{R}^{V \times d}\), where the \(i\)-th row \(\mathbf{v}_i\) is the embedding vector for word \(w_i\). \emph{Convention: we treat embeddings as \textbf{rows}; one-hot words index rows via $\mathbf{x}\mathbf{W}$ (row lookup).}

% Chapter 13 (continued)

\paragraph{Embedding lookup as a tiny network (shapes).}
\label{sec:nlp_neural_network_architecture_for_word_embeddings}
Fix a vocabulary of size \(V\) and an embedding dimension \(d\) (e.g., \(V=10{,}000\), \(d=300\)). A token is an index \(i\in\{1,\ldots,V\}\), represented as a one-hot row vector \(\mathbf{x}\in\{0,1\}^{1\times V}\). The embedding table \(\mathbf{W}\in\mathbb{R}^{V\times d}\) is trainable, and the embedding for the token is the row lookup
\begin{equation}
    \begin{aligned}
        \mathbf{h} &= \mathbf{x}\mathbf{W}\in\mathbb{R}^{1\times d},\\
        \mathbf{z} &= \mathbf{h}\mathbf{W}_{\text{out}} \in \mathbb{R}^{1\times V},\\
        \hat{\mathbf{y}}_j &= \frac{\exp(z_j)}{\sum_{k=1}^V \exp(z_k)}, \quad j=1,\ldots, V,\\
...
To turn the lookup into a prediction task (CBOW or skip-gram), we add output-side vectors \(\mathbf{W}_{\text{out}}\in\mathbb{R}^{d\times V}\), compute logits \(\mathbf{z}\), apply a softmax to get a vocabulary distribution \(\hat{\mathbf{y}}\), and then minimize cross\hyp{}entropy (equivalently, negative log-likelihood) against the one-hot target \(\mathbf{y}\).
During learning, \(\mathbf{W}\) and \(\mathbf{W}_{\text{out}}\) update so that words that share contexts end up with similar vectors.

\paragraph{Context window and sequential input.}
\label{sec:nlp_context_window_and_sequential_input}

Reusing the same toy phrase, let the context window size be 4 around the target token. To predict ``automatic'' in ``to buy an automatic car,'' the context words are
\[
...
\paragraph{Interpretation of the weight matrix \texorpdfstring{\(\mathbf{W}\)}{W}.}
...
\subsection{Word2Vec objectives in detail}
...
\subsubsection{CBOW objective (detailed form)}
\label{sec:nlp_continuous_bag_of_words_cbow_sub_2}

In the detailed CBOW form, the model predicts center token \(w_t\) from its window context. For a sequence \( w_1, w_2, \ldots, w_T \) and half-window size \( c \), the context is \( \{ w_{t-c}, \ldots, w_{t-1}, w_{t+1}, \ldots, w_{t+c} \} \).
...
\paragraph{Key Insight:} Multiplying one-hot input \( \mathbf{x}_{t+j} \) by \(\mathbf{W}\) performs a row lookup. The embedding matrix therefore behaves as a trainable feature table.

\subsubsection{skip-gram objective (detailed form)}
...
\paragraph{Interpretation:} Skip-gram pushes words with similar neighborhoods toward similar embeddings.
...
Hyperparameters: window \(c=2\)--\(5\) (often dynamic/symmetric), embedding dim \(d=100\)--\(300\), negatives \(k=5\)--\(20\), unigram noise \(P_n(w)\propto f(w)^{0.75}\), learning rate on the order of \(10^{-3}\)--\(10^{-2}\).\\
...
\begin{figure}[t]
    ...
    \caption[Analogy geometry in embedding space]{Analogy geometry in embedding space. The offset ``v(king) - v(man) + v(woman) approx v(queen)'' forms a parallelogram; a similar gender direction shifts ``doctor'' toward ``nurse.'' Solid and dashed vectors highlight the shared relational direction. Points are shown after a 2D principal component analysis (PCA) projection, so directions are approximate. \textit{Source: author-generated schematic; inspired by \citet{Mikolov2013}.}}
...
\begin{equation}
    \log X \approx \mathbf{U}_r \boldsymbol{\Sigma}_r \mathbf{V}_r^\top,
    \label{eq:svd}
\end{equation}
where \(\mathbf{U}_r \in \mathbb{R}^{V \times r}\) and \(\mathbf{V}_r \in \mathbb{R}^{V \times r}\) contain the top-\(r\) singular vectors (for the desired embedding dimension \(d=r\)), and \(\boldsymbol{\Sigma}_r \in \mathbb{R}^{r \times r}\) is a diagonal matrix of the corresponding singular values. The truncation rank \(r\), often between 100 and 300 in practice, acts exactly like the embedding dimensionality knob in neural models.

By setting
\[
\mathbf{v}_i = (\mathbf{U}_r)_i \boldsymbol{\Sigma}_r^{1/2}, \quad \mathbf{u}_k = (\mathbf{V}_r)_k \boldsymbol{\Sigma}_r^{1/2},
\]
...
\begin{itemize}
    \item \textbf{Cultural bias:} Text corpora often contain stereotypes or skewed representations of gender, ethnicity, and other social categories (e.g., news archives that associate ``nurse'' more frequently with women than men).
    \item \textbf{Historical bias:} Older texts may reflect outdated or prejudiced views. Digitized literature from the 19th century, for instance, over-represents colonial perspectives.
    \item \textbf{Language-specific bias:} Different languages and dialects encode different cultural norms and connotations, such as grammatical gender or honorifics that privilege particular groups.
\end{itemize}
...
There is no single fix, because the ``bias direction'' you measure depends on the corpus, the task, and the group definitions you care about. In practice, mitigations usually fall into three buckets: post-processing (identify a direction and project it out; e.g., Hard Debiasing by \citet{Bolukbasi2016}), data-side interventions (rebalance or augment so the corpus does not teach the same stereotype as strongly), and objective-side constraints (regularize training so certain associations are discouraged). The key is to treat mitigation as iterative: apply a change, then rerun the same probes and downstream slices you used to detect the issue.
...
\begin{itemize}
    ...
    \item \textbf{Calibration and reliability:} Evaluate calibration (expected calibration error (ECE), reliability diagrams) overall and for key subgroups; severely miscalibrated models magnify harm when used for decision support.
    \item \textbf{Disaggregated evaluation:} Report accuracy, receiver operating characteristic / precision--recall (ROC/PR), and calibration metrics by subgroup rather than only aggregate scores; look for systematic performance gaps.
...
\begin{enumerate}
    ...
    \item \textbf{Privacy \& security.} Scan corpora for personally identifiable information (PII), redact when necessary, and restrict raw-data access. When embeddings leave the lab, accompany them with an acceptable-use policy and redaction guarantees.
...
\paragraph{Contextual embeddings and transformers.}\label{sec:nlp_contextual_embeddings_and_transformers}
...
\textbf{Exercise 1 (Easy).} Train a small skip\hyp{}gram (negative sampling) embedding model on a modest corpus and audit the geometry: nearest neighbors, cosine similarity, and one analogy-style probe (even if it fails). \\
\textbf{Deliverable:} three neighbor lists + one short note explaining one neighbor that surprised you (and what it says about distributional meaning).

\smallskip
\textbf{Exercise 2 (Medium).} Compare two embedding objectives on the same corpus: a local-window method (word2vec-style) and a global method (GloVe-style or a matrix-factorization baseline). Focus on what changes \emph{qualitatively}: which words move, which neighborhoods stabilize, and which analogies become more/less consistent. \\
\textbf{Deliverable:} one side-by-side neighborhood comparison + one paragraph explaining the difference as an objective/weighting effect.

\smallskip
\textbf{Exercise 3 (Medium).} Run one bias audit and treat it as an engineering check, not a headline: pick a simple test (e.g., WEAT-style, neighborhood bias, or projection tests), then try one mitigation (corpus filtering, re-weighting, or a debiasing transform) and report what it changes and what it does not. \\
\textbf{Deliverable:} the bias score(s) before/after + one paragraph stating a limitation of your chosen test.
...
\medskip
\paragraph{The fundamental limitation of static embeddings.}
Every embedding learned in this chapter assigns \emph{one} vector per word type---the same vector for ``bank'' regardless of whether the surrounding tokens describe a river or a financial institution. This type-level representation is a deliberate simplification: it keeps training cheap and enables the geometric structure (offset analogies, co-occurrence geometry) discussed above. But it fails for polysemous words, and it makes the representation blind to syntactic role, clause scope, and long-range discourse context. Auditing nearest neighbors (\S above) reveals this directly: the ten closest words to ``bank'' are a mixture of financial and geographical senses, reflecting corpus frequency rather than in-context meaning.

\paragraph{Contextual representations: the path forward.}
The solution is to replace the lookup table with a \emph{context-sensitive encoder}: a function that maps a \emph{full token sequence} to one vector per position, not one vector per type. ELMo (Embeddings from Language Models; \citealp{Peters2018}) introduced this idea using a bi-directional stacked LSTM: the final token representations are weighted sums of all LSTM layers, capturing different levels of syntactic and semantic abstraction depending on the downstream task. BERT (Bidirectional Encoder Representations from Transformers; \citealp{Devlin2019}) replaced the recurrent backbone with the self-attention mechanism, enabling fully parallel computation over the whole sequence and scaling to much larger corpora and model sizes. Both models are pre-trained on large unlabeled corpora via self-supervised objectives (masked-language modeling for BERT; next/previous-token prediction for ELMo) and then \emph{fine-tuned} on labeled downstream data---a two-stage pipeline that re-uses the expensive pre-training across many tasks. \Cref{chap:transformers} opens precisely at the point where recurrent state is replaced by attention, making contextual representation practical and scalable. Keep the distributional intuitions of this chapter in view as you read: the geometry of token embeddings persists inside transformer models, but context is now injected via query--key--value attention rather than recurrence.

\medskip
\paragraph{Where we head next.} \Cref{chap:transformers} keeps the same token geometry but changes context formation: attention replaces recurrent state, so dependencies are queried directly. The same many\hyp{}to\hyp{}one decisions from \Cref{sec:rnn_sentiment_worked_example} remain, but computational path and scaling behavior change. Keep the same audit lens as you read: tokenization choices, masking correctness, and evaluation protocol matter as much as architecture. After that, \Cref{chap:softcomp} pivots to soft computing (fuzzy logic and evolutionary ideas) as an alternative paradigm for reasoning under uncertainty.
