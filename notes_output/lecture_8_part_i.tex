...
\begin{align}
    \mathbf{h} &= \mathbf{x}\mathbf{W}\in\mathbb{R}^{1\times d}. \label{eq:hidden_layer}
\end{align}
...
\begin{align}
    \mathbf{z} &= \mathbf{h}\mathbf{W}_{\text{out}} \in \mathbb{R}^{1\times V}. \label{eq:output_logits}
\end{align}
...
\end{align}
Training then minimizes cross\hyp{}entropy (equivalently, negative log-likelihood) against the one-hot target \(\mathbf{y}\):
\begin{align}
