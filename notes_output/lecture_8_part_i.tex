...
The solution is to replace the lookup table with a \emph{context-sensitive encoder}: a function that maps a \emph{full token sequence} to one vector per position, not one vector per type. ELMo (Embeddings from Language Models) introduced this idea using a bi-directional stacked LSTM: the final token representations are weighted sums of all LSTM layers, capturing different levels of syntactic and semantic abstraction depending on the downstream task. BERT (Bidirectional Encoder Representations from Transformers; \citealp{Devlin2019}) replaced the recurrent backbone with the self-attention mechanism, enabling fully parallel computation over the whole sequence and scaling to much larger corpora and model sizes. Both models are pre-trained on large unlabeled corpora via self-supervised objectives (masked-language modelling for BERT; next/previous-token prediction for ELMo) and then \emph{fine-tuned} on labelled downstream data---a two-stage pipeline that re-uses the expensive pre-training across many tasks. \Cref{chap:transformers} opens precisely at the point where recurrent state is replaced by attention, making contextual representation practical and scalable. Keep the distributional intuitions of this chapter in view as you read: the geometry of token embeddings persists inside transformer models, but context is now injected via query--key--value attention rather than recurrence.
