...
\Cref{chap:rnn} framed language as prediction under context: the model must assign probability to the next token from a compressed state. This chapter addresses the missing ingredient behind that setup: representations that turn discrete symbols into usable geometry. We build those embeddings, training objectives, and audits here; \Cref{chap:transformers} then reuses the same representation primitives inside attention-based architectures for longer-context modeling and deployment. The roadmap in \Cref{fig:roadmap} places this as the application/deployment tail of the neural strand.
