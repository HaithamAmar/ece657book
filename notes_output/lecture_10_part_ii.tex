...
This chapter reuses the trilogy defaults \((\wedge=\min,\ \vee=\max,\ C(\mu)=1-\mu)\), with \(T\) for generic t\hyp{}norms and \(S\) for aggregation alternatives when explicitly stated. If these symbols conflict with earlier chapters, use local definitions and refer to \Cref{app:notation_collisions}.
\end{tcolorbox}
