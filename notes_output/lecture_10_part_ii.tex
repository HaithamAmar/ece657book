...
\]
These come directly from the definitions: \(\mu_{\text{Cold}}(-2)=(0-(-2))/5=0.4\) and \(\mu_{\text{Rising}}(0.5)=0.5/2=0.25\).
With \(T=\min\) (\eqref{eq:antecedent_min}), the rule firing strengths are
\[
...
Using Mamdani min-implication (\eqref{eq:min_implication}), each consequent is clipped at its firing strength (e.g., \(\mu_{\text{High}}'(u)=\min(\alpha_1,\mu_{\text{High}}(u))\)). Aggregating consequents by max (\eqref{eq:output_aggregation}) yields \(\mu_{\text{out}}(u)\).
