...
\]
With \(T=\min\) (\Cref{eq:antecedent_min}), the rule firing strengths are
\[
...
Using Mamdani min-implication (\Cref{eq:min_implication}), each consequent is clipped at its firing strength (e.g., \(\mu_{\text{High}}'(u)=\min(\alpha_1,\mu_{\text{High}}(u))\)). Aggregating consequents by max (\Cref{eq:output_aggregation}) yields \(\mu_{\text{out}}(u)\).
Finally, centroid defuzzification (\Cref{eq:centroid_defuzz}) produces a crisp heater-power command. Using a uniform grid on \(u \in [0,1]\) (step \(10^{-4}\)) gives
\[
u^\star \approx 0.577.
\]
