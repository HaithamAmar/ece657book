...
\index{fuzzy set}
\index{fuzzy set!type-2 fuzzy set}
\index{fuzzy set!interval type-2 (IT2)}
\index{membership function}
\index{membership function!triangular}
\index{membership function!trapezoidal}
\index{membership function!Gaussian}
\index{defuzzification}
\index{centroid (defuzzification)}
\index{footprint of uncertainty (FOU)}
\index{Karnik--Mendel algorithm}
\graphicspath{{assets/lec9/}{assets/lec16/}}

\noindent Building on \Cref{chap:softcomp}, we formalize the foundations of fuzzy logic: universes of discourse, fuzzy sets, membership functions, and the operators used throughout the fuzzy trilogy. You will see some ideas from \Cref{chap:softcomp} again, but now in a form you can compute and sanity-check numerically (a deliberate preview\(\rightarrow\)formalization pattern). These tools define the thermostat labels used later and prepare the transfer/inference steps developed in \Crefrange{chap:fuzzyrelations}{chap:fuzzyinference}.
...
\end{equation}
With this design, \(e=-2\) yields \(\mu_{\text{Cold}}(-2)=0.5\): ``cold'' is half true (and in the linear region \(e<0\), so \(-e>0\)). Later chapters reuse this shoulder \emph{shape} but may shift its breakpoints to reflect different sensors, comfort bands, or operating assumptions. This number is not a probability; it is a degree of truth for a linguistic predicate. The rest of this chapter is about making these mappings explicit (shapes, overlaps, and operators) so they can be inspected and tuned rather than assumed.

\begin{tcolorbox}[summarybox, title={Author's note: membership design is where judgment enters}]
Membership functions are an explicit way to encode boundary subjectivity. At the extremes, many labels are uncontroversial (very short vs.\ very tall, clearly cold vs.\ clearly hot). The modeling work happens in the overlap zone. We choose shapes and breakpoints so borderline cases are represented as graded truth rather than forced yes/no decisions. In practice, start by anchoring obvious extremes from measurements, then tune overlap widths to match observed behavior and control smoothness.
\end{tcolorbox}

\subsection{Fuzzy sets and the universe of discourse}
\label{sec:fuzzysets_recap_fuzzy_sets_and_the_universe_of_discourse}

A \emph{fuzzy set} \( A \) in a universe of discourse \( X \) is characterized by a \emph{membership function} \(\mu_A: X \to [0,1]\). This membership function assigns to each element \( x \in X \) a degree of membership \(\mu_A(x)\), which quantifies the extent to which \( x \) belongs to the fuzzy set \( A \). Throughout the fuzzy trilogy, this notation stays fixed: \(\mu_A\) always means membership of set \(A\) over the currently named universe \(X\), so when we switch from temperature error to speed or rule consequents, we restate \(X\) rather than changing the meaning of \(\mu_A\).
...
\paragraph{Mathematical representation.} For each \(x \in X\), the membership grade satisfies \(\mu_A(x)\in[0,1]\).
...
The thermostat's \textit{Cold} label in \Cref{eq:fuzzysets_cold_membership_example} is a left-shoulder special case of this family: membership stays at 1 up to the cold threshold, then decreases linearly to 0 at the comfort boundary.
...
\begin{tikzpicture}
    \begin{axis}[
            ...
            \path[fill=cbBlue!10, draw=none] (axis cs:75,0) rectangle (axis cs:80,1.05);
            % Grade C trapezoid
            \addplot[thick, cbBlue] coordinates {
            ...
            \addplot[gray, dashed] coordinates {(75,0) (75,1.05)};
            \addplot[gray, dashed] coordinates {(80,0) (80,1.05)};
            \node[font=\scriptsize, cbBlue!60!black] at (axis cs:77.5,0.68) {shared region};
        \end{axis}
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Trapezoidal membership functions for grades C and B with the shared region highlighted. Scores between 75 and 80 partially satisfy both grade definitions. This overlap keeps boundary cases smooth instead of forcing abrupt label switches.}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption[Overlapping membership functions for Small/Medium/Large labels]{Overlapping membership functions for the ``Small'', ``Medium'', and ``Large'' weight labels. The shaded overlaps capture gradual transitions and reduce abrupt rule changes for small numeric perturbations.}
...
\paragraph{Shoulder and Bounded-Core Fuzzy Sets}
Three shape families cover most linguistic labels:
\begin{itemize}
  \item A \emph{left-shoulder set} (sometimes called an open-left set in older literature) reaches membership 1 for all sufficiently small \(x\) and then decreases smoothly to 0; useful for labels such as ``Very Cold'' or ``Very Low.''
  \item A \emph{right-shoulder set} (open-right) mirrors this for large \(x\); useful for labels such as ``Very Hot'' or ``Very High.''
  \item A \emph{bounded-core set} (sometimes called a closed set in older texts, though that usage is non-standard) attains membership 1 only on a bounded interval and tapers to 0 on both sides; triangular and trapezoidal shapes are the canonical examples (e.g., ``Comfy'' temperature, ``Medium'' speed).
\end{itemize}
These distinctions help in modeling asymmetric uncertainties or preferences. This shoulder/bounded-core terminology is standard in modern fuzzy-set writing; avoid the informal ``open/closed'' labels in formal definitions.

\begin{tcolorbox}[summarybox, title={Type-2 Fuzzy Sets: handling uncertain membership}]
In a standard (type-1) fuzzy set the membership value \(\mu_A(x)\) is a crisp number. When expert knowledge is itself imprecise or when membership functions are learned from noisy data, it is more realistic to let the membership value itself be a fuzzy set. This leads to \emph{type-2 fuzzy sets} \citep{Zadeh1975}.

The \textbf{footprint of uncertainty (FOU)} is the bounded region in the \((x, \mu)\) plane swept out by all admissible membership grades:
\[
  \text{FOU}(A) = \bigcup_{x \in X} [{}^{l}\mu_A(x),\; {}^{u}\mu_A(x)],
\]
where \({}^{l}\mu_A\) and \({}^{u}\mu_A\) are the lower and upper membership functions bounding the uncertainty.

An \textbf{interval type-2 (IT2) fuzzy set} is the practically dominant special case: at each \(x\) the secondary membership is uniformly 1 over the FOU interval and 0 outside. IT2 sets are used in robust fuzzy controllers (thermostat, traffic signal, servo drives) when sensor noise or time-varying parameters make crisp breakpoints unreliable.

\textbf{Reduction:} before using an IT2 fuzzy set for inference, it must be \emph{type-reduced} to a type-1 set (or a crisp value). The Karnik--Mendel iterative algorithm is the standard method; simpler closed-form approximations (Nie--Tan method) are often sufficient in practice.
\end{tcolorbox}
...
\begin{itemize}
    \item \textbf{Probability} models uncertainty about events under a probabilistic model. In both frequentist and Bayesian interpretations, probabilities satisfy additivity over mutually exclusive events:
    \[
    \sum_i P(E_i) = 1.
    \]
    For example, the probabilities of mutually exclusive outcomes (red, blue, black) in one draw sum to 1.

    \item \textbf{Possibility} measures compatibility/plausibility of values with a vague concept, and it is not additive in the probabilistic sense. A possibility distribution \(\pi(x)\) is usually normalized by \(\sup_x \pi(x)=1\), not by summation.
\end{itemize}

Thus, membership functions in fuzzy sets represent \emph{possibility} rather than \emph{probability}. This distinction is fundamental in fuzzy logic and inference (cf.~\Cref{tab:fuzzy-vs-prob} in \Cref{chap:softcomp}). When treating a membership as a possibility distribution \(\pi(x)\), we normalize by \(\sup_x \pi(x)=1\), yielding \(\Pi(A)=\sup_{x\in A}\pi(x)\) and \(N(A)=1-\Pi(A^c)\).

A practical sanity check is to ask what ``0.7'' means in context. If it is probability, it speaks to event uncertainty under a probability model. If it is fuzzy membership, it means ``compatible to degree 0.7'' with a linguistic predicate. The same number can appear in both settings, but the semantics and valid operations are different.
...
While operations such as union, intersection, and complement define how to combine or modify fuzzy sets, \emph{operators} formalize the rules behind those combinations. In practice, an operator is a mapping on membership grades that encodes how conservative or permissive the model should be when evidence conflicts.

Three operator families appear repeatedly in the fuzzy trilogy:
\begin{itemize}
    \item \textbf{Unary operators:} complement and linguistic hedges (for example, ``not,'' ``very,'' ``more or less'') that reshape a single membership function.
    \item \textbf{Binary operators:} t\hyp{}norms and s\hyp{}norms that implement fuzzy AND/OR and determine how multiple antecedents combine.
    \item \textbf{Relation operators:} extension, projection, and composition operators that transport fuzzy information between related universes.
\end{itemize}

The next subsection starts with unary negation operators, because complement choice directly affects De Morgan duality and downstream rule behavior.
% Chapter 16 (continued)
...
We use the standard fuzzy negation \(C(\mu)=1-\mu\), so \(\mu_{A^c}(x)=1-\mu_A(x)\) (see \eqref{eq:fuzzy_complement}). It is simple, involutive, and usually the safest default unless a parameterized negation is explicitly required.
...
\subsection{Triangular norms (t\hyp{}norms)}
\label{sec:fuzzysets_triangular_norms_t_norms}
...
For a geometric intuition, \Cref{fig:tnorm-surfaces} visualizes two common choices for fuzzy \texttt{AND}: the conservative minimum and the multiplicative product.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{lec16_fuzzy_and}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption[Fuzzy AND surfaces for common t\hyp{}norms]{Fuzzy AND surfaces comparing minimum versus product t\hyp{}norms; analogous OR surfaces show similar differences. Choices here influence rule aggregation in \Cref{chap:fuzzyinference}. The plot makes the design choice visible: conservative minimum versus multiplicative attenuation. \textit{Source: author-generated.}}
...
\subsection{Triangular conorms (t\hyp{}conorms / s\hyp{}norms)}
\label{sec:fuzzysets_triangular_conorms_t_conorms_s_norms}
...
An important relationship between t\hyp{}norms and s\hyp{}norms is their complementarity via a negation operator. Throughout this section we use the \emph{standard} fuzzy negation \(N(x) = 1 - x\), so that the complement of \(\mu_A\) and the duality between \(T\) and \(S\) can be written together as
\begin{equation}
\begin{aligned}
\mu_{A^c}(x) &= 1 - \mu_A(x),\\
...
\subsection{Examples of common t\hyp{}norm / s\hyp{}norm pairs}
\label{sec:fuzzysets_examples_common_t_s_norm_pairs}
...
\label{eq:fuzzy_subset_relation}
\end{equation}
...
Because fuzzy membership values lie in \([0,1]\), the subset relation in \eqref{eq:fuzzy_subset_relation} can also be quantified by a scalar measure indicating \emph{how much} \(A\) is included in \(B\).
...
\begin{equation}
\mu_{A^{(d)}}(x) = \big(\mu_A(x)\big)^{1/\alpha}, \qquad \alpha \geq 1 \quad \text{(dilation).}
\end{equation}\label{eq:dilation}

\begin{equation}
\mu_{A^{(c)}}(x) = \big(\mu_A(x)\big)^{\beta}, \qquad \beta \geq 1 \quad \text{(contraction).}
...
Two transfer cues matter for what comes next. First, treat membership functions as \emph{objects you can manipulate}: once the shapes are written down, you can reason about overlaps, negations, and edge cases without guessing. Second, the same max/min mindset will reappear when we move from sets to \emph{relations}: we will build fuzzy relations over Cartesian products and then compose them (a matrix-like operation) to propagate graded information between universes.

As you generate new membership functions below, keep a short audit checklist: verify the output stays in \([0,1]\), check endpoints and monotonicity where intended, and confirm that the universe/units match the linguistic meaning. Those checks are invariants you will want to preserve later when you tune breakpoints, operator defaults, or rule bases.
...
\centering
% Avoid inline math in captions; it wraps poorly in some EPUB renderers.
\caption{Typical operator choices in fuzzy inference and their qualitative effects. Here the t\hyp{}norm implements fuzzy AND, the s\hyp{}norm implements fuzzy OR, and the implication shapes consequents. The table supports default-operator selection for a Mamdani-style pipeline and anticipates qualitative output behavior.}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        \begin{groupplot}[
            ...
            tick label style={font=\footnotesize, text=gray!70!black},
            label style={font=\small},
            title style={font=\normalsize\bfseries, yshift=1ex},
            clip mode=individual
        ]
            % --- Panel A: Base sets + clipping ---
            \nextgroupplot[title={(A) Clipped consequents}, ylabel={Membership}]

            \addplot[cbBlue, thick] coordinates {(0,1) (0.5,0)};
            \addplot[cbOrange, thick] coordinates {(0.25,0) (0.5,1) (0.75,0)};
            \addplot[cbGreen, thick] coordinates {(0.5,0) (1,1)};

            \addplot[gray!70, dashed, thick] coordinates {(0,0.3) (1,0.3)};
            \addplot[gray!70, dashed, thick] coordinates {(0,0.133) (1,0.133)};

            \node[font=\footnotesize, text=gray!80!black, anchor=south west, inner sep=1pt] at (axis cs:0.01,0.31){$0.3$};
            \node[font=\footnotesize, text=gray!80!black, anchor=south west, inner sep=1pt] at (axis cs:0.01,0.143){$0.133$};

            \node[font=\footnotesize, text=cbBlue, anchor=south west] at (axis cs:0.02,0.85) {Low};
            \node[font=\footnotesize, text=cbOrange, anchor=south] at (axis cs:0.50,1.02) {Medium};
            \node[font=\footnotesize, text=cbGreen, anchor=south east] at (axis cs:0.98,0.85) {High};

            % --- Panel B: Aggregated output + centroid ---
            \nextgroupplot[title={(B) Aggregated output}]

            \addplot[draw=none, fill=gray!20] coordinates {
                (0.25,0) (0.325,0.3) (0.675,0.3) (0.71675,0.133) (1,0.133) (1,0) (0.25,0)
            };

            \addplot[cbOrange, thick] coordinates {
                (0.25,0) (0.325,0.3) (0.675,0.3) (0.75,0)
            };

            \addplot[cbGreen, thick] coordinates {
                (0.5,0) (0.5665,0.133) (1,0.133) (1,0)
            };

            \addplot[black!75, line width=1.1pt, line join=round] coordinates {
                (0.25,0) (0.325,0.3) (0.675,0.3) (0.71675,0.133) (1,0.133)
            };

            \addplot[red!75!black, dashed, thick] coordinates {(0.579755,0) (0.579755,0.5)};
            \addplot[red!75!black, only marks, mark=*, mark size=1.8pt] coordinates {(0.579755,0.15)};
            \node[
                ...
                fill opacity=0.8,
                text opacity=1
            ] at (axis cs:0.58,0.52) {$s^{\star}\approx 0.58$};
        \end{groupplot}
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{End-to-end fuzzy inference example. (A) Consequent membership functions with clipping levels from firing strengths at T = 27 deg C. (B) Aggregated output set (max of truncated consequents) and a centroid marker near s* approx 0.58. This trace provides an end-to-end sanity check for clipping, aggregation, and centroid defuzzification.}
...
\textbf{Exercise 1 (Easy).} Define 3--5 fuzzy labels for a new universe (e.g., vehicle speed, battery state of charge, or network latency). Compute membership degrees for three sample points and explain one design choice (coverage, overlap, or saturation). \\
\textbf{Deliverable:} the membership definitions + the three computed degree vectors + one sentence justifying your overlap.

\smallskip
\textbf{Exercise 2 (Medium).} Compare semantics, not just formulas: pick two t\hyp{}norm/s\hyp{}norm pairs (e.g., max--min vs.\ max--product) and compute union/intersection at several points. Interpret the numerical differences as different meanings of ``AND'' and ``OR.'' \\
\textbf{Deliverable:} a small pointwise table + one paragraph explaining which pair is more strict/lenient and in what sense.

\smallskip
\textbf{Exercise 3 (Medium).} Run a units/scale audit: take one set of membership functions and rescale the universe (change units or normalize), then show what must be adjusted so that behavior remains consistent downstream. \\
\textbf{Deliverable:} before/after plots (or parameter tables) + one paragraph describing how scale mistakes create controllers that ``never fire'' or ``always saturate.''

\medskip
\noindent\textbf{Fast-track note.} Later fuzzy chapters build directly on these primitives. If rule outputs become unstable, revisit universe scaling and membership overlap first.
\end{tcolorbox}

\paragraph{Where we head next.} \Cref{chap:fuzzyrelations} moves from set-level primitives to \emph{transfer}: how to push fuzzy labels through mappings between related universes (extension principle, projection, and composition). \Cref{chap:fuzzyinference} then uses those relation tools to assemble complete rule composition and defuzzification pipelines.
