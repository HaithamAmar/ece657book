...
\begin{tikzpicture}
    ...
    \caption{Schematic: Trapezoidal membership functions for grades C and B with the overlapping region shaded. Scores near 78--82 partially satisfy both grade definitions. Use this when designing overlapping grade/linguistic bins so boundary cases behave smoothly.}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption[Overlapping membership functions for Small/Medium/Large labels]{Schematic: Overlapping membership functions for the ``Small'', ``Medium'', and ``Large'' weight labels. The shaded overlaps capture gradual transitions.}
...
\begin{figure}[t]
    ...
    \ifdefined\HCode
        \caption{Schematic: Fuzzy AND surfaces comparing minimum versus product t\hyp{}norms; analogous OR surfaces show similar differences. Choices here influence rule aggregation in \Cref{chap:fuzzyinference}. Use this when deciding whether conjunction should behave like a conservative minimum or a multiplicative attenuation.}
    \else
        % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
        \caption{Schematic: Fuzzy AND surfaces comparing minimum versus product t\hyp{}norms; analogous OR surfaces show similar differences. Choices here influence rule aggregation in \Cref{chap:fuzzyinference}. Use this when deciding whether conjunction should behave like a conservative minimum or a multiplicative attenuation.}
    \fi
...
\centering
% Avoid inline math in captions; it wraps poorly in some EPUB renderers.
\caption{Schematic: Typical operator choices in fuzzy inference and their qualitative effects. Here the t-norm implements fuzzy AND, the s-norm implements fuzzy OR, and the implication shapes consequents. Use this when choosing default operators for a Mamdani-style pipeline and predicting qualitative behavior.}
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Schematic: End-to-end fuzzy inference example. (A) Consequent membership functions with clipping levels from firing strengths at T = 27 deg C. (B) Aggregated output set (max of truncated consequents) and a centroid marker near s* approx 0.58. Use this when sanity-checking clipping, aggregation, and centroid defuzzification end to end.}
