...
\begin{tikzpicture}
    ...
    \caption{Schematic: Trapezoidal membership functions for grades C and B with the overlapping region shaded. Scores near 78--82 partially satisfy both grade definitions.}
...
\begin{figure}[t]
    ...
    \ifdefined\HCode
        \caption{Schematic: Fuzzy AND surfaces comparing minimum versus product t\hyp{}norms; analogous OR surfaces show similar differences. Choices here influence rule aggregation in Chapter~\ref{chap:fuzzyinference}.}
    \else
        % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
        \caption{Schematic: Fuzzy AND surfaces comparing minimum versus product t\hyp{}norms; analogous OR surfaces show similar differences. Choices here influence rule aggregation in Chapter~\ref{chap:fuzzyinference}.}
    \fi
...
\centering
% Avoid inline math in captions; it wraps poorly in some EPUB renderers.
\caption{Schematic: Typical operator choices in fuzzy inference and their qualitative effects. Here the t-norm implements fuzzy AND, the s-norm implements fuzzy OR, and the implication shapes consequents.}
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Schematic: End-to-end fuzzy inference example. (A) Consequent membership functions with clipping levels from firing strengths at T = 27 deg C. (B) Aggregated output set (max of truncated consequents) and a centroid marker near s* approx 0.58.}
