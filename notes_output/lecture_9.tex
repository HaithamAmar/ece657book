...
\noindent Building on \Cref{chap:softcomp}, this chapter formalizes the foundations of fuzzy logic: universes of discourse, fuzzy sets, membership functions, and the basic operators used throughout the fuzzy trilogy. These tools define the thermostat labels used in later chapters and prepare the transfer/inference steps developed in \Crefrange{chap:fuzzyrelations}{chap:fuzzyinference}.
...
Use \Cref{fig:lec9-grade-trapezoids} as the derivation anchor for this section.
...
Use \Cref{fig:lec9-membership-overlap} as the derivation anchor for this section.

\Cref{fig:tnorm-surfaces} is the operator-shape reference for choosing conjunction behavior in rule aggregation.
...
Use \Cref{fig:tnorm-surfaces} as the derivation anchor for this section.
...
Use \Cref{tab:fuzzy-operators} as the derivation anchor for this section.

\begin{table}[t]
\centering
% Avoid inline math in captions; it wraps poorly in some EPUB renderers.
\caption{Typical operator choices in fuzzy inference and their qualitative effects. Here the t-norm implements fuzzy AND, the s-norm implements fuzzy OR, and the implication shapes consequents. Use it when choosing default operators for a Mamdani-style pipeline and predicting qualitative behavior.}
...
\Cref{tab:fuzzy-operators} is the operator-choice checklist referenced in the Mamdani design discussion.
...
Use \Cref{fig:fuzzy-end-to-end} as the derivation anchor for this section.
