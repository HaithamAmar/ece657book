...
Building on \Cref{chap:fuzzysets}, we address a fundamental question: \emph{How do we transfer fuzzy knowledge from one universe of discourse to another related universe?} This question arises whenever the same linguistic label must be reused across units, sensors, or derived variables (Celsius vs.\ Fahrenheit; position vs.\ velocity), each with its own universe and membership functions. The roadmap in \Cref{fig:roadmap} places this in the fuzzy-relations step of the soft-computing strand.
...
\paragraph{Notation.} Throughout this chapter we use the trilogy defaults stated above: \(\wedge=\min\), \(\vee=\max\), and complement \(1-\mu\). When we introduce a general t\hyp{}norm \(T\), it appears explicitly (e.g., in \Cref{eq:extension-two-var} and \Cref{eq:composition_general}).
...
\begin{figure}[t]
    ...
    \caption{Schematic: Mapping a fuzzy set through the function ``y = x-squared''. The membership at an output value y is the supremum over all pre-images x that map to y; shared images such as x = +/-1 map to y = 1 using the maximum membership. Use this when applying the extension principle to a non-invertible function.}
...
\begin{table}[h]
\centering
\caption{Schematic: Popular t\hyp{}norms and their typical roles. Use this when choosing a default conjunction operator and understanding its qualitative behavior.}
\label{tab:tnorms}
...
\begin{figure}[h]
    ...
    \end{tabular}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Schematic: Illustrative fuzzy relation table (left) together with its projections onto the error universe (middle) and the rate-of-change universe (right). These are the exact quantities used in the running thermostat example before composing rules. Use this when building rule antecedents from a relation and checking which universes each projection lives on.}
...
\paragraph{Where we head next.} In \Cref{chap:fuzzyinference}, each rule induces a fuzzy relation on (input \(\times\) output); sup-$T$ composition and projection compute the implied output set before aggregation and defuzzification. That chapter uses the same defaults (max--min with the standard complement), as summarized in \crefrange{eq:antecedent_min}{eq:output_aggregation}.
