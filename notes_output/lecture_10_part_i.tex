...
This chapter establishes the transfer machinery used by inference: move labels across universes with the extension principle, handle many-to-one mappings with sup aggregation, and keep operator choices consistent across composition, projection, and complement. When debugging, check domain feasibility first, then preimage aggregation, then operator consistency; most silent errors come from one of those three.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Exercises and lab ideas}]
Run one many-to-one transfer (e.g., \(y=x^2\)) and one constrained-domain transfer, then verify one reported membership value from code.
\end{tcolorbox}
