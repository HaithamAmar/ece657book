...
\begin{figure}[t]
    ...
    \caption{Mapping a fuzzy set through the function ``y = x-squared''. The membership at an output value y is the supremum over all pre-images x that map to y; shared images such as x = +/-1 map to y = 1 using the maximum membership. Use this when applying the extension principle to a non-invertible function.}
...
\begin{table}[h]
\centering
\caption{Popular t\hyp{}norms and their typical roles. Use this when choosing a default conjunction operator and understanding its qualitative behavior.}
\label{tab:tnorms}
...
\begin{figure}[h]
    ...
    \end{tabular}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Illustrative fuzzy relation table (left) together with its projections onto the error universe (middle) and the rate-of-change universe (right). These are the exact quantities used in the running thermostat example before composing rules. Use this when building rule antecedents from a relation and checking which universes each projection lives on.}
