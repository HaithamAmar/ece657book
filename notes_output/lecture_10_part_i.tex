...
Similarly, projection onto \( Y \), denoted \( \pi_Y(R) \), keeps each \(y\)-column and takes the strongest supporting \(x\)-value in that column:
\begin{equation}
...
Projection collapses relations back onto a target universe; use the definitions in \eqref{eq:projection_x}--\eqref{eq:projection_y} and apply them after cylindrical extension to recover the desired marginal.
