% Chapter 17
\section{Fuzzy Relations: Composition, Projection, and the Extension Principle}\label{chap:fuzzyrelations}
\index{extension principle}
\index{fuzzy relation}
\index{composition (fuzzy relations)}
\index{t-norm}
\index{t-conorm}
...
\paragraph{Rolling-window note (what changes vs.\ \Cref{chap:fuzzysets}).}
In \Cref{chap:fuzzysets}, the main design work was \emph{within} a universe: choose membership shapes, overlaps, and operators. Here the memberships are assumed given; the new question is \emph{between} universes: how does a fuzzy label move through a mapping without losing meaning? Treat this chapter as the transfer layer that makes the later inference pipeline (\Cref{chap:fuzzyinference}) reusable across sensors, units, and derived variables.
...
\paragraph{Notation.} When we introduce a general t\hyp{}norm \(T\), it appears explicitly in the multi\hyp{}variable extension principle and in the general relation\hyp{}composition rule. We point to it where those formulas are introduced. For symbol overloads when reading across parts, see \Cref{app:notation_collisions}.
...
\end{equation}
The strongest preimage membership determines the membership of \(y\). When the mapping depends on multiple fuzzy variables (e.g., \(f(x_1, x_2)\)), the individual memberships are combined with a chosen t\hyp{}norm before taking the supremum, as shown later in \eqref{eq:extension-two-var}.
...
\begin{itemize}
    ...
    \item In continuous settings we assume \(f\) is measurable so that the preimage sets \(\{x \mid f(x)=y\}\) are well-defined and the supremum exists.
\end{itemize}

\subsection{Interpretation}
\label{sec:fuzzyrelations_interpretation}

For single\hyp{}input mappings, the output membership is the strongest preimage; when multiple fuzzy inputs are involved we first combine their memberships with a chosen t\hyp{}norm (cf. \eqref{eq:extension-two-var}) and then take the supremum. It helps to read this as a two\hyp{}stage logic:

\emph{Within one candidate explanation} (one preimage tuple), a t\hyp{}norm plays the role of ``AND'' by combining the memberships that must simultaneously hold. \emph{Across all candidate explanations} that map to the same \(y\), the supremum plays the role of ``OR'' by keeping the strongest support. In discrete examples the supremum collapses to a max, which makes the meaning especially transparent.

This matters because membership values are degrees of compatibility, not probabilities to be summed. If many different \(x\) values map to the same \(y\), we do not want \(y\) to look ``more true'' simply because there are many redundant preimages; we want \(y\) to inherit the best support that the mapping can provide. This approach preserves the logical interpretation of membership values and respects the structure of the mapping \( f \).
...
\end{aligned}
\label{eq:extension_squared_example}
\end{equation}

Even on this very small domain the mapping \(f(x)=x^2\) is \emph{non-injective} (many-to-one): \(x=-1\) and \(x=1\) both map to \(y=1\). The supremum rule is precisely what handles this case --- it selects the strongest preimage membership rather than double-counting or averaging. Notice also that no \(x\) maps to a negative \(y\), so the mapping is \emph{non-surjective} onto \(\mathbb{R}\); for those output values the extension principle correctly returns \(\mu_B(y)=0\) (no supporting evidence). Together, these two properties --- non-injectivity and non-surjectivity --- are the general cases the definition \eqref{eq:membership-transform} is designed to handle.
...
\end{tcolorbox}
\paragraph{Visual intuition.} \Cref{fig:lec10-extension} walks through a simple mapping \(y=x^2\), showing how memberships on \(X\) lift to memberships on \(Y\) via the supremum over all preimages that map to the same point.
...
\begin{figure}[t]
    ...
    \caption{Mapping a fuzzy set through the function ``y = x-squared''. The membership at an output value y is the supremum over all preimages x that map to y; shared images such as \(x=\pm 1\) map to y = 1 using the maximum membership. Helpful when applying the extension principle to a non-invertible function.}
...
For two variables, the extension principle becomes:
\begin{equation}
...
\begin{equation}
\begin{aligned}
\mu_B(0) &= \min\{\mu_{A_1}(0), \mu_{A_2}(0)\} = 0.1,\\
\mu_B(1) &= \max\{0.1, 0.5, 0.1, 0.4\} = 0.5.
\end{aligned}
\label{eq:extension_two_var_example}
\end{equation}
...
We now focus on computational choices (discretization vs.\ alpha\hyp{}cuts) before moving on to projection operations.
...
T(a, b) &= \min(a, b).
\end{aligned}
\label{eq:relation_cartesian_product}
\end{equation}
...
The projection of \( R \) onto \( X \), denoted \( \pi_X(R) \), keeps each \(x\)-row and asks for the strongest compatible \(y\)-value in that row:
\begin{equation}
    \begin{aligned}
...
Similarly, projection onto \( Y \), denoted \( \pi_Y(R) \), keeps each \(y\)-column and takes the strongest supporting \(x\)-value in that column; the second line of \eqref{eq:projection_operators} records that operator.


\paragraph{Total Projection}

The \emph{total projection} of \( R \) is the maximum membership value over the entire relation; the third line of \eqref{eq:projection_operators} gives this global summary.
...
\end{itemize}
In implementation terms, these are row-wise and column-wise max operations on a relation matrix. That viewpoint becomes useful in \Cref{chap:fuzzyinference}, where rule relations are repeatedly projected back to the output universe before aggregation and defuzzification.
...
\begin{figure}[h]
    ...
    \]
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Illustrative fuzzy relation matrix (left) together with its row-wise and column-wise projections (middle/right). These are the exact quantities reused in the running thermostat example before composing rules. Use it to verify which universe each projection inhabits before the rule base is composed.}
...
Projection collapses relations back onto a target universe; use the operators in \eqref{eq:projection_operators} and apply them after cylindrical extension to recover the desired marginal.
...
\paragraph{From max--min to sup--\(T\)}
\label{sec:fuzzyrelations_recap_and_motivation}
Max--min composition is one instance of the broader sup--\(T\) family in \eqref{eq:composition_general}. Any valid t\hyp{}norm \(T\) (minimum, product, \L{}ukasiewicz) gives a consistent composition rule; max--min remains the default because it is easiest to audit. Keep the worked example from \Cref{sec:fuzzyrelations_fuzzy_inference_via_composition_of_relations} as the reference trace while reading the properties below.
...
While max--min is standard, other t\hyp{}norms and aggregation rules can be useful when you need smoother transitions or stronger attenuation of partial matches. The key requirement is consistency: pick one family and carry it through the full inference pipeline.

\begin{itemize}
    \item \textbf{Max-Product Composition:}
    \begin{equation}
    \mu_R(x, z) = \max_{y} \left( \mu_{R_1}(x, y) \cdot \mu_{R_2}(y, z) \right).
    \label{eq:max_product_composition}
    \end{equation}
    Compared with max--min, this penalizes weak intermediate links more strongly and often yields smoother control surfaces.

    \item \textbf{Averaging-style aggregations:} In some applications, the final \(\max_y\) is replaced by weighted averaging over \(y\)-paths to preserve contributions from multiple moderate paths rather than only the strongest one.
    This can improve smoothness, but it also weakens the clear ``best support path'' interpretation of max-based composition.
...
\begin{itemize}
    \item Apply the extension principle to move a fuzzy set through a mapping (and state the domain assumptions explicitly).
    \item Compute one sup--\(T\) composition end-to-end and explain where \(\sup\) (aggregation) and \(T\) (conjunction strength) enter.
    \item Choose an operator family once (t\hyp{}norm/s\hyp{}norm/complement) and carry it consistently through composition, projection, and aggregation.
\end{itemize}

\medskip
\noindent\textbf{Common pitfalls.}
\begin{itemize}
    \item Mixing operator defaults across steps and then trying to ``debug'' the surface after the fact.
    \item Losing track of universes/units so the transfer is computed on the wrong domain.
    \item Treating smoother alternatives as ``better'' by default: they change semantics, not just aesthetics.
\end{itemize}
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Exercises}]
\textbf{Exercise 1 (Easy).} Build a tiny fuzzy relation \(R(X,Y)\) on a discrete grid and compute both projections: ``what \(Y\) is plausible given \(X\)'' and ``what \(X\) is plausible given \(Y\).'' \\
\textbf{Deliverable:} the relation table + both projected fuzzy sets + one sentence interpreting each projection.

\smallskip
\textbf{Exercise 2 (Medium).} Compute one sup--\(T\) composition end-to-end for two relations under max--min and under max--product. Treat the comparison as a semantics question: what changes when conjunction is ``bottleneck'' vs.\ ``multiplicative''? \\
\textbf{Deliverable:} the composed relation under both choices + a short explanation of one entry that differs and why.

\smallskip
\textbf{Exercise 3 (Medium).} Apply the extension principle to a many-to-one mapping (e.g., \(y=x^2\)) under a constrained domain and show how the constraint changes the transferred membership. \\
\textbf{Deliverable:} a small numerical check (a few membership values) + one paragraph naming the implicit assumption you are making about admissible pre-images.
\end{tcolorbox}
