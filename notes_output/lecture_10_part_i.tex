...
For single-input mappings, the output membership is the strongest preimage; when multiple fuzzy inputs are involved we first combine their memberships with a chosen t\hyp{}norm (cf. \eqref{eq:extension-two-var}) and then take the supremum. Intuitively, this means:
...
\paragraph{Recap and motivation}
...
\paragraph{Generalization: sup--\(T\) composition}
\label{sec:fuzzyrelations_generalization_of_fuzzy_relation_composition}
Equation \eqref{eq:composition_general} already defines the general sup--\(T\) composition. A valid t\hyp{}norm \(T: [0,1]^2 \to [0,1]\) is commutative, associative, monotone in each argument, and satisfies \(T(a,1)=a\); common choices include the minimum, product, and \L{}ukasiewicz operators. Setting \(T(a,b)=\min(a,b)\) recovers the standard max--min composition used throughout this chapter.

\paragraph{Example pointer}
\label{sec:fuzzyrelations_example_calculation_of_composition}
The max--min example in \Cref{sec:fuzzyrelations_fuzzy_inference_via_composition_of_relations} already walks through a concrete composition; with that numeric pattern in mind, we now focus on the properties that hold for any valid sup--\(T\) operator.
...
\begin{itemize}
    \item For a many-to-one map (e.g., \(y=x^2\)), compute the transformed fuzzy set and document how multiple preimages are aggregated.
    \item Repeat one transfer example with a restricted domain and show exactly where infeasible \(x\)-values are removed.
    \item Reproduce one chapter figure from code and verify one reported membership value against your implementation.
\end{itemize}

\medskip
\noindent\textbf{Fast-track note.} The extension principle is the transfer layer for the full inference pipeline. In \Cref{chap:fuzzyinference}, each rule output uses the same transfer-and-aggregate pattern.
\end{tcolorbox}
