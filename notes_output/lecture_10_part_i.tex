...
\paragraph{Notation.} Throughout this chapter we use the trilogy defaults stated above: \(\wedge=\min\), \(\vee=\max\), and complement \(1-\mu\). When we introduce a general t\hyp{}norm \(T\), it appears explicitly (e.g., in \Cref{eq:extension-two-var} and \Cref{eq:composition_general}). For symbol overloads when reading across parts, see \Cref{app:notation_collisions}.
