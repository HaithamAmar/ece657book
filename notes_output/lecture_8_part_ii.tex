...
See \Cref{sec:softcomp_from_crisp_sets_to_fuzzy_sets} for the membership-function definition and \Cref{sec:softcomp_fuzzy_logic_motivation_and_intuition} for the crisp-versus-fuzzy intuition; here we keep a final example and the forward pointer.
...
\Crefrange{chap:fuzzysets}{chap:fuzzyinference} pick up the thermostat running example end to end. \Cref{chap:fuzzysets} constructs membership functions and operators (the primitives). \Cref{chap:fuzzyrelations} transports fuzzy labels across related universes via the extension principle (the transfer layer). \Cref{chap:fuzzyinference} then assembles complete Mamdani/Sugeno inference and defuzzification (the decision layer). Keep this overview handy: it is the conceptual map, while the fuzzy trilogy works through the algebra and the numeric sanity checks.
