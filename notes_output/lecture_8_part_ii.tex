...
\begin{itemize}
    \item Rewrite one crisp threshold policy (for example, braking or thermostat actuation) as fuzzy labels and evaluate at least three boundary cases.
    \item On one toy rule base, compare max--min and max--product operator choices and explain how the output behavior changes.
    \item Build a one-page setup sheet (universes, memberships, operator defaults) and use it to reproduce the same outputs after a delayed rerun.
\end{itemize}
