...
\paragraph{Running example (thermostat).}
We revisit a smart thermostat that regulates a room using two linguistic inputs (temperature error and rate of change) and one output (heater power). This compact scenario returns throughout the fuzzy trilogy: first to parameterize membership functions (\Cref{chap:fuzzysets}), then to transport labels across related universes (\Cref{chap:fuzzyrelations}), and finally to assemble complete inference systems (\Cref{chap:fuzzyinference}).
