...
The sequence strand treated language and perception as differentiable computation: stateful models in \Cref{chap:rnn}, representation layers in \Cref{chap:nlp}, and modern attention in \Cref{chap:transformers}. Many engineered systems, however, are constrained less by data or compute than by specification: the relevant concepts are linguistic (``too warm,'' ``slightly fast,'' ``acceptable risk'') and the controller must remain auditable. Soft computing makes that vagueness explicit.
...
Use \Cref{tab:fuzzy-vs-prob} as the derivation anchor for this section.
...
\Cref{tab:boolean-vs-fuzzy} is the quick translation reference from crisp logic operators to fuzzy operators.
...
Use \Cref{tab:boolean-vs-fuzzy} as the derivation anchor for this section.
\FloatBarrier
