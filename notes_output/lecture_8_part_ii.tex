...
\begin{itemize}
    ...
    \item Work through a compact thermostat/autofocus controller example that grounds the design choices introduced throughout the fuzzy trilogy.
\end{itemize}
\end{tcolorbox}
...
\paragraph{Running example (thermostat).}\label{par:softcomp_thermostat_intro}
Throughout the fuzzy trilogy we use a single running example: a smart thermostat that regulates a room by mapping two linguistic inputs to one linguistic output.

\begin{itemize}
    \item \textbf{Input 1 --- temperature error} \(e = T_{\text{room}} - T_{\text{set}}\) (\si{\celsius}), ranging roughly from \(-5\) to \(+5\). Labels: \textit{Cold}, \textit{Comfy}, \textit{Hot}.
    \item \textbf{Input 2 --- rate of change} \(\dot{e}\) (\si{\celsius\per\minute}), ranging from \(-2\) to \(+2\). Labels: \textit{Falling}, \textit{Stable}, \textit{Rising}.
    \item \textbf{Output --- heater power} \(u \in [0,1]\). Labels: \textit{Off}, \textit{Low}, \textit{Medium}, \textit{High}.
\end{itemize}

The fuzzy rule \emph{``IF error is Cold AND rate is Falling THEN power is High''} is the kind of statement the system must represent, evaluate, and defuzzify. \Cref{chap:fuzzysets} parameterizes the membership functions for each label, \Cref{chap:fuzzyrelations} shows how labels transfer across related universes, and \Cref{chap:fuzzyinference} assembles the complete inference pipeline. Keep this loop in mind as each formalism is introduced.
...
Soft computing, a label Zadeh popularized in the 1990s \citep{Zadeh1994}, is a computational paradigm designed to handle problems where precision and certainty are either impossible or prohibitively expensive to obtain. Unlike hard computing, soft computing tolerates \emph{imprecision}, \emph{uncertainty}, and \emph{approximate reasoning} to achieve solutions that are:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Neurocomputing (and neuro-fuzzy hybrids):} Learning from data through neural networks, sometimes combined with fuzzy rule bases (e.g., adaptive neuro-fuzzy inference system (ANFIS)),
...
\begin{itemize}
    ...
    \begin{equation}
        ...
        \label{eq:softcomp_cnn_probs_example}
    \end{equation}
\end{itemize}
...
Hard computing insists on crisp logic and exact arithmetic: precise thresholds, deterministic algorithms, and guarantees when assumptions hold. Soft computing is approximate by design, trading exactness for robustness when concepts are linguistic, measurements are noisy, or the specification itself is negotiable. Many real systems blend the two: hard constraints and safety checks wrapped around a soft inner loop (approximate reasoning or heuristic search), chosen based on which assumptions you can justify and which failure modes you can tolerate.
...
Soft computing is defined in \Cref{sec:softcomp_soft_computing_motivation_and_definition}; here we map its constituents to where they are developed in this book so you can follow the learning outcomes above without re-reading the same motivation.
\begin{itemize}
    \item \textbf{Fuzzy logic:} this chapter and the fuzzy trilogy \Crefrange{chap:fuzzysets}{chap:fuzzyinference} build membership functions, rules, and inference pipelines.
    \item \textbf{Neurocomputing and hybrids:} neural models live in \Crefrange{chap:supervised}{chap:transformers}; neuro-fuzzy hybrids such as ANFIS \citep{Jang1993} sit at the interface.
    \item \textbf{Probabilistic modeling:} the statistical strand in \Crefrange{chap:supervised}{chap:logistic} remains complementary to fuzzy possibility views \citep{Dubois1988}.
    \item \textbf{Evolutionary computation:} \Cref{chap:evo} shows how evolutionary search tunes rule bases and membership parameters \citep{Herrera2008,Ishibuchi2007}.
\end{itemize}


\begin{table}[t]
\centering
\caption{Fuzzy vs.\ probabilistic reasoning. Distinguishes randomness (probability) from graded concepts (fuzziness) when interpreting uncertainty.}
...
\Cref{tab:boolean-vs-fuzzy} maps crisp logic operators to their fuzzy counterparts.

\begin{table}[t]
\centering
\caption{Boolean vs.\ fuzzy operators. Mapping from crisp logic rules to graded operators for fuzzy inference.}
...
\paragraph{Why this is useful in practice}
Classical rule systems and purely statistical pipelines often assume either crisp thresholds or well-specified probability models. Fuzzy logic is useful when neither assumption is realistic: the data are coarse, the concepts are linguistic, and the control policy must still be auditable. In that setting, fuzzy sets let us encode expert knowledge directly, preserve graded boundaries, and still produce an interpretable decision path.
...
Neural networks, evolutionary search, probabilistic modeling, and fuzzy logic solve different bottlenecks, even when they are applied to the same system. Neural networks are strongest when enough data exist and predictive accuracy is the main objective. Evolutionary methods are effective when the objective is rugged, discontinuous, simulator\hyp{}in\hyp{}the\hyp{}loop, or otherwise unfriendly to gradients. Probabilistic models are strongest when uncertainty semantics matter (calibration, risk, and decision thresholds). Fuzzy logic is strongest when the bottleneck is \emph{specification}: concepts are linguistic, boundaries are negotiable, and the decision logic must remain inspectable.

In practice, these paradigms are complementary building blocks. A fuzzy controller can sit on top of neural features; evolutionary search can tune membership functions, rule weights, or neural hyperparameters; and probabilistic tools can sit alongside either when you need the output to carry a defensible notion of uncertainty. The goal of this part is not to pick a winner, but to give you multiple ways to represent, update, and audit intelligent behavior depending on what the application can actually provide (data, expert knowledge, simulators, or constraints).
...
Zadeh's contribution was to formalize a practical middle ground: represent imprecise concepts with membership functions, reason with them compositionally, and keep the rules close to human language. The point was not to replace probability, but to model vagueness directly when boundaries are conceptual rather than random.
...
Despite its advantages, fuzzy logic has two honest challenges. First, there is rarely a single systematic recipe that turns ``expert knowledge'' into a complete, consistent rule base; design is iterative and coverage-driven. Second, linguistic labels still need engineering choices: membership functions and ranges come from elicitation or data-driven tuning, and poor choices show up directly as brittle or non-monotone behavior. We treat this as a design-and-audit problem: make defaults explicit, test edge cases numerically, and tune only after the basic behavior is correct.
% Chapter 15 (continued)
...
\end{aligned}
\label{eq:softcomp_relational_union_intersection}
\end{equation}

The third canonical operator is the set difference
\begin{equation}
A \setminus B = \{ x \mid x \in A \text{ and } x \notin B \},
\label{eq:softcomp_relational_set_difference}
\end{equation}
which removes from \(A\) any elements that also belong to \(B\). For instance, if \(A\) is the set of all graduate students and \(B\) the set of research assistants, then \(A \setminus B\) contains graduate students who are not currently research assistants.
...
\paragraph{Fuzzy logic as a mathematical language.}
\label{sec:softcomp_fuzzy_logic_as_a_new_mathematical_language}
Zadeh's insight was to synthesize these classical mathematical languages into a framework with graded truth values; the next section builds the intuition and contrasts crisp versus fuzzy reasoning.
...
\begin{tcolorbox}[summarybox, title={Author's note: why study fuzzy systems now?}]
Fuzzy inference began in the 1960s, so it is reasonable to ask why this material still matters in a world shaped by large language models. The answer is practical and methodological. Practically, fuzzy systems remain useful when specifications are linguistic, data are limited, and the decision path must stay auditable (for example, policy-heavy control settings). Methodologically, they expose a core AI design pattern: represent vague concepts explicitly, compose rules, and inspect the resulting behavior under constraints.
\par\smallskip
This chapter is therefore not nostalgia. It is one lens for how intelligent behavior can be engineered. Modern systems can combine lenses: neural models for representation learning, fuzzy rules for explicit policy structure, and evolutionary search for tuning when gradients are unreliable.
\end{tcolorbox}
...
0 & \text{if } x \notin A.
\end{cases}
\label{eq:softcomp_characteristic_function}
\end{equation}
...
\begin{equation}
\mu_{\tilde{A}}: X \to [0,1],
\label{eq:softcomp_membership_function}
\end{equation}
...
\paragraph{Thermostat at a glance.} Throughout \Crefrange{chap:fuzzysets}{chap:fuzzyinference} we reuse a fuzzy thermostat: inputs are temperature error and rate (linguistic labels such as \texttt{Cold}, \texttt{Warm}, \texttt{Hot}; \texttt{Falling}, \texttt{Stable}, \texttt{Rising}); rules map these to heater power; defuzzification turns the fuzzy action into a crisp control signal. Keep this loop in mind as membership functions and operators are introduced.
...
\begin{itemize}
    ...
    \item Write one rule: IF error is \texttt{Cold} AND rate is \texttt{Rising} THEN power is \texttt{Low}; preview centroid defuzzification.
\end{itemize}
\end{tcolorbox}
...
\noindent\textbf{Forward pointer.} \Cref{chap:fuzzysets} builds the membership functions and universes for the thermostat inputs/outputs; \Cref{chap:fuzzyrelations} provides the transfer operators (extension principle, projection, composition) that move fuzzy labels between related variables; and \Cref{chap:fuzzyinference} assembles full inference and defuzzification. \Cref{chap:evo} later returns to the other soft-computing pillar: evolutionary search as a tuning tool for rule bases and memberships.

% Chapter 15: Conclusion and Closure

\subsection{Wrapping up soft computing and previewing the fuzzy trilogy}
...
\paragraph{Fuzzy sets recap and why it matters in practice}
See \Cref{sec:softcomp_from_crisp_sets_to_fuzzy_sets} for the formal membership-function definition and \Cref{sec:softcomp_fuzzy_logic_motivation_and_intuition} for the crisp-versus-fuzzy contrast. The practical payoff is that one observation can carry multiple linguistic truths at once. For example, a height value can be partly ``short'' and partly ``medium,'' and that overlap is not ambiguity in the bad sense; it is deliberate tolerance around boundaries that would otherwise be brittle in a crisp model.

The same idea controls actuators more smoothly. In braking or thermostat control, we do not need to jump between ``off'' and ``full.'' We can combine fuzzy evidence and produce graded actions such as ``slightly increase'' or ``strongly reduce,'' which usually improves stability near operating thresholds.

\paragraph{Next steps: membership functions, transformations, and inference}
\Crefrange{chap:fuzzysets}{chap:fuzzyinference} now turn this motivation into a complete engineering pipeline. \Cref{chap:fuzzysets} builds the membership primitives. \Cref{chap:fuzzyrelations} moves those memberships between related universes through the extension principle. \Cref{chap:fuzzyinference} composes full rule systems and defuzzification into a final control decision. Keep this map in view: the next chapters add algebraic detail, but they serve one continuous workflow.

\paragraph{Part transition thesis}
As we move from the fuzzy trilogy to evolutionary computing, the design contract stays the same while the update mechanism changes. In Part III, behavior is shaped by rule composition over linguistic memberships. In Part IV, behavior is improved by population search over candidate encodings. Two different lenses, one engineering goal: build systems that perform reliably under realistic constraints.
...
\textbf{Exercise 1 (Easy).} Rewrite one crisp threshold policy (braking, thermostat actuation, or an alarm trigger) as fuzzy labels and test at least three boundary cases where crisp logic flips abruptly. \\
\textbf{Deliverable:} the crisp policy + the fuzzy rewrite + a short table of boundary-case inputs and outputs.

\smallskip
\textbf{Exercise 2 (Medium).} On one small rule base, compare max--min and max--product choices end-to-end and explain the behavioral difference in terms of ``how conjunction is interpreted.'' \\
\textbf{Deliverable:} one input\(\rightarrow\)output curve under both operators + one paragraph describing which regime is more conservative/aggressive and why.

\smallskip
\textbf{Exercise 3 (Medium).} Build a reproducibility sheet for a fuzzy controller: universes (with units), memberships (with parameters), operator defaults, discretization step, and test points. Rerun the same controller later and verify you can reproduce the outputs exactly. \\
\textbf{Deliverable:} the one-page sheet + a short note explaining which missing detail would make reproduction impossible.

\medskip
\noindent\textbf{Fast-track note.} As you move into \Crefrange{chap:fuzzysets}{chap:fuzzyinference}, keep operator choices explicit and consistent. Many downstream mismatches come from hidden defaults set at the start.
\end{tcolorbox}

\paragraph{Where we head next.} \Crefrange{chap:fuzzysets}{chap:fuzzyinference} carry the thermostat running example end-to-end: \Cref{chap:fuzzysets} builds membership functions, \Cref{chap:fuzzyrelations} transports information across universes through relations, and \Cref{chap:fuzzyinference} assembles full inference plus defuzzification.
