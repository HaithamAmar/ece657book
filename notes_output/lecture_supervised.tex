...
\begin{align}
R(h_\theta) &= \mathbb{E}_{(\mathbf{x}, y)\sim \mathcal P}\big[\ell\big(h_\theta(\mathbf{x}), y\big)\big], \label{eq:population_risk}\\
\hat R_N(h_\theta) &= \frac{1}{N} \sum_{i=1}^N \ell\big(h_\theta(\mathbf{x}_i), y_i\big). \label{eq:empirical_risk}
\end{align}
