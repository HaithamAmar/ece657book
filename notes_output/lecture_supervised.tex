...
\Cref{chap:symbolic} illustrated a non-statistical lens: solve problems by transformation search, with explicit goal tests. We now switch to the data-driven lens. The roadmap in \Cref{fig:roadmap} marks this as the core supervised strand.
...
\begin{tikzpicture}[scale=0.95, background rectangle/.style={fill=white}, show background rectangle]
    ...
    \ensuretikzbackgroundlayers
    \end{tikzpicture}
    \caption{Schematic: Why L1 promotes sparsity. Minimizing loss subject to an L2 constraint tends to hit a smooth boundary; an L1 constraint has corners aligned with coordinate axes, so tangency often occurs at a point where some coordinates are exactly zero. Use this when choosing between L1 and L2 penalties for feature selection.}
...
\end{tikzpicture}
    ...
    \caption{Schematic: A typical lasso path as the regularization strength increases. Coefficients shrink, and some become exactly zero, yielding sparse models. Use this when interpreting how penalty strength trades accuracy for sparsity.}
...
\begin{table}[!htbp]
\centering
\caption{Schematic: Common losses and typical use (reference for \Crefrange{chap:supervised}{chap:perceptron}). Use this when matching a loss to a modeling assumption and a downstream decision metric.}
...
\end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Schematic: Classification losses as functions of the signed margin z. Use this when comparing how different losses treat confident mistakes and near-boundary points.}
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
    ...
    \ensuretikzbackgroundlayers
    \end{tikzpicture}
    \caption{Schematic: Regression losses versus prediction error. The Huber loss transitions from quadratic to linear to reduce sensitivity to outliers. Use this when choosing a loss that is robust to heavy-tailed noise.}
...
\begin{tikzpicture}[x=0.9\linewidth,y=1.0cm, font=\scriptsize, background rectangle/.style={fill=white}, show background rectangle]
    ...
    \ensuretikzbackgroundlayers
    \end{tikzpicture}
    \caption{Schematic: Dataset partitioning into training, validation, and test segments. Any resampling scheme should preserve disjoint evaluation data; when classes are imbalanced, shuffle within strata so each split reflects the overall class mix. Use this when designing splits that support trustworthy model selection and reporting.}
...
\begin{tikzpicture}[scale=0.90, transform shape, node distance=0.60cm, every node/.style={font=\scriptsize}, background rectangle/.style={fill=white}, show background rectangle]
    ...
    \ensuretikzbackgroundlayers
    \end{tikzpicture}
    \caption{Schematic: Mini ERM pipeline (split once, iterate train/validate, then test only the best model on the held-out set). Use this when enforcing a clean separation between tuning and final reporting.}
...
\ensuretikzbackgroundlayers
\end{tikzpicture}
    \caption{Schematic: Learning curves reveal under/overfitting: the validation curve flattens while additional data continue to decrease training error only marginally. A shaded patience window marks when early stopping would halt if no validation improvement occurs. Use this when deciding whether you need more data, more capacity, or different regularization.}
...
\begin{figure}[t]
    ...
    \caption[Calibration and capacity diagnostics (reliability and double descent)]{Schematic: Calibration and capacity diagnostics. Left: reliability diagram with binned predicted probabilities vs. empirical accuracy; Expected Calibration Error (ECE) measures deviation from the diagonal. Right: illustrative double-descent risk vs. model size (log-scale on the x-axis); dashed line sketches a scaling-law trend used to choose capacity/regularization.}
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
    ...
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Schematic: Ridge regularization shrinks parameter norms as the penalty strength increases. Use this when tuning weight decay to control variance without forcing sparsity.}
...
\medskip
\paragraph{Where we head next.} In \Cref{chap:logistic}, we shift from continuous targets to discrete labels: the model predicts class probabilities, the loss becomes cross\hyp{}entropy, and the evaluation toolkit expands (confusion matrices, ROC/PR curves, and threshold selection).
