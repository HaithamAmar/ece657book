...
\begin{tikzpicture}[scale=0.95, background rectangle/.style={fill=white}, show background rectangle]
    ...
    \ensuretikzbackgroundlayers
    \end{tikzpicture}
    \caption{Schematic: Why L1 promotes sparsity. Minimizing loss subject to an L2 constraint tends to hit a smooth boundary; an L1 constraint has corners aligned with coordinate axes, so tangency often occurs at a point where some coordinates are exactly zero.}
...
\end{tikzpicture}
    ...
    \caption{Schematic: A typical lasso path as the regularization strength increases. Coefficients shrink, and some become exactly zero, yielding sparse models.}
...
\begin{table}[!htbp]
\centering
\caption{Schematic: Common losses and typical use (reference for \Crefrange{chap:supervised}{chap:perceptron}).}
...
\end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Schematic: Classification losses as functions of the signed margin z.}
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
    ...
    \ensuretikzbackgroundlayers
    \end{tikzpicture}
    \caption{Schematic: Regression losses versus prediction error. The Huber loss transitions from quadratic to linear to reduce sensitivity to outliers.}
...
\begin{tikzpicture}[x=0.9\linewidth,y=1.0cm, font=\scriptsize, background rectangle/.style={fill=white}, show background rectangle]
    ...
    \ensuretikzbackgroundlayers
    \end{tikzpicture}
    \caption{Schematic: Dataset partitioning into training, validation, and test segments. Any resampling scheme should preserve disjoint evaluation data; when classes are imbalanced, shuffle within strata so each split reflects the overall class mix.}
...
\begin{tikzpicture}[scale=0.90, transform shape, node distance=0.60cm, every node/.style={font=\scriptsize}, background rectangle/.style={fill=white}, show background rectangle]
    ...
    \ensuretikzbackgroundlayers
    \end{tikzpicture}
    \caption{Schematic: Mini ERM pipeline (split once, iterate train/validate, then test only the best model on the held-out set).}
...
\ensuretikzbackgroundlayers
\end{tikzpicture}
    \caption{Schematic: Learning curves reveal under/overfitting: the validation curve flattens while additional data continue to decrease training error only marginally. A shaded patience window marks when early stopping would halt if no validation improvement occurs.}
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
    ...
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Schematic: Ridge regularization shrinks parameter norms as the penalty strength increases.}
