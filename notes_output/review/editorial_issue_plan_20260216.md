...
- Publishability RC gate (2026-03-01): fixed EPUB citations rendering by enabling Pandoc citeproc with `notes_output/refs.bib` during the EPUB build (while suppressing citeproc's bibliography so the BibTeX-rendered References section remains the single source of truth). Added an EPUB citation visibility audit (`epub_builder/scripts/audit_epub_citations.py`) and wired it into `notes_output/scripts/run_production_checks.sh` (Apple + Kindle). Fixed an EPUB orphan-punctuation artifact near the seq2seq attention softmax explanation in `lecture_transformers.tex` and removed tab/trailing-whitespace issues flagged by publishing QC in `lecture_4_part_i.tex` and `lecture_4_part_ii.tex`. Release checks re-run; warning gate passed. Citation audit passed. EPUBCheck passed.
