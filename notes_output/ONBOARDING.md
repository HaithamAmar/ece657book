...
- If you change figures, run the Apple build first; Kindle is slower because it rasterizes math for broad device compatibility.
  - Cross-references (important):
    ...
    - Equation numbers are read from `notes_output/ece657_notes.aux` when present (generated by the PDF build). If the `.aux` file is missing, equation links still work but the displayed number may be `?`.
  - Visible numbering (EPUB polish):
    - Chapter headings are rewritten to include `Chapter N:` when the chapter `\label{chap:...}` is present.
    - Labeled subsection/subsubsection headings are prefixed with their `.aux` number (e.g., `1.3 ...`) so EPUB readers show section numbers.
