...
Sequence problems break that interface. When order matters, the model needs a state that can carry context forward, and the training loop must account for the fact that the same parameters are reused again and again across time.
...
One canonical sequence task is language modeling: predict the next token from the preceding context.
...
A minimal language-model run looks like this:
\begin{itemize}
    \item Choose a tokenization and vocabulary; map tokens to IDs.
    \item Turn IDs into vectors via a learned embedding table (\(\mathbf{x}_t\)).
    \item Pick a recurrent cell \(f\) (vanilla RNN, GRU, LSTM) and an output map \(g\).
    \item Train with the next-token objective \eqref{eq:auto_rnn_1c468f5940} using truncated BPTT and correct padding masks.
    \item Monitor log-likelihood/perplexity on a validation split, and monitor gradient norms (clipping is the first safety valve).
\end{itemize}
This is the same ERM loop as earlier chapters, but the bookkeeping now runs over \emph{time} as well as over batches.
\end{tcolorbox}
