% Chapter 12
\section{Recurrent Neural Networks: Dynamics, Memory, and Gating}\label{chap:rnn}
...
\Cref{chap:cnn} showed how architectural bias (convolution/pooling) improves sample efficiency while preserving the same optimization loop. This chapter keeps that loop but changes the axis from space to time: sequence models need memory so present predictions can depend on prior inputs. A useful continuity is \emph{parameter sharing}: CNNs reuse one filter across spatial locations, while recurrent neural networks (RNNs) reuse one transition rule across time steps. \Cref{fig:roadmap} flags this as the sequential branch of the neural strand.
...
\begin{itemize}
    ...
    \item Derive the forward dynamics and backpropagation-through-time (BPTT) updates for a simple (ungated) RNN cell.
...
In time series, yesterday changes what you expect at the next step. In language, the same words in a different order can mean something else entirely, and the next word depends on the whole prefix. You can see that in predictive text: ``I want to buy \ldots'' pulls you toward different completions than ``Write a book about Teddy \ldots'' even before you specify what you are buying or writing about. In control, current action depends on prior state; in speech, a phoneme depends on the surrounding phonemes.

One brute-force workaround is to build a fixed history window and feed it to a standard MLP: at time \(t\), concatenate \(\mathbf{x}_t,\mathbf{x}_{t-1},\ldots,\mathbf{x}_{t-k}\) and ask the network to predict \(y_t\). That can work for short memory, but it scales poorly as \(k\) grows, and it does not reuse parameters in a way that reflects the repeated structure of ``the same kind of step happens again and again''.

Recurrent neural networks make the smallest structural change that addresses the bottleneck: add a \emph{state} that is updated each step, and reuse the same update rule across time. The rest of this chapter is about making that idea precise, training it reliably, and recognizing the failure modes (especially gradient flow) that motivate gating and attention.
...
\begin{equation}
    ...
    \label{eq:rnn_state_update_generic}
\end{equation}
...
\paragraph{Terminology (steps and representations).}
Throughout this chapter, \(t\) indexes \emph{steps} in a sequence. In a time series, \(\mathbf{x}_t\) can be a measured feature vector. In other domains the raw observation at step \(t\) can be discrete (a symbol from a finite set) or structured (a set of features), but it is always converted into a numeric input vector \(\mathbf{x}_t\) before it enters the recurrence. We keep the derivations in \(\mathbf{x}_t\) so the same math applies across domains; \Cref{chap:nlp} makes the ``discrete symbols \(\rightarrow\) vectors'' story concrete for text.

\paragraph{Interpretation:} The hidden state $\mathbf{h}_t$ acts as a \emph{summary} or \emph{encoding} of the entire input history $\{\mathbf{x}_1, \mathbf{x}_2, \ldots, \mathbf{x}_t\}$. This allows the network to make predictions that depend on order and history, not just the current input.
...
\paragraph{What recurrence buys you.}
A feedforward network reacts to the current input \(\mathbf{x}_t\) unless you deliberately pack history into the input features. An RNN adds a state \(\mathbf{h}_t\) that carries information forward, so predictions can depend on order and on longer histories.
...
We start with the state-update picture, then unroll it to derive BPTT. From there we look at what breaks on long sequences (vanishing/exploding gradients) and the practical fixes (truncation, clipping, normalization, and gated cells), and we end with a short bridge to attention.
...
A simple (ungated) RNN is trained the same way as our earlier networks: pick a loss, compute gradients, and update parameters. For sequence prediction the default loss is cross\hyp{}entropy accumulated over time, with state updates of the form
\(\mathbf{h}_t = f(\mathbf{x}_t W_{xh} + \mathbf{h}_{t-1} W_{hh} + b_h)\)
and a per\hyp{}step predictive distribution \(p_\theta(y_t\mid \mathbf{h}_t)\).
The main design knobs are the state size and how far you backpropagate through time (full BPTT versus a truncation window). In practice, most failures are either gradient blow\hyp{}up/decay on long sequences or a train/inference mismatch when teacher forcing is used without a decoding strategy that matches it.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \end{groupplot}
    \end{tikzpicture}
    \caption{Decision boundaries for logistic regression (left) versus a shallow MLP (right). Linear models carve a single hyperplane, whereas hidden units can warp the boundary to follow non-convex manifolds such as the moons dataset.}
...
Here BCE (binary cross\hyp{}entropy) for a binary target $y\in\{0,1\}$ and logit $z$ is $\mathcal{L}(z, y)=\log(1+e^{-z})$ for $y{=}1$ and $\log(1+e^{z})$ for $y{=}0$; the \emph{logit} $z$ is the pre\hyp{}sigmoid score so that $\sigma(z)$ yields the predicted probability. If labels are recoded to \(y_{\pm}\in\{-1,+1\}\), the same loss is written compactly as \(\log(1+e^{-y_{\pm}z})\); do not mix that margin form with the \(y\in\{0,1\}\) convention used here. The middle panel contrasts a conservative schedule (smooth decay) with a more aggressive one (faster initial drop but risk of oscillation), and the right panel shows early stopping triggered when validation loss ceases to improve while training loss continues decreasing.
We will reuse these when tuning sequence models, where overfitting appears as a divergence between per\hyp{}step training and validation likelihood (per\hyp{}token in text models).
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \end{groupplot}
    \end{tikzpicture}
    \caption{Binary cross-entropy geometry (left), effect of learning-rate schedules on loss (middle), and the typical training/validation divergence that motivates early stopping (right).}
...
\begin{tcolorbox}[summarybox, title={Historical note: Hopfield networks and stable recurrence}]
\Cref{chap:hopfield} covered Hopfield networks as an associative memory: a recurrent system with symmetric weights and an energy function that decreases under the update rule, so the dynamics settle into stored patterns (attractors). That is a different use of recurrence than the one in this chapter.

Here we use recurrence to model \emph{ordered} data. The goal is not to converge to a fixed point, but to carry a useful state forward so predictions at time \(t\) can depend on earlier inputs. The shared theme is that dynamics matter; the diagnostics differ (energy trace and overlap for Hopfield, versus likelihood/perplexity and gradient health for sequence models).
\end{tcolorbox}
...
RNNs appear in a few standard input--output shapes. In a many\hyp{}to\hyp{}one task (e.g., sentiment), the model consumes \(\mathbf{x}_{1:T}\) and emits one label from the final state. In many\hyp{}to\hyp{}many tasks (e.g., tagging), it emits an output at every step. In one\hyp{}to\hyp{}many settings (conditional generation), it starts from a conditioning signal (an initial state or an encoder output) and then generates autoregressively. Bidirectional encoders run the recurrence forward and backward and concatenate states when you want both left and right history during labeling, or when you want a richer encoder representation for a decoder.
...
\begin{equation}\label{eq:rnn_state_update}
    \begin{aligned}
        \mathbf{h}_0 &= \mathbf{0} \qquad &&\text{(initial hidden state)},\\
        \mathbf{h}_t &= f(\mathbf{x}_t W_{xh} + \mathbf{h}_{t-1} W_{hh} + b_h), \qquad &&t=1,\ldots, T,\\
        \mathbf{y}_t &= g(\mathbf{h}_t W_{hy} + b_y), \qquad &&t=1,\ldots, T.
    \end{aligned}
\end{equation}
...
Padding mask \(M\in\{0,1\}^{B\times T}\): loss \(L=\sum_{b, t} M_{b, t}\,\text{CE}(\hat y_{b, t}, y_{b, t})/\sum_{b, t} M_{b, t}\), where \(\text{CE}(\cdot,\cdot)\) is the cross-entropy loss. Masks preview the padding/causal masks detailed in \Cref{chap:transformers}.
\end{tcolorbox}
...
\begin{tcolorbox}[summarybox, title={Numeric check: shared weights mean summed gradients}]
It helps to see the ``sum across time'' in one tiny example. Consider a scalar RNN with identity activation,
\[
h_t=W_{hh}h_{t-1}+W_{xh}x_t,\qquad y_t=h_t,
\]
and a two\hyp{}step squared\hyp{}error objective \(L=\frac{1}{2}\big[(y_1-t_1)^2+(y_2-t_2)^2\big]\).
Pick \(h_0=1\), \(W_{hh}=0.5\), \(W_{xh}=1\), \((x_1,x_2)=(1,2)\), and \((t_1,t_2)=(1,2)\). The forward pass gives
\[
h_1=1.5,\quad h_2=2.75,\quad e_1=y_1-t_1=0.5,\quad e_2=y_2-t_2=0.75.
\]
Because \(W_{hh}\) is reused, its gradient receives contributions from \emph{both} steps:
\[
\frac{\partial L}{\partial W_{hh}}
= e_1\frac{\partial h_1}{\partial W_{hh}} + e_2\frac{\partial h_2}{\partial W_{hh}}
= e_1 h_0 + e_2\big(h_1 + W_{hh}h_0\big)
= 2.0.
\]
The second term is the one students often miss: changing \(W_{hh}\) changes \(h_1\), which then changes \(h_2\). A finite\hyp{}difference check matches the value above.
\end{tcolorbox}
...
\begin{equation}
\mathbf{h}_t = \sigma_h \left( \mathbf{x}_t \mathbf{W}_{xh} + \mathbf{h}_{t-1} \mathbf{W}_{hh} + \mathbf{b}_h \right),
\label{eq:simple_rnn_hidden}
\end{equation}
\begin{equation}
\mathbf{y}_t = \sigma_y \left( \mathbf{h}_t \mathbf{W}_{hy} + \mathbf{b}_y \right).
\label{eq:simple_rnn_output}
\end{equation}
\begin{figure}[t]
    \centering
    \begin{tikzpicture}[
        ...
        \draw[arrow] (h2) -- node[above, font=\scriptsize] {$\mathbf{W}_{hh}$} (h3);
    ...
    \end{tikzpicture}
    \caption{Unrolled RNN over three time steps. The brace marks that the same parameters \(\mathbf{W}_{xh}\), \(\mathbf{W}_{hh}\), and \(\mathbf{W}_{hy}\) are reused at every step; backpropagation through time (BPTT) accumulates gradient contributions through all copies before updating those shared weights.}
...
The forward pass produces outputs \(\hat{y}_t\) and the loss accumulates over time; gradients are computed via BPTT on the unrolled graph (see the unrolling discussion in \Cref{sec:rnn_mathematical_formulation_of_a_simple_rnn_cell} and the flow in \Cref{fig:lec7-bptt}).
...
\begin{tcolorbox}[summarybox, title={Simple (ungated) RNN cell (forward + BPTT)}]
\begin{verbatim}
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    \caption{Backpropagation through time (BPTT): unrolled forward pass (black) and backward gradients (pink) through time.}
    \label{fig:lec7-bptt}
\end{figure}
...
Empirically, simple (ungated) RNNs often struggle to preserve dependencies beyond roughly 5--10 steps in language settings, which is why gated cells became the default for longer sequences.
\begin{figure}[t]
    \centering
    \includegraphics[width=0.72\linewidth]{lec7_vanish_explode}
    \caption[Vanishing and exploding gradients]{Vanishing (blue) versus exploding (orange) gradients on a log scale. The gray strip highlights the stability band; the inset reminds readers that repeated Jacobian products either shrink gradients (thin blue arrows) or amplify them (thick orange arrows). \textit{Source: author-generated.}}
...
\par\smallskip
\textbf{Author's note: do not fight a simple RNN.} If your task needs dependencies longer than a handful of steps, do not over-tune an ungated RNN. Start with clipping and a sensible truncation window, but move to GRU/LSTM once long-range information vanishes.
\end{tcolorbox}
...
\begin{figure}[t]
    ...
    \caption{Gradient norms (left) explode without clipping (orange) but remain bounded when the global norm is clipped at tau (green). Training loss (right) stabilizes as a result.}
...
\caption{Teacher forcing vs.\ inference in a sequence-to-sequence decoder. Gold arrows show supervised targets; orange arrows highlight autoregressive feedback that motivates scheduled sampling.}
...
\begin{figure}[!t]
    ...
    \noindent\resizebox{0.85\textwidth}{!}{%
        ...
        \ifdefined\HCode
            \caption{Long Short-Term Memory (LSTM) cell (Hochreiter and Schmidhuber, 1997; Gers, Schmidhuber, and Cummins, 2000).}
        \else
            \caption{Long Short-Term Memory (LSTM) cell \citep{Hochreiter1997,Gers2000}.}
        \fi
    \label{fig:lec7-lstm}
\end{figure}
...
\begin{figure}[!t]
    ...
    \ifdefined\HCode
    \caption{Gated Recurrent Unit (GRU) cell (Cho et al., 2014).}
    \else
    \caption{Gated Recurrent Unit (GRU) cell \citep{Cho2014}.}
    \fi
    \label{fig:lec7-gru}
\end{figure}
...
\begin{itemize}
    \item Pads leaking into the loss (mask missing or applied after reduction).
    \item Hidden state carried across unrelated sequences (forgetting to reset or detach).
    \item No clipping on long sequences (exploding gradients is the first failure mode).
    \item BatchNorm inside recurrence (prefer LayerNorm for sequence models).
    \item Teacher forcing at training time without thinking through inference-time decoding.
    \item Dropout applied independently at each step when you intended variational/recurrent dropout.
\end{itemize}
\end{tcolorbox}

\subsection{From Recurrent State to Attention: Bridging Gated RNNs and Transformers}
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Attention heatmap for a translation model. Rows are target tokens (decoder steps) and columns are source tokens (encoder positions). Each cell is an attention weight; the dot in each row marks the source position receiving the most attention.}
    \label{fig:lec7-attention}
\end{figure}


\subsection{Synthesis: RNN Derivations, Gating Decisions, and Sequence Modelling Limits}
\label{sec:rnn_wrapping_up_the_derivations}

One canonical sequence task is \emph{next-step prediction}: use the history up to step \(t\) to predict a target at step \(t+1\). The target can be continuous (forecasting), categorical (a label at each step), or discrete from a finite set (symbols/words). The mechanics are the same: define a per-step predictive distribution (or regression output), accumulate a loss over time, and train by BPTT on the unrolled computation graph.

\subsubsection{Optional: language modeling as next-step prediction (preview)}
\label{sec:rnn_optional_language_modeling_preview}
...
\begin{aligned}
    \mathbf{h}_t &= f(\mathbf{h}_{t-1}, \mathbf{x}_t; \theta), \\
    P(w_{t+1} \mid w_1, \ldots, w_t) &= g(\mathbf{h}_t; \theta),
\end{aligned}
\label{eq:rnn_output_prob}
\end{equation}
where \( \mathbf{x}_t \) is the input representation (e.g., a word embedding) of the token \( w_t \), \( f \) is the recurrent update function parameterized by \(\theta\), and \( g \) maps the hidden state to a probability distribution over the vocabulary. Because the hidden state is computed recursively, \(\mathbf{h}_t\) is a deterministic summary of the entire prefix \((w_1,\ldots,w_t)\):
\[
\mathbf{h}_t=f(f(\cdots f(\mathbf{h}_0,\mathbf{x}_1),\ldots),\mathbf{x}_t).
\]
...
\paragraph{Training objective (negative log-likelihood).}
Given a training sequence \( (w_1, w_2, \ldots, w_T) \), we typically train by minimizing the negative log-likelihood (NLL) (equivalently, maximizing the likelihood). With the next-token convention, the loss is
\begin{equation}
    \mathcal{L}(\theta) = -\sum_{t=1}^{T-1} \log P(w_{t+1} \mid w_1, \ldots, w_t; \theta).
\label{eq:rnn_next_token_nll}
\end{equation}

This is just cross-entropy over the vocabulary at each step: the model is rewarded for assigning high probability to the observed next token. A common reporting metric is \emph{perplexity}, defined as \(\exp(\text{average NLL per token})\); lower perplexity means the model assigns higher probability to the observed text. If you are not working with text, you can ignore perplexity---the underlying idea is simply ``negative log-likelihood per step.'' \Cref{chap:nlp} returns to this objective in a more application-facing way (tokenization, embeddings, and audit considerations).
...
This setup highlights a subtle but important dependency between \emph{representation} and \emph{generalization}. If tokens are treated as unrelated IDs (e.g., purely one-hot indicators), then ``great'' and ``awesome'' are orthogonal inputs: the model has no built-in reason to transfer evidence across similar words unless the training objective supplies a geometry that makes them nearby. In \Cref{chap:nlp}, we return to this same kind of task and show how learning word vectors from neighboring words gives the model a meaningful input space before any sequence model is applied.

\begin{tcolorbox}[summarybox, title={From sequence data to a next-step model (a minimal checklist)}]
A minimal sequence\hyp{}model run looks like this. First decide how each step becomes a numeric input vector \(\mathbf{x}_t\) (measured features, engineered features, or learned embeddings). Next decide the prediction target shape (many\hyp{}to\hyp{}one label, many\hyp{}to\hyp{}many labeling, or next\hyp{}step prediction), then pick the recurrent cell \(f\) (simple RNN, GRU, or LSTM) and the output map \(g\). Train with a per\hyp{}step loss summed over time using truncated BPTT and correct padding masks. Finally, monitor validation loss and gradient norms; clipping is the first safety valve when sequences get long.
\par\smallskip
It is the same ERM loop as earlier chapters; the new wrinkle is that the loss (and therefore the gradients) accumulates across time as well as across batches. In \Cref{chap:nlp}, we specialize this checklist to text (discrete symbols, embeddings, next\hyp{}token loss, and perplexity).
\end{tcolorbox}
...
\paragraph{Optional refresher (padding masks and encoders).}
Variable-length sequences are almost always padded to a common length for batching. The padding is not data, so you must carry a mask through the loss (and through attention later) so pads contribute \emph{zero} training signal. For encoder--decoder intuition, it helps to remember that many sequence models compress a history into a state (or a set of states) and then produce outputs from that representation; \Cref{chap:transformers} revisits this idea with attention rather than recurrence.

\begin{tcolorbox}[summarybox, title={Key takeaways}]
\begin{itemize}
\item Sequence losses add across time; shared parameters mean gradients accumulate across unrolled steps (BPTT).
\item Truncated BPTT and padding masks are the engineering tools that make training feasible on long and variable\hyp{}length sequences.
\item Stability tools recur across architectures: clipping, gating (GRU/LSTM), and normalization; attention offers an alternative route to long\hyp{}range dependencies.
\item For text, the sequence is discrete and inputs are embeddings; \Cref{chap:nlp} makes that concrete and introduces next\hyp{}token cross\hyp{}entropy and perplexity as a domain\hyp{}specific diagnostic.
\end{itemize}

\medskip
\noindent\textbf{A good checkpoint.}
\begin{itemize}
    \item Explain BPTT, truncation, and masking for variable\hyp{}length sequences.
    \item Describe why gates help (information and gradient flow) and when attention is a practical alternative.
    \item If you care about language modeling, relate cross\hyp{}entropy, log\hyp{}likelihood, and perplexity after reading \Cref{chap:nlp}.
\end{itemize}
...
\textbf{Exercise 1 (Easy).} Implement a padding-mask invariance test for a sequence model: pad the same batch to two different lengths and verify that (i) the loss is unchanged and (ii) gradients on real tokens are unchanged (to tight tolerance). \\
\textbf{Deliverable:} a short test that fails when masking is wrong + one paragraph explaining how mask bugs can masquerade as ``model improvements.''

\smallskip
\textbf{Exercise 2 (Medium).} Train a small time-series predictor and compare full BPTT to truncated BPTT (several window sizes \(K\)). Track training stability (gradient norms, loss oscillations) and final predictive quality under a fixed compute budget. \\
\textbf{Deliverable:} one plot showing the stability/quality trade-off across \(K\) + a short conclusion about credit assignment horizon.

\smallskip
\textbf{Exercise 3 (Medium).} Reproduce vanishing/exploding behavior by sweeping the effective scale of \(\mathbf{W}_{hh}\) (or its spectral norm proxy) and testing gradient clipping as a mitigation. \\
\textbf{Deliverable:} gradient-norm traces (with/without clipping) + one paragraph linking the observation to why gating (LSTM/GRU) exists.
...
\begin{tcolorbox}[summarybox, title={Beyond gated RNNs: state-space models (S4, Mamba)}]
Gated RNNs (LSTM/GRU) solve vanishing gradients but remain \emph{sequential}: computing \(\mathbf{h}_t\) requires \(\mathbf{h}_{t-1}\), so training cannot be parallelized across time the way Transformer attention can. Structured State-Space Models (S4) recast the recurrence as a \emph{convolution}: the state-space equations
\[
  \mathbf{h}'(t) = \mathbf{A}\mathbf{h}(t) + \mathbf{B}x(t),
  \qquad
  y(t) = \mathbf{C}\mathbf{h}(t),
\]
have a closed-form convolution kernel \(\bar{K} = (\mathbf{C}\mathbf{B},\, \mathbf{C}\mathbf{A}\mathbf{B},\, \mathbf{C}\mathbf{A}^2\mathbf{B}, \ldots)\) that allows \emph{parallel-scan} training across the full sequence in \(O(L \log L)\) time instead of \(O(L)\) sequential steps. At inference the recurrence form recovers constant-cost per-token generation. Mamba \citep{Gu2023Mamba} extends S4 with input-dependent (selective) state transitions, matching Transformer perplexity on language modeling while retaining linear-time inference.

\textbf{When to prefer SSMs over attention or gated RNNs:} (i) very long sequences (\(>8\mathrm{k}\) tokens) where quadratic attention cost is prohibitive; (ii) streaming or low-latency inference where constant-time recurrent steps matter; (iii) biological or physical time-series where the continuous-time state-space formulation aligns with the data-generating process.
\end{tcolorbox}
