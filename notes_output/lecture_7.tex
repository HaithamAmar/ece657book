...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    \ifdefined\HCode
        \caption{Binary cross-entropy geometry (left), effect of learning-rate schedules on loss (middle), and the typical training/validation divergence that motivates early stopping (right). Use this when choosing a learning-rate schedule and interpreting early-stopping signals.}
    \else
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    \caption{Backpropagation through time (BPTT): unrolled forward pass (black) and backward gradients (pink) through time. Use this when reasoning about truncation windows and why long histories strain gradient flow.}
    \label{fig:lec7-bptt}
\end{figure}
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Gradient norms (left) explode without clipping (orange) but remain bounded when the global norm is clipped at tau (green). Training loss (right) stabilizes as a result. Use this when setting clipping thresholds to stabilize training without freezing learning.}
...
\caption{Teacher forcing vs.\ inference in a sequence-to-sequence decoder. Gold arrows show supervised targets; orange arrows highlight autoregressive feedback that motivates scheduled sampling. Use this when diagnosing train/test mismatch from teacher forcing.}
...
\begin{figure}[!t]
    ...
    \noindent\resizebox{0.85\textwidth}{!}{%
        ...
        \ifdefined\HCode
            \caption{Long Short-Term Memory (LSTM) cell \citep{Hochreiter1997,Gers2000}. Use this when mapping gates and the cell state to the gradient-flow path.}
        \else
            \caption{Long Short-Term Memory (LSTM) cell \citep{Hochreiter1997,Gers2000}. Use this when mapping gates and the cell state to the gradient-flow path.}
        \fi
    \label{fig:lec7-lstm}
...
\begin{figure}[!t]
    ...
    \ifdefined\HCode
    \caption{Gated Recurrent Unit (GRU) cell \citep{Cho2014}. Use this when comparing gating to vanilla recurrence and to LSTM-style memory.}
    \else
    \caption{Gated Recurrent Unit (GRU) cell \citep{Cho2014}. Use this when comparing gating to vanilla recurrence and to LSTM-style memory.}
    \fi
    \label{fig:lec7-gru}
...
Natural language processing (NLP) requires converting words into numerical representations that sequence models can process. Since machines operate on numbers, words must be encoded appropriately. \Cref{chap:nlp} develops the embedding objectives and evaluation tools; here we focus on how those representations flow through recurrent computation.
...
\centering
\small
\caption{Feature-based word vectorization example. Each word is mapped to a vector of graded semantic features; fractional entries (e.g., 0.5) indicate mixed usage across contexts. Use this when building intuition for how discrete symbols become vectors that downstream models can consume.}
...
\begin{figure}[t]
    ...
    % Avoid inline math in captions; it wraps poorly in some EPUB renderers.
    \caption{Toy 2D projection of word embeddings showing neighboring clusters (countries vs. capitals). Light hulls highlight clusters; arrows show that country-to-capital displacement vectors align, a visual check on analogy structure. Use this when sanity-checking whether embedding offsets behave consistently across many pairs.}
