...
\paragraph{Terminology (time steps versus tokens).}
Throughout this chapter, \(t\) indexes \emph{steps} in a sequence. In a time series, \(\mathbf{x}_t\) can be a measured feature vector. In language, the sequence is discrete: each step corresponds to a \emph{token} \(w_t\) (a word or subword unit from a finite vocabulary), and the numeric input \(\mathbf{x}_t\) is typically an embedding vector looked up from a learned table. We keep the math generic in \(\mathbf{x}_t\) so the same derivations apply across domains.
...
One canonical sequence task is language modeling: predict the next token from the preceding prefix.
...
This is just cross-entropy over the vocabulary at each step: the model is rewarded for assigning high probability to the observed next token. A common reporting metric is \emph{perplexity}, defined as \(\exp(\text{average NLL per token})\); lower perplexity means the model assigns higher probability to the observed text.
...
\begin{tcolorbox}[summarybox, title={From corpus to next-token model (a minimal checklist)}]
A minimal language\hyp{}model run looks like this. First choose a vocabulary (and a tokenization scheme) and map the text to IDs. Those IDs become numeric inputs by looking up embeddings \(\mathbf{x}_t\). Next pick the recurrent cell \(f\) (simple RNN, GRU, or LSTM) and the output map \(g\), then train with the next\hyp{}token objective \eqref{eq:auto_rnn_1c468f5940} using truncated BPTT and correct padding masks. Finally, monitor validation NLL/perplexity and keep an eye on gradient norms; clipping is the first safety valve when sequences get long.
\par\smallskip
It is the same ERM loop as earlier chapters; the only new wrinkle is that the loss (and therefore the gradients) accumulates across time as well as across batches.
\end{tcolorbox}
...
\begin{itemize}
\item Language modeling is trained by minimizing next-token negative log-likelihood (cross-entropy); perplexity is a readable re-expression of that loss.
    \item Token vectors provide the numeric inputs; RNN hidden states encode history over time.
