...
\begin{itemize}
    \item \textbf{Core thread (RNNs):} state and unrolling $\rightarrow$ parameter sharing $\rightarrow$ BPTT $\rightarrow$ stabilization (clipping, gating motivation) $\rightarrow$ word representations as sequence inputs.
    \item \textbf{Optional reminders:} brief notes on padding and autoencoders appear later; skim them if you are here primarily for RNN dynamics and training.
\end{itemize}
\end{tcolorbox}
