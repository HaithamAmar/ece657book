...
\begin{itemize}
    ...
    \item \textbf{Optional detours:} quick padding/autoencoder reminders (for encoder intuition). For word representations and embedding objectives, jump to \Cref{chap:nlp}.
\end{itemize}
\end{tcolorbox}
...
In this chapter, we explored the foundational ideas behind modeling sequences with recurrent neural networks (RNNs). One common example is language modeling: predicting the next token in a sequence from its preceding context.
...
\medskip
\paragraph{Where we head next.} \Cref{chap:nlp} turns the sequence viewpoint into practical language tooling: embedding objectives, evaluation by analogies/neighborhoods, and the deployment/audit checklist for real use. Keep the sentiment example (\Cref{sec:rnn_sentiment_worked_example}) in mind: once tokens live in a geometry rather than as unrelated IDs, the same many\hyp{}to\hyp{}one classifier can generalize more systematically. \Cref{chap:transformers} then returns to architecture, replacing recurrence with attention so information can move globally and training can exploit parallel hardware.
