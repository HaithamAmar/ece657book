...
\begin{itemize}
    \item Train a many\hyp{}to\hyp{}one sentiment classifier; plot gradient norms with and without clipping.
    \item Train a small time\hyp{}series predictor; compare full BPTT vs.\ truncated BPTT windows and quantify the stability trade\hyp{}off.
    \item Implement a many\hyp{}to\hyp{}many tagging model; verify that padding masks do not affect the loss or gradients.
    \item Optional preview: train a small LSTM next\hyp{}token model; compare perplexity with/without weight tying and scheduled sampling (then revisit in \Cref{chap:nlp}).
