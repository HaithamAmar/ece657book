...
Convolutional networks are the simplest version of that bargain. They keep the ERM workflow from \Crefrange{chap:supervised}{chap:logistic} and the gradient\hyp{}training machinery from \Cref{chap:mlp}, but move a large fraction of the ``modeling bias'' into architecture. Local connectivity and weight sharing make images tractable: the model spends capacity on patterns that actually recur across a grid.
...
\Cref{fig:cnn_receptive_field_growth} provides the receptive-field intuition used in the depth discussion.
...
\paragraph{Deep optimization challenges.}
...
\Cref{fig:lec6-dropout} provides the validation-curve diagnostic view used for dropout decisions.
