...
There is also a useful continuity lens from earlier chapters: a convolutional filter is a learned \emph{local template} that is reused everywhere. In that sense it rhymes with the prototype/basis ideas from \Cref{chap:rbf,chap:som}---but with strict weight sharing and end-to-end gradient training, so the templates are shaped by the final task loss rather than chosen or tuned in isolation.
...
In practice, this is also where ``deep learning'' stopped being a promise and became a reliable workflow: large datasets and modern accelerators made the iterative training loop feasible, while architectural priors such as locality and weight sharing made that loop data-efficient on image grids.
...
\begin{itemize}
    ...
    \item \textbf{Going deeper:} depth as receptive-field growth, and why stacked small kernels can see large context.
...
Dense feedforward networks can be competitive on small tabular problems, but on images they become data-hungry fast: you are trying to learn spatial structure from scratch while fitting millions of parameters. The issue is not that the model class is ``wrong''; it is that the sample complexity and compute burden are badly mismatched to the grid structure of vision data.

\paragraph{High-dimensional inputs and flattening}

Consider image data, which is naturally represented as a 2D matrix (or 3D tensor for color images). For example, a single-channel (grayscale) image of size $256 \times 256$ pixels can be represented as a matrix:
\[
...
\begin{equation}
N_{\text{samples}} \gtrsim 10 \times 6{,}500{,}000 \approx 65{,}000{,}000,
...
We already derived the gradient-descent training loop in \Cref{chap:mlp}: define a loss \(\mathcal{L}\), compute gradients, then update parameters. CNNs do not change that contract. What changes is the \emph{geometry} of the representation: convolutional weights are shared across space, so one filter update aggregates learning signal from every spatial location where that filter was applied.
...
Consider a deep network with $L$ layers. Let $\delta \mathbf{W}^{(\ell)} = \nabla_{\mathbf{W}^{(\ell)}} \mathcal{L}$ denote the gradient of the loss with respect to the weights at layer $\ell$. If we look at the chain rule across many layers and treat each layer's Jacobian as ``roughly similar'' in scale, then the gradient at early layers can be expressed schematically as:
\begin{equation}
\delta \mathbf{W}^{(1)} \approx \left( \mathbf{W}^{(2)} \mathbf{D}^{(2)} \right) \left( \mathbf{W}^{(3)} \mathbf{D}^{(3)} \right) \cdots \left( \mathbf{W}^{(L)} \mathbf{D}^{(L)} \right) \delta \mathbf{W}^{(L)}.
    \label{eq:gradient_chain}
\end{equation}

Here $\mathbf{W}^{(\ell)}$ denotes the weight matrix connecting layers $\ell-1$ and $\ell$, while $\mathbf{D}^{(\ell)} = \operatorname{diag}\!\big(f'(z^{(\ell)})\big)$ collects the activation derivatives at layer $\ell$. The product therefore chains together Jacobians from layers $2$ through $L$. If the spectral norm (largest singular value) of each factor \(\mathbf{W}^{(\ell)} \mathbf{D}^{(\ell)}\) exceeds one, then $\|\delta \mathbf{W}^{(1)}\|$ grows exponentially with $L$, causing \textbf{exploding gradients}. Conversely, norms less than one cause $\|\delta \mathbf{W}^{(1)}\|$ to shrink exponentially, leading to \textbf{vanishing gradients}.
...
Batch normalization stabilizes the scale of activations during training by normalizing them (per channel) and then learning a scale and shift. The engineering payoff is simple: fewer layers end up saturated, so gradients behave more predictably early in training. Remember that BN behaves differently at train time (mini-batch statistics) versus evaluation time (running statistics).
