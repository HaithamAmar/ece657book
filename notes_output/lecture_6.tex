...
\Cref{chap:hopfield} used energy to tame recurrence and create stable memories. Here we return to deep feedforward perception, where convolutions and pooling impose a spatial inductive bias that improves sample efficiency and robustness. This chapter also sets up the next sequence run (\Crefrange{chap:rnn}{chap:transformers}): the optimization loop stays familiar, while dependency structure and representation choices change. The roadmap in \Cref{fig:roadmap} shows this as the deep feedforward branch.
