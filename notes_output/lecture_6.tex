...
\index{batch normalization (BN)}
\index{dropout}
\index{weight initialization}
\index{weight initialization!Xavier initialization}
\index{weight initialization!He initialization}
\index{ResNet}
\index{skip connection}
\index{data augmentation}
\index{MixUp}
\index{gradient clipping}
\graphicspath{{assets/lec6/}{assets/lec8/}}

\Cref{chap:hopfield} used energy to make recurrence stable: dynamics settle, memories persist, and failure has a concrete signature. Vision problems have a different flavor. The system must form a representation that is rich enough to support decisions, yet structured enough to learn from finite data.

Convolutional networks are the simplest version of that bargain. They keep the empirical risk minimization (ERM) workflow from \Crefrange{chap:supervised}{chap:logistic} and the gradient\hyp{}training machinery from \Cref{chap:mlp}, but make images tractable by enforcing locality and weight sharing, so the model spends capacity on patterns that recur across the grid.

There is also a useful continuity lens from earlier chapters: a convolutional filter is a learned \emph{local template} that is reused everywhere. This is the same high-level idea as prototypes/bases in \Cref{chap:rbf,chap:som}---but with strict weight sharing and end-to-end gradient training, so the templates are shaped by the task loss rather than chosen or tuned in isolation.


The second thread in this chapter is pragmatic. Deep models are only useful when they train. After the CNN mechanics, we transition into the deep\hyp{}training toolkit (initialization, activations, normalization, regularization, and optimizers) that will reappear in \Crefrange{chap:rnn}{chap:transformers}.
...
\begin{itemize}
  ...
  \item Explain receptive-field growth across layers and the role of downsampling.
  \item Read a CNN as a learned feature map and connect that viewpoint to linear margins (hinge/soft-margin) and earlier kernel-style ideas in \Cref{chap:supervised}.
  \item Choose losses/metrics deliberately and apply practical stabilizers (BN, dropout, weight decay, optimizers) by recognizing the failure signatures they address.
\end{itemize}
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Design motif}]
Keep the same statistical learning loop from \Crefrange{chap:supervised}{chap:logistic}, but move a large fraction of the ``bias'' into the architecture: locality and weight sharing.
\end{tcolorbox}

\subsection{Motivation and map}
...
CNNs emerged as a pragmatic answer: keep end-to-end gradient training, but bake in locality and translation structure through sparse connectivity and shared weights. Early convolutional networks such as LeNet established this recipe on document and zip-code recognition tasks \citep{LeCun1989,LeCun1998}. The result is a model class that is both more sample-efficient and more computationally viable on large grids.

\paragraph{How to read this chapter.}
\begin{itemize}
    \item \textbf{Core thread (CNNs):} locality + weight sharing $\rightarrow$ convolution/pooling mechanics $\rightarrow$ channels/feature maps $\rightarrow$ end-to-end classifiers.
    \item \textbf{Going deeper:} depth as receptive-field growth, and why stacked small kernels can see large spatial extent.
    \item \textbf{Training toolkit:} the stabilizers you will reuse later (initialization, activations, normalization, regularization, optimizers).
\end{itemize}
One term we will use early is \emph{receptive field}: the receptive field of a feature value is the region of the input grid that can influence it. The word is literal---outside that region, the unit does not ``receive'' signal from the input---and stacking layers grows this region.

\paragraph{Connection to the Ch.\,1 case study.}
The industrial-plant camera system introduced in \Cref{sec:intro_case_study_ai_enabled_camera_as_an_intelligent_system} is a direct application of this chapter: it takes a pixel grid as input, uses convolutional feature extraction to localise humans in the frame, and outputs an alarm signal when the occupancy score exceeds a calibrated threshold. Many of the tools discussed below---local receptive fields, weight sharing, pooling, and normalization---fit naturally into that pipeline.

\subsection{Why fully connected layers break on images}
\label{sec:cnn_why_fully_connected_layers_break_on_images}

Dense feedforward networks can be competitive on small tabular problems, but on images they become data-hungry fast for two reasons: flattening throws away the grid structure, and dense connectivity explodes parameter count. The issue is not that the model class is ``wrong''; it is that the sample complexity and compute burden are badly mismatched to the structure of vision data.

An image is naturally a 2D array (or a 3D tensor with channels). For example, a single-channel (grayscale) image of size $256 \times 256$ pixels can be represented and flattened as
\begin{equation}
\begin{aligned}
X &\in \mathbb{R}^{256 \times 256},\\
\mathbf{x} &= \mathrm{vec}(X) \in \mathbb{R}^{65,536},
\end{aligned}
\label{eq:cnn_image_vectorization}
\end{equation}
...
\[
N_{\text{samples}} \gtrsim 10 \times 6{,}553{,}600 \approx 65{,}536{,}000,
\]
...
Even if you could collect that many labeled examples, training cost becomes part of the model design. Each epoch is a full sweep over the dataset; each sweep costs time, memory bandwidth, and usually specialized hardware. Large models also cost money to store and to run: inference latency, memory footprint, and energy use become constraints that matter as much as accuracy. This is another reason architectural bias matters: it reduces degrees of freedom and makes it possible to spend computation on structure that transfers across images.

\paragraph{Overfitting risk (what to watch for).}
With high capacity, ``training accuracy goes up'' is not evidence of learning the right thing. Watch the gap between training and validation curves, check calibration (are probabilities meaningful?), and run slice audits (lighting, viewpoint, background, demographic proxies) to detect memorization or shortcut learning early.

\subsection{Sparse connectivity and parameter sharing}
\label{sec:cnn_sparse_connectivity_and_parameter_sharing}

CNNs replace dense connections with \emph{local receptive fields}. A unit's receptive field is simply the portion of the input that can influence that unit's value through the connections that reach it. The term is borrowed from sensory systems: a neuron ``receives'' evidence from a region, not from everywhere at once.
...
Here \(\sigma(\cdot)\) is used only as an activation nonlinearity, while earlier statistical chapters use \(\sigma^2\) for variance. Keep \Cref{app:notation_collisions} nearby when moving between probability and deep-learning sections.
...
\label{eq:cnn_cross_correlation}
\end{equation}
...
\begin{tcolorbox}[summarybox, title={Worked example: 1D cross-correlation values (stride + padding)}]
...
\paragraph{Convolutional hyperparameters (what you choose up front).}
...
Real inputs are multi-channel. A red\hyp{}green\hyp{}blue (RGB) image has three channels, so a \(k\times k\) filter is really \(k\times k\times C_{\text{in}}\) and produces one output map. A convolutional layer applies \(C_{\text{out}}\) such filters, yielding a volume of feature maps with shape \(H\times W\times C_{\text{out}}\). This is why ``same'' padding refers to spatial dimensions only: channel depth is set by the number of filters, not by padding.

\begin{tcolorbox}[summarybox, title={Dimensionality accounting example}]
...
\paragraph{From feature maps to classifiers.}
...
\begin{tcolorbox}[summarybox, title={Losses, metrics, and the ``CNN as features'' viewpoint}]
...
\textbf{Loss choice.} For classification, the default is softmax cross\hyp{}entropy because it turns scores into probabilities that can be audited (and sometimes calibrated) when the model is well-regularized. For regression-style outputs (counts, keypoints, continuous targets), squared error and its robust variants are common.

\textbf{Margins and hinge.} If you replace cross\hyp{}entropy with a hinge-style objective, you are pushing for a margin in the learned feature space. This is the same geometric idea as the soft-margin story in \Cref{chap:supervised}; the difference is that kernels fix the feature map ahead of time, while CNNs learn \(\phi_\theta\) from data.

\textbf{Metric choice.} Accuracy is not enough when scores are used as probabilities. Track calibration (e.g., reliability curves and expected calibration error (ECE)) and slice performance, as emphasized in \Cref{chap:logistic} and revisited in the audit boxes later in this chapter.
\end{tcolorbox}

\paragraph{Multi-branch convolution blocks (Inception idea).}
...
\paragraph{Stack depth versus receptive field.} Stacking identical \(3\times3\) filters still grows the effective receptive field: each stage wraps a thicker box around the original pixels, which explains why deep-but-narrow CNNs can capture wide spatial context without enormous kernels even when individual kernels remain small. \Cref{fig:cnn_receptive_field_growth} sketches the nested growth across depth.

\begin{tcolorbox}[summarybox, title={Quick check: receptive field growth for stacked \(3\times3\)}]
If each convolution uses stride \(1\) and a \(3\times3\) kernel, then every new layer expands the receptive field by \(2\) pixels in each spatial direction. So the effective receptive field sizes go \(3\times3\), then \(5\times5\), then \(7\times7\), and so on. Depth is therefore a controlled way to ``see farther'' without jumping to very large kernels.
\end{tcolorbox}
...
\begin{figure}[t]
    ...
    \end{tikzpicture}
    \caption{Receptive field growth across depth. Even with small \(3\times3\) kernels, stacking layers expands the spatial context seen by deeper units, which is why depth can replace very large kernels.}
...
Consider a deep network with $L$ layers, and focus on the error signals that get propagated backward. Let \(\boldsymbol{\delta}^{(\ell)}=\partial \mathcal{L}/\partial \mathbf{z}^{(\ell)}\) denote the derivative of the loss with respect to the pre\hyp{}activation at layer \(\ell\). A single backprop step has the form
\begin{equation}
\boldsymbol{\delta}^{(\ell-1)} \;=\; \left(\mathbf{W}^{(\ell)}\right)^\top \boldsymbol{\delta}^{(\ell)} \;\odot\; f'\!\left(\mathbf{z}^{(\ell-1)}\right),
...
\begin{itemize}
    \item \textbf{Exploding gradients:} gradients become extremely large, causing numerical instability and making parameters diverge.
    \item \textbf{Vanishing gradients:} gradients become extremely small in early layers, so those weights learn slowly and the model struggles to build useful low-level features.
\end{itemize}
...
\label{eq:cnn_sigmoid_derivative}
\end{equation}
...
\begin{itemize}
    \item \textbf{Loss becomes NaN (not a number) / diverges quickly:} lower learning rate; check data normalization; add gradient clipping; verify BN statistics (or switch norm).
    ...
    \item \textbf{High accuracy but poor probability decisions:} audit calibration (reliability curves, ECE) and re-threshold on held-out data rather than trusting raw scores (\Cref{chap:logistic}).
\end{itemize}
\end{tcolorbox}
...
Initialization is not just a detail: it sets the scale of activations and gradients at the start of training. A practical rule is to pick the weight variance to be inversely proportional to fan\hyp{}in (and sometimes fan\hyp{}out), so signals neither blow up nor fade as they pass through many layers. Xavier and He initializations are two standard choices that implement this idea for common nonlinearities.

\begin{tcolorbox}[summarybox, title={Initializing weights: Xavier and He}]
Both schemes derive from a variance-preservation argument: choose the initial weight scale so that, at the start of training, the variance of activations does not explode or vanish as the signal propagates through many layers.

\textbf{Xavier / Glorot initialization} targets symmetric activations near zero (tanh, sigmoid in their linear regime):
\[
...
\textbf{He / Kaiming initialization} corrects for the fact that ReLU zeros out half its inputs, so the effective fan-in is halved:
\[
...
\textbf{Rule of thumb:} use Xavier for tanh/sigmoid activations; use He for ReLU and its variants (Leaky ReLU, ELU, GELU). Without proper initialization, deep networks often train very slowly or not at all --- this single choice can mean the difference between a model that converges in hundreds of epochs and one that never moves. See \Cref{chap:backprop} for the gradient-flow view.
\end{tcolorbox}
...
Batch normalization stabilizes the scale of activations during training by normalizing them (per channel) and then learning a scale and shift. The practical payoff is that fewer layers sit in saturated regimes, so gradients behave more predictably early in training. Remember that BN behaves differently at train time (mini-batch statistics) versus evaluation time (running statistics).

\paragraph{Gradient clipping}

For exploding gradients, gradient clipping limits the maximum gradient norm during backpropagation, preventing a single bad batch from producing a destructive update. It is not a substitute for good initialization and normalization, but it is a practical safety valve when training is otherwise stable.

\paragraph{Residual (skip) connections}

Even with careful initialization and normalization, plain deep networks hit a \emph{degradation ceiling}: at some depth, accuracy on the training set itself begins to fall, indicating an optimization (not generalization) problem. \citet{He2016} solved this with a simple idea: instead of learning the desired mapping \(\mathcal{H}(\mathbf{x})\) directly, let the layers learn the \emph{residual} \(\mathcal{F}(\mathbf{x}) = \mathcal{H}(\mathbf{x}) - \mathbf{x}\) and add the identity shortcut explicitly:
\[
  \mathbf{y} = \mathcal{F}(\mathbf{x}) + \mathbf{x}.
\]
The gradient of the loss with respect to early-layer inputs now has a direct path through the ``\(+\mathbf{x}\)'' branch, bypassing the nonlinearities:
\[
...
\]
The identity term \(\mathbf{I}\) ensures that the gradient never completely vanishes, even when the learned residual is near zero. This single architectural change allowed training of networks with 50, 100, and even 1\,000+ layers and is now a universal building block of modern deep architectures.

\begin{tcolorbox}[summarybox, title={Data augmentation: expanding effective dataset size}]
A well-designed augmentation pipeline can double or triple effective training-set diversity without collecting new data. Key families:
\begin{itemize}
  \item \textbf{Geometric:} random horizontal/vertical flips, rotations ($\pm$15--30\textdegree), crops and resizes, affine shears.
  \item \textbf{Colour/photometric:} brightness, contrast, saturation, hue jitter; grayscale conversion; Gaussian blur.
  \item \textbf{Mixing augmentations:} \textbf{MixUp} linearly interpolates two training images and their labels (\(\tilde{x}=\lambda x_i+(1-\lambda)x_j\)); \textbf{CutMix} pastes a rectangular region from one image into another and mixes labels proportionally to area.
  \item \textbf{Test-time augmentation (TTA):} average predictions over multiple augmented views at inference to reduce prediction variance.
\end{itemize}
\textbf{Key rules:} (i) augmentations must be label-preserving (a flipped cat is still a cat; a horizontally flipped digit ``6'' may not be a ``6''); (ii) training and evaluation pipelines must be kept separate; (iii) report the exact augmentation recipe alongside results --- it is a model hyperparameter.
\end{tcolorbox}
...
\begin{itemize}
    \item \textbf{Train/val mismatch:} augmentation, preprocessing, and normalization must match at evaluation time (except stochastic augmentation).
    ...
    \item \textbf{Run-to-run variance:} report seeds, spread, and stopping policy; use \Cref{app:repro_standards} as the default reporting format.
\end{itemize}
\end{tcolorbox}

As a concrete diagnostic, \Cref{fig:lec6-dropout} shows how dropout can flatten the validation curve relative to the no-dropout baseline.


\begin{figure}[t]
    \centering
    \includegraphics[width=0.65\linewidth]{lec6_dropout_curves}
    \caption[Dropout effect on training/validation curves (validation flattening)]{Illustrative dropout effect on training/validation curves. Compared to a no\hyp{}dropout baseline, validation curves flatten and generalization improves; the point is the shape of the curves, not the exact values. \textit{Source: author-generated.}}
    \label{fig:lec6-dropout}
\end{figure}

\paragraph{Batch normalization.} BN accelerates convergence by keeping per\hyp{}channel activations on a stable scale and then letting the network relearn the best scale/shift. \Cref{fig:lec6-bn} contrasts the pre- and post\hyp{}normalization activation distributions; the practical point is that gradients stay in a usable range more often. In later chapters, you will see the same stability instinct show up again in LayerNorm placement choices in very deep stacks.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \nextgroupplot[title={Post-BN (per-channel)}]
            \addplot[cbBlue, thick, samples=200, domain=-6:6] {1/sqrt(2*pi*1.0)*exp(-(x^2)/2)};
        \end{groupplot}
    \end{tikzpicture}
    \caption{Batch normalization first maps each channel to a standard normal shape and then applies a learned scale and shift (\(\gamma,\beta\)); the normalized distribution shown here is the pre-affine step that stabilizes gradients.}
    \label{fig:lec6-bn}
\end{figure}


\paragraph{Adaptive optimizers.} While plain SGD remains a workhorse, Adam and related methods adapt learning rates per-parameter \citep{Kingma2015}. \Cref{fig:lec6-optimizers} sketches typical loss trajectories; Adam often converges faster initially, whereas SGD+momentum can be steadier later (and sometimes attains a slightly lower asymptote after fine-tuning).
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[cbBlue, thick, samples=120, domain=0:60] {0.15 + 0.95*exp(-x/20)}; \addlegendentry{SGD+momentum}
            \addplot[cbOrange, thick, samples=120, domain=0:60] {0.20 + 0.90*exp(-x/10)}; \addlegendentry{Adam}
        \end{axis}
    \end{tikzpicture}
    \caption{Schematic loss curves illustrating typical behavior: Adam often drops loss quickly early, while SGD+momentum can be steadier later.}
...
\begin{tcolorbox}[summarybox, title={Derivation closure: implement, cache, and fail fast}]
\begin{itemize}
    \item \textbf{Implement blocks with shapes:} treat each block (conv/BN/activation/pool) as a typed transform and keep input-output shapes explicit.
    \item \textbf{Cache what the backward pass needs:} pre-activations, normalization statistics, and masks should be stored, not re-derived ad hoc.
    \item \textbf{Fail fast:} validate shape arithmetic, then run a one-batch overfit test before full training to catch broken gradients early.
    \item \textbf{Report like an engineer:} publish seed, augmentation recipe, and calibration metrics with every benchmark, following \Cref{app:repro_standards}.
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}
    \item Letting shape accounting drift (mismatched padding/stride) until the first dense layer fails.
...
\medskip
\noindent\textbf{Ethical considerations.} CNNs are often deployed where mistakes have real cost (medical triage, access control, surveillance). Treat data provenance and slice audits as part of correctness: a high top\hyp{}1 accuracy on a benchmark does not guarantee fair or safe behavior in a new population or capture setup.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Exercises}]
\textbf{Exercise 1 (Easy).} Do one full shape-and-value sanity check: hand-compute a 1D cross-correlation for a short input and kernel, then verify the output values and shapes against a short script for several \((n,f,s,p)\) settings. \\
\textbf{Deliverable:} the hand calculation + the matching script output + a one-line rule that predicts the output length.

\smallskip
\textbf{Exercise 2 (Medium).} Test equivariance vs.\ invariance empirically: translate an image (or a simple synthetic pattern), record an intermediate feature map before pooling/striding, and show how it shifts; then show how pooling/stride changes the behavior. \\
\textbf{Deliverable:} two feature-map visualizations + one paragraph explaining what information pooling throws away on purpose.

\smallskip
\textbf{Exercise 3 (Medium).} Compare a plain deep CNN to the same network with identity skips (residual-style). Keep the dataset tiny so you can overfit fast, and treat the experiment as an optimization diagnosis rather than a leaderboard. \\
\textbf{Deliverable:} training curves for both models + one short explanation linking the difference to gradient flow.

\par\medskip
\noindent\textbf{If you are skipping ahead.} The optimization and regularization knobs here carry almost unchanged into \Crefrange{chap:rnn}{chap:transformers}; what changes is how context is represented and propagated, not the core loss-minimization discipline.
\end{tcolorbox}

\medskip
\paragraph{Where we head next.} \Cref{chap:rnn} keeps the same training and audit discipline but shifts the modeling axis from spatial locality to temporal state and sequence dependence.
