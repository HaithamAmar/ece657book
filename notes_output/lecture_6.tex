...
\Cref{chap:hopfield} used energy to make recurrence stable: dynamics settle, memories persist, and failure has a concrete signature. Vision problems have the opposite flavor. The system must form a representation that is rich enough to support decisions, but structured enough to learn from finite data.

Convolutional networks are the simplest version of that bargain. They keep the ERM workflow from \Crefrange{chap:supervised}{chap:logistic} and the gradient-training machinery from \Cref{chap:mlp}, but move a large fraction of the ``modeling bias'' into architecture. Local connectivity and weight sharing make images tractable: the model spends capacity on patterns that actually recur across a grid.
...
Use \Cref{fig:cnn_receptive_field_growth} as the architecture anchor for this section.
...
\Cref{fig:cnn_receptive_field_growth} is the receptive-field intuition reference for the depth discussion.
...
Use \Cref{fig:lec6-dropout} as the architecture anchor for this section.
...
\Cref{fig:lec6-dropout} is the validation-curve diagnostic reference for dropout decisions.
...
Use \Cref{fig:lec6-bn} as the architecture anchor for this section.
...
Use \Cref{fig:lec6-optimizers} as the architecture anchor for this section.
