...
Convolutional networks are the simplest version of that bargain. They keep the ERM workflow from \Crefrange{chap:supervised}{chap:logistic} and the gradient\hyp{}training machinery from \Cref{chap:mlp}, but make images tractable by enforcing locality and weight sharing, so the model spends capacity on patterns that recur across the grid.
...
\begin{itemize}
  ...
  \item Choose losses and metrics deliberately (e.g., cross-entropy vs.\ MSE; accuracy vs.\ calibration) for CNN-based models.
  ...
  \item Describe practical stabilizers (BN, dropout, weight decay, optimizers) and the failure signatures they address.
\end{itemize}
\end{tcolorbox}
...
\textbf{Metric choice.} Accuracy is not enough when scores are used as probabilities. Track calibration (e.g., reliability/ECE) and slice performance, as emphasized in \Cref{chap:logistic} and revisited in the audit boxes later in this chapter.
\end{tcolorbox}
...
At scale, this architectural prior became decisive once large labeled datasets and GPU training matured: the sample-efficiency and compute-efficiency gap versus classical kernel pipelines narrowed and then flipped in favor of deep convolutional stacks.
...
\paragraph{Adaptive optimizers.} While plain SGD (often called ``vanilla'' SGD) remains a workhorse, Adam and related methods adapt learning rates per-parameter \citep{Kingma2015}. \Cref{fig:lec6-optimizers} summarizes the typical loss trajectories; Adam converges faster initially, whereas SGD+momentum often attains a slightly lower asymptote after fine-tuning.
