...
\begin{equation}
\delta \mathbf{W}^{(1)} \approx \left( \mathbf{W}^{(2)} D^{(2)} \right) \left( \mathbf{W}^{(3)} D^{(3)} \right) \cdots \left( \mathbf{W}^{(L)} D^{(L)} \right) \delta \mathbf{W}^{(L)}.
    \label{eq:gradient_chain}
\end{equation}
where $\mathbf{W}$ represents the weight matrix and $f'$ is the derivative of the activation function.

Here $\mathbf{W}^{(\ell)}$ denotes the weight matrix connecting layers $\ell-1$ and $\ell$, while $D^{(\ell)} = \operatorname{diag}\!\big(f'(z^{(\ell)})\big)$ collects the activation derivatives at layer $\ell$. The product therefore chains together Jacobians from layers $2$ through $L$. If the spectral norm (largest singular value) of each factor \(\mathbf{W}^{(\ell)} D^{(\ell)}\) exceeds one, then $\|\delta \mathbf{W}^{(1)}\|$ grows exponentially with $L$, causing \textbf{exploding gradients}. Conversely, norms less than one cause $\|\delta \mathbf{W}^{(1)}\|$ to shrink exponentially, leading to \textbf{vanishing gradients}.
