...
MLPs are flexible, but they are not ``set-and-forget'' models. The main limitations show up as training instability, sensitivity to design choices, and difficulty diagnosing silent implementation mistakes.
...
\begin{itemize}
    ...
    \item \textbf{Silent implementation bugs:} shape/broadcasting mistakes can produce gradients that look plausible but are wrong. This is why the shape ledger and finite-difference checks in this chapter are not optional ``extras''.
\end{itemize}
...
At this point the mechanics are all on the table. Backpropagation is a recipe you can execute: a forward pass that produces and caches intermediate quantities, followed by a backward pass that reuses those caches to produce gradients for every layer. The rest of the chapter focuses on practical stability (activations, initialization, stopping) and the habits that keep implementations honest.
