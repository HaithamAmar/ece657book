...
\end{equation}
where $b_j^{(l)}$ is the bias term for neuron $j$ in layer $l$, and $f(\cdot)$ is the activation function, typically nonlinear (e.g., sigmoid, ReLU). Equation~\eqref{eq:forward_pass} makes it explicit that we sum over every incoming neuron $i$ in layer $l-1$ to form the affine pre-activation $z_j^{(l)}$.

\subsection{Loss and Objective}
\label{sec:backprop_loss_and_objective}

To keep the algebra uncluttered (and aligned with \Cref{chap:mlp}), we use a simple squared\hyp{}error objective. Let the network output be \(\mathbf{a}^{(L)}\) and let \(\mathbf{t}\) be the target. For classification, \(\mathbf{t}\) is often one\hyp{}hot (a vector with a 1 at the correct class index and 0 elsewhere). If you want the output to behave like a probability vector, you normally pair that target with a sigmoid/softmax output and cross\hyp{}entropy; we keep squared error here so the chain rule is the only moving part. For a single example, a standard choice is the half-squared error:
\begin{equation}
...
In \Cref{chap:mlp} we wrote a unit as \(y=f(p)\), where \(p\) is the pre\hyp{}activation and \(y\) is the post\hyp{}activation output. In this chapter we use the standard deep-learning pair \(a=f(z)\): \(p \leftrightarrow z\) and \(y \leftrightarrow a\). The calculus is unchanged; the notation just scales cleanly to many layers and batch matrix form. If you jump into chapters out of order, keep \Cref{app:notation_collisions} nearby for symbol overloads.
...
\begin{tcolorbox}[summarybox, title={Mini example (aligned with this chapter): mean squared error (MSE) backprop on a two-layer MLP}]
\footnotesize
\begin{verbatim}
...
\end{verbatim}
\normalsize
The elementwise product \texttt{*} mirrors the Hadamard (element\hyp{}by\hyp{}element) product notation from \Cref{eq:forward_pass}. The key technical point is the \texttt{fprime} factors: this is exactly where hard thresholds break the learning signal.
\end{tcolorbox}
...
\smallskip
\noindent\textbf{Interpretation.} The term \(\delta_j^{(l+1)}\) measures how sensitive the loss is to changes in the pre\hyp{}activation \(z_j^{(l+1)}\). Our task reduces to computing these \(\delta\) terms for all neurons in the network.
...
Backpropagation is reverse\hyp{}mode automatic differentiation (AD) on the network graph. A forward pass stores intermediate values (pre\hyp{}activations and activations); a backward pass reuses those stored values to compute all gradients efficiently. Frameworks (PyTorch, JAX, TensorFlow) automate the mechanics, but the manual story is what lets you sanity-check shapes, signs, and scaling when training does something surprising.

The step-by-step output-layer derivation and the hidden-layer recursion are already established in \Cref{sec:backprop_output_layer_error_terms_sub,sec:backprop_hidden_layer_error_terms_sub}. The next step is to see how those same gradients are accumulated in batch and stochastic settings.
...
\begin{itemize}
    ...
    \item \textbf{Stochastic gradient descent (SGD):} Update weights after each training example using the instantaneous gradient \(-\eta \, \delta^{(n)} \mathbf{x}^{(n)}\). The updates are noisy, but that noise can be useful: it often keeps learning moving in flat regions and can help you avoid getting stuck too early.
\end{itemize}

\begin{tcolorbox}[summarybox, title={Optimizer and stability notes}]
Start with plain SGD first; it makes it obvious whether the gradients you derived are doing the right thing. If training oscillates, add momentum; if step-size sensitivity dominates, try an adaptive method like AdamW (see \Cref{chap:cnn}). For regularization, tie this back to the L2 discussion in \Cref{chap:supervised}: in SGD, ``weight decay'' is weight shrinkage toward zero; in AdamW, that shrinkage is applied as a separate decay step rather than being mixed into the adaptive gradient. If gradients occasionally spike (common in deep or ill-conditioned nets), clipping is a practical safety valve. For classification, pair cross\hyp{}entropy with a numerically stable softmax (log\hyp{}sum\hyp{}exp / subtract the max logit), as discussed in \Cref{chap:cnn}.
\end{tcolorbox}
...
\begin{figure}[t]
    ...
    \caption[Computational graph for backpropagation (reverse-mode automatic differentiation)]{Computational graph for a feedforward network. Backpropagation is reverse\hyp{}mode automatic differentiation (AD): the forward sweep caches intermediate values, and the reverse sweep propagates deltas while accumulating weight/bias gradients from those cached values.}
...
\subsection{Training Procedure and Epochs}
\label{sec:backprop_training_procedure_and_epochs_in_multi_layer_perceptrons}

An epoch is one complete pass over the training set. Most implementations shuffle the data and iterate over mini\hyp{}batches: each update is cheaper than a full-batch gradient, and the resulting noise is often helpful for making progress.
...
A \emph{hidden layer} is any layer whose activations are not directly compared to the target: it sits between the input and output, and its outputs are internal signals the network learns to shape into useful features.
...
\begin{itemize}
    ...
    \item \textbf{Activation choice:} will gradients flow (ReLU family), or will they saturate (sigmoid/tanh) for the scale of your pre\hyp{}activations? This connects directly to the smooth-activation discussion in \Cref{sec:mlp_differentiable_activations_and_the_sigmoid_trick}.
\end{itemize}
...
\begin{itemize}
    ...
    \item Choose an activation that keeps gradients alive (ReLU is a standard choice; if you see many dead units, a leaky ReLU is a simple alternative).
...
Later deep\hyp{}learning chapters revisit these choices in larger architectures; for now, it helps to see the canonical activation functions side by side.

\paragraph{Comparing canonical nonlinearities}

With the backprop derivation in place, it helps to compare the standard nonlinearities side by side. \Cref{fig:lec4-activations} overlays the step, sigmoid, tanh, and ReLU curves so saturation regions and derivative behavior are visually apparent in one view.
...
\begin{tikzpicture}[background rectangle/.style={fill=white}, show background rectangle]
        ...
        \nextgroupplot[
            ...
            \addplot[cbOrange, thick, domain=-3:3] {1/(1+exp(-x))};
            \addplot[cbOrange, dashed, domain=-3:3] {1/(1+exp(-x))*(1-1/(1+exp(-x)))};
        \nextgroupplot[
            title={tanh},
            ymin=-1.1, ymax=1.1,
            ytick={-1,0,1},
...
\centering
\small
\caption{Activation derivative quick reference. Forward-pass activations can be reused directly for sigmoid and tanh derivatives; ReLU depends on the sign of the pre-activation.}
\begin{tabular}{@{}llll@{}}
\toprule
Activation & \(f(z)\) & \(f'(z)\) & Reuse from forward pass? \\
\midrule
Sigmoid & \(\sigma(z)\) & \(\sigma(z)\bigl(1-\sigma(z)\bigr)\) & Yes (\(a(1-a)\)) \\
\(\tanh\) & \(\tanh(z)\) & \(1-\tanh^2(z)\) & Yes (\(1-a^2\)) \\
ReLU & \(\max(0,z)\) & \(\mathbf{1}_{z>0}\) & Yes (sign of pre-activation) \\
\bottomrule
\end{tabular}
\end{table}
For ReLU, take derivative \(0\) at the origin.

\paragraph{Trade-offs}
Some activations match the intuition of a smooth ``firing curve''; others are chosen mainly because they train well. Sigmoid and tanh saturate at large magnitude inputs, which slows gradients in deep networks. ReLU avoids saturation on the positive side but can produce ``dying ReLUs'' when biases push units negative and the gradients become zero; if many units stall, use He initialization, reduce the learning rate, or swap to a leaky ReLU with a small negative slope (e.g., 0.01).
...
\subsection{Applications of Backpropagation-Trained Networks}
...
\subsection{Limitations and Known Failure Modes of MLPs}
...
At this point you have the full backprop workflow: store computations from the forward pass, compute \(\delta^{(L)}\), propagate \(\delta\)'s backward, and assemble gradients with the shapes kept honest. To close the derivation, it helps to write the same story once more in a compact matrix form.
...
\end{aligned}
\label{eq:backprop_delta_recursion}
\end{equation}
...
\begin{equation}
    \begin{aligned}
        \frac{\partial \mathcal{L}}{\partial \mathbf{W}^{(l)}} &= (\mathbf{A}^{(l-1)})^\top \boldsymbol{\delta}^{(l)},\\
        \frac{\partial \mathcal{L}}{\partial \mathbf{b}^{(l)}} &= \mathbf{1}^\top \boldsymbol{\delta}^{(l)}.
    \end{aligned}
    \label{eq:backprop_param_grads}
\end{equation}
...
\begin{itemize}
    \item Backpropagation is the chain rule organized: cache forward values, propagate \(\delta\)'s backward using \Cref{eq:backprop_delta_recursion}, then assemble gradients with \Cref{eq:backprop_param_grads}.
    ...
    \item For softmax + cross\hyp{}entropy, the output-layer error simplifies to \((\hat{\mathbf{Y}}-\mathbf{Y})/B\); a small finite-difference check can confirm this identity, and it is used in the modern-practice code pattern earlier in the chapter.
...
\end{itemize}
    ...
    \begin{itemize}
        ...
        \item Stop when \(L_{\text{val}}\) has not improved for \(k\) consecutive epochs (a patience of \(k\in[5,10]\) is a reasonable first try). If the curve is noisy, require a small minimum improvement before you reset patience.
...
\begin{itemize}
    ...
    \item \textbf{Make runs comparable:} record seeds, optimizer, schedule, and stopping rule. The reporting checklist in \Cref{app:repro_standards} helps keep comparisons honest.
\end{itemize}
\end{tcolorbox}

\begin{tcolorbox}[summarybox, breakable, title={Backpropagation (dense MLP) pseudocode}]
\footnotesize
\begin{verbatim}
function MLPTrainStep(X, Y, params, eta):
    # X: B x d_in, Y: B x d_out; params: {W[l], b[l]} for l=1..L
    # Forward (cache everything the backward pass will reuse)
    A[0] <- X
    for l in 1..L:
        Z[l] <- A[l-1] @ W[l]
        Z[l] <- Z[l] + b[l]   # b broadcast across the batch
        A[l] <- f_l(Z[l])
    # Backward (delta[l] = dL/dZ[l])
    dA <- dLoss_dA(A[L], Y)
    delta[L] <- dA * fprime_L(Z[L])
    for l in L..1:
        gradW[l] <- transpose(A[l-1]) @ delta[l] / B
        gradb[l] <- sum_rows(delta[l]) / B
        if l > 1:
            dAprev <- delta[l] @ transpose(W[l])
            delta[l-1] <- dAprev * fprime_{l-1}(Z[l-1])
    # Update
    for l in 1..L:
        W[l] <- W[l] - eta * gradW[l]
        b[l] <- b[l] - eta * gradb[l]
...
\end{verbatim}
\normalsize
This matches the matrix-form recap: the forward pass produces activations and caches; the backward pass propagates layerwise error signals and assembles gradients; the update step is an optimizer choice (plain SGD shown here). The most common implementation bugs are shape errors and missing transposes, so keep the shape ledger and one tiny gradient check close.
\end{tcolorbox}

\begin{tcolorbox}[summarybox, title={Exercises}]
\textbf{Exercise 1 (Easy).} For the pseudocode box above, write the \emph{cache contract}: list exactly what must be stored during the forward pass so the backward pass can be computed without re-running forward computations. \\
\textbf{Deliverable:} a one-page ``shape + cache ledger'' (names, shapes, and where each cached tensor is used).

\smallskip
\textbf{Exercise 2 (Medium).} Implement the 2-layer MLP train step (forward + backward) and run a finite-difference gradient check on a tiny batch. Use the same loss convention as the chapter (mean over batch) and report the worst relative error across parameters. \\
\textbf{Deliverable:} the max relative error + the first bug you found (transpose, batch scaling, or a cached value) and how you diagnosed it.

\smallskip
\textbf{Exercise 3 (Medium).} Create a controlled ``gradient flow'' stress test: keep the dataset fixed and vary only (i) activation (sigmoid/tanh/ReLU) and (ii) initialization scale. Track per-layer gradient norms over the first 50 updates and relate the curves to the saturation/derivative picture in \Cref{fig:lec4-activations}. \\
\textbf{Deliverable:} one plot of gradient norms by layer + one paragraph explaining which failure mode you observed (vanishing, exploding, or silent plateaus) and why.
    \end{tcolorbox}
    \medskip
    \noindent\textbf{If you are jumping chapters.} Keep one habit: run a tiny gradient check before scaling up.

    \medskip
    \paragraph{Where we head next.} \Cref{chap:rbf} is a deliberate contrast chapter, not a kernel detour. The representation remains nonlinear, but the training decomposition changes: instead of learning every layer end-to-end, we first choose localized basis functions and then solve a linear readout more directly. This gives a second answer to the question ``where should the nonlinearity live?'' before we move to prototype organization in \Cref{chap:som} and energy-based recurrence in \Cref{chap:hopfield}.
