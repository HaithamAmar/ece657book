...
Building on the two\hyp{}neuron derivation in \Cref{chap:mlp}, we now derive backpropagation as a systematic application of the chain rule to an $L$-layer network. The key idea is unchanged: compute local error terms (the \(\delta\)'s), then reuse them to obtain all weight gradients efficiently. \Cref{fig:roadmap} marks this chapter as the training engine for deep models.
...
Use \Cref{fig:backprop-computational-graph} as the derivation anchor for this section.
...
Use \Cref{fig:lec4_backprop_flow} as the derivation anchor for this section.
...
Use \Cref{fig:lec4-activations} as the derivation anchor for this section.
