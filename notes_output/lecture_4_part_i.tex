...
Changing one early weight perturbs the hidden activations, which perturbs every downstream pre\hyp{}activation, which perturbs the final loss. The influence is a ``ripple'' through the computation graph. Backpropagation does not change that cause-and-effect; it just computes the ripple efficiently via the chain rule: cache forward values (and local slopes) once, then sweep backward reusing them. For example, \Cref{chap:mlp} highlighted the sigmoid identity \(\sigma'(p)=y(1-y)\) as a small instance of this cache\(\rightarrow\)reuse pattern.
\end{tcolorbox}
...
Backpropagation is the bookkeeping device that makes this tractable: it packages those intermediate effects into reusable local error terms (the \(\delta\)'s) so you can compute all gradients with one backward sweep.
...
\begin{itemize}
    ...
    \item \textbf{Gradient norms:} Track \(\|\nabla \mathbf{W}^{(l)}\|\) per layer; look for dead layers (all zeros) or explosions.
    ...
    \item \textbf{Shape assertions:} Verify that \(\mathbf{Z}^{(l)}, \mathbf{A}^{(l)},\boldsymbol{\delta}^{(l)}\) have the expected batch shapes and that bias broadcasts correctly.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Backward pass:} compute \(\nabla_{\mathbf{W}^{(\ell)}} \mathcal{L}\) using the cached activations/masks so that dropped units contribute zero gradient.
...
\begin{itemize}
    \item \textbf{Implement:} encode one canonical forward signature \((\mathbf{A}^{(l-1)}, \mathbf{W}^{(l)}, \mathbf{b}^{(l)}) \mapsto (\mathbf{Z}^{(l)}, \mathbf{A}^{(l)})\), then reuse it for every layer.
    \item \textbf{Cache:} store \((\mathbf{A}^{(l-1)}, \mathbf{Z}^{(l)})\) per layer so backward passes only apply local Jacobians and matrix multiplications.
