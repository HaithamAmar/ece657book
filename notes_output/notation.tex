...
\begin{table}[h]
\centering
\caption{Book-wide notation summary. Reference when a symbol's default meaning is unclear before a chapter-local notation note appears.}
\begin{tabular}{@{}ll@{}}
\toprule
...
\begin{itemize}
    ...
    \item For activation notation, \(f(\cdot)\) is the generic placeholder used when the chapter does not commit to a specific nonlinearity, \(\sigma(\cdot)\) is the logistic sigmoid, and \(\varphi(\cdot)\) denotes radial-basis responses; \(\phi(\cdot)\) remains reserved for generic feature maps.
